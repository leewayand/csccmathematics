...
We have seen that addition (or subtraction) of constants shifts the domain or range and shifts the graph as a rigid object - the movement is the same for every point. 
...
$g$ represents numbers in the domain of $W$ and $2g$ represents numbers in the domain of $T$.  
...
The only peculiarity to the stretching idea is $0$.  Since $2 \cdot 0 = 0$, whenever $g = 0$, then $2g = 0$.  While the domain is being stretched, it is being stretched away from $0$. Stretching cannot move $0$. It gets pinned down where it is.
...
What if we multiply by $-2$ rather than $2$?
...
\textbf{\textcolor{blue!55!black}{$\blacktriangleright$}} Let's define a new function again based on $T$.
...
It is like the domain of $T$ is on a rubberband and it shrunk in half and then reflected around $0$.  As $r$ walks through the domain of $M$ from negative to positive (left to right on the number line), the corresponding walking in the domain of $T$ is from positive to negative (right to left).
...
The domain shrinks around $0$. $M(0) = T(0) = 0$.  The rubberband is pinned at $0$, because any multiple of $0$ is still $0$.  
...
Multiplication of the domain by a constant doesn't change the shape of the graph.  It might squish it horizontally or stretch it horizontally, but the shape remains.  All the function characteristics are relatively the same in each graph when you view them from $0$. The maximums and minimums are still in relatively the same places when viewed from $0$.  

You may have to view this in reverse, if the multiplication coefficient was negative, but all the characteristics and features remain.
...
\textbf{\textcolor{red!70!black}{Composition : Inside = Domain = Horizontal}}


Shifting and stretching horizontally can be viewed algebraically as composition with an inner linear function.



Suppose we have a function, $G(x)$. 

Suppose we want to create a new function, $F(t)$, by shifting and stretching the domain of $G$.
...
We can view this as composition with an inner function.

Let $L(w) = A \, w + B$


\[ F(t) = G(A \, t + B) = (F \circ L_{in})(t) = F(L_{in}(t))\]




\begin{align*}
F(t) =& G(3t-2) =& (G \circ L)(t)  & \text{ where } L(w) = 3w-2 \\
F(t) =& G(6-t) =& (G \circ L)(t)   & \text{ where } L(w) = 6-w    \\
F(t) =& G(5t+7) =& (G \circ L)(t)  & \text{ where } L(w) = 5w+7   \\
F(t) =& G(1-8t) =& (G \circ L)(t)  & \text{ where } L(w) = 1-8w    \\
F(t) =& G(-4t+9) =& (G \circ L)(t) & \text{ where } L(w) = -4w+9   
\end{align*}
...
\subsection*{Stretching Vertically}
...
The induced domain of $K$ is equal to the domain of $W$.  The coefficient $2$ is multiplying the function values of $W$, which doesn't affect the domain.
...
$m$ has a global minimum represented by the left endpoint of the middle line segment. 

$m$ has a global maximum represented by the left endpoint of the right line segment.
...
Multiplying a function by a number smaller than $1$, squeezes the graph vertically.  Our word for this is ``compress''.  Multiplying a function by a number between $-1$ and $1$ compresses the graph vertically.
...
Neither shifting nor stretching (or compressing) changes the shape of the graph.  The extreme features may change their values, but they remain relatively in the same position.  The shape may be reflected about one (or both) of the axes, but the shape is the same relative to the rest of the graph, just drawn in reverse from the original.
...
\textbf{\textcolor{red!70!black}{Composition : Outside = Range = Vertical}}


Shifting and stretching vertically can be viewed algebraically as composition with an outer linear function.



Suppose we have a function, $G(x)$. 

Suppose we want to create a new function, $F(t)$, by shifting and stretching the range (function values) of $G$.
...
\[ F(t) = A \, G(t) + B = (L_{out} \circ F)(t) = L_{out}(F(t)) \]




\begin{align*}
F(t) & = \, 3 \, G(t)-2 & = (L \circ G)(t)  & \text{ where } L(w) = 3w-2 \\
F(t) & = \, 6-G(t) & = (L \circ G)(t)   & \text{ where } L(w) = 6-w    \\
F(t) & = \, 5 \, G(t)+7 & = (L \circ G)(t)  & \text{ where } L(w) = 5w+7   \\
F(t) & = \, 1 - 8 \, G(t) & = (L \circ G)(t)  & \text{ where } L(w) = 1-8w    \\
F(t) & = \, -4 \, G(t)-9 & = (L \circ G)(t) & \text{ where } L(w) = -4w-9   
\end{align*}
