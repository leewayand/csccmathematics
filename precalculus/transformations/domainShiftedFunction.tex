...
The composition of $f$ and $g$ is a third function denoted by $f \circ g$.
...
The composition uses the output of $g$ as the input to $f$.

$\blacktriangleright$ The domain of $f \circ g$ is a subset of the domain of $g$.\\
$\blacktriangleright$ The range of of $f \circ g$ is a subset of the range of $f$.


The domain of $f \circ g$ includes all the domain numbers for $g$ such that their $g$-value is in the domain of $f$.

The range of $f \circ g$ includes all the range numbers for $f$ such that there is a $g$-value in the domain of $f$.


Our first investigation into composition will be with linear functions.
...
How does each of these affect $f$?
...
One way to create a new function from an existing function is to shift all the domain numbers in the function pairs. 
...
We'll begin with a graphical description and then translate it to an algebraic description. 
...
With $T$ as our initial function, we can build a new function through composition.
...
\subsection*{A New Function : B}

$B(k)$ is a new function we are creating, but its definition is based on $T(v)$.


\textbf{Define $B$ by the equation \textcolor{blue!55!black}{$\blacktriangleright$ $B(k) = T(k+3)$} with the induced domain.}


Induced domain means that the domain of $B$ will be deduced from the domain of $T$.
...
\begin{explanation} \textbf{\textcolor{red!75!green}{Domain}} 
...
This domain was ``induced'' from the domain of $T$ through the definition of $B$, which is using $T$.  The values in the domain of $B$ are real numbers, $k$, such that $k+3$ is in the domain of $T$.

The domain of $B$ is a shift of the domain of $T$.  
...
$T$ is $B$ with the domain shifted. All the values of $B$ are still there. They just have new domain partners.  

All the points from the graph of $B$ are still there.  They just have new first coordinates.  They have moved horizontally.


\textbf{\textcolor{purple!85!blue}{Second Question:}}  The graph of $T$ has four strategic points.  What are the corresponding points on the graph of $B$?
...
\textbf{\textcolor{blue!55!black}{$\blacktriangleright$}} How do these translate to strategic points on the graph of $B$?
...
The strategic points on the graph of $B$ are connected in the same way they were connected in the graph of $T$.
...
\textbf{\textcolor{purple!85!blue}{Third Question:}} What is the formula for $B(k)$?



\begin{explanation} \textbf{\textcolor{red!75!green}{On $k \in (-7,-4]$}}


We have a formula for $T(v)$.  We are attempting to build a formula for $B(k)$ from the formula for $T(v)$.

We are given that $B(k) = T(k+3)$.  We are given that $v = k+3$ and we already know the values of $v$ for $T$.  We want to know the corresponding values of $k$.

The first part of the definition of $T$ says $T(v) = 2v - 1$ on $(-4,-1]$.

\[
v \in (-4,-1]
\]

When $v \in (-4,-1]$, then $v = k+3 \in \left(\answer{-4},\answer{-1}\right]$.  

That gives us $k \in \left(\answer{-7},\answer{-4}\right]$ .


For the first interval in the definition of $T$, we are using the formula $T(v) = 2v - 1$. 

Now $v = k + 3$, with $k \in (-7,-4]$.
...
\begin{explanation} \textbf{\textcolor{red!75!green}{On $k \in [-2,4)$}} 
...
The first part of the definition of $T$ says $T(v) = -v + 3$ on $[1,7)$.

\[
v \in [1,7)
\]

When $v \in [1,7)$, then $v = k+3 \in \left[\answer{1},\answer{7}\right)$.  

That gives us $k \in \left[\answer{-2},\answer{4}\right)$ .


For the second interval in the definition of $T$, we are using the formula $T(v) = -v + 3$. 

Now $v = k + 3$, with $k \in [-2, 4)$.
...
\begin{warning} \textbf{\textcolor{red!70!black}{Variables}} 
...
The graph of $V$ has $7$ strategic points, $6$ endpoints and a vertex.
...
$x$ represents the domain values of $f$.   

If $x - 1 \in [-6, 6)$, then $x \in \left[\answer{-5}, \answer{7}\right)$.  


This is the domain for $f$.  It is shifted $1$ to the right from the domain of $V$.  

The individual domain intervals become $[-5, -1]$, $\left(\answer{-1}, \answer{5}\right]$, and $[5, 7)$.
...
Let's try shifting $V$ from the other side of the definition.
...
Define a new function, $g(y)$, by $g(x-3) = V(x)$ with the induced domain.



What is a formula for $g$?

What does the graph of $g$ look like?
...
The domain of $V$ is $[-6, 6)$.  These values are represented by $x$ in this definition, because $x$ is the expression inside the parentheses for $V$ and that is where the expression lives for the domain values of $V$.

$x-3$ represents the domain values of $g$.   

If $x \in [-6, 6)$, then $y = x-3 \in \left[\answer{-9}, \answer{3}\right)$.  


This is the domain for $g$.  It is shifted $3$ to the left from the domain of $V$.  

The individual domain intervals become $[-9, -5]$, $\left(\answer{-5}, \answer{1}\right]$, and $[1, 3)$.



Now for a formula for $g(y)$.

If $g(x-3) = V(x)$, then $g(y) = V(y+3)$
...
\begin{cases}
  -2(y+3)-3 & \text{ on } [-9, -5]   \\
  -((y+3)+3)((y+3)-3) & \text{ on } (-5, 1]  \\
  \frac{7(y+3)}{4} - 8 & \text{ on } (1, 3)
\end{cases}
\]
...
\begin{tikzpicture} 
  \begin{axis}[
            domain=-10:10, ymax=10, xmax=10, ymin=-10, xmin=-10,
            axis lines =center, xlabel=$y$, ylabel={$z=g(y)$},
            ytick={-10,-8,-6,-4,-2,2,4,6,8,10},
            xtick={-10,-8,-6,-4,-2,2,4,6,8,10},
            ticklabel style={font=\scriptsize},
            every axis y label/.style={at=(current axis.above origin),anchor=south},
          ...
          \addplot [line width=2, penColor, smooth,samples=100,domain=(-9:-5)] {-2*x-9};
          \addplot [line width=2, penColor, smooth,samples=100,domain=(-5:1)] {-1*(x*(x+6))};
          \addplot [line width=2, penColor, smooth,samples=100,domain=(1:3)] {1.75*(x+3)-8};




      \addplot[color=penColor,fill=penColor,only marks,mark=*] coordinates{(-9,9)};
      ...
      \addplot[color=penColor,fill=penColor,only marks,mark=*] coordinates{(1,-7)};

      \addplot[color=penColor,fill=white,only marks,mark=*] coordinates{(1,-1)};
      \addplot[color=penColor,fill=white,only marks,mark=*] coordinates{(3,2.5)};
...
\begin{center}
\textbf{\textcolor{red!70!black}{But that is NOT how it is always presented to us.}}
\end{center}
...
It is reversed from how it was originally defined, $g(x) = f(x-8)$, because the ``inside'' of $f$ is $x-8$ here.  That is not what is done to $t$.  When we solve $t=x-8$ for $x$, we reverse all of the arithmetic and discover what was done to $t$ to get $x$.
...
more examples can be found by following this link\\ \link[More Examples of Transformations]{https://ximera.osu.edu/csccmathematics/precalculus/precalculus/transformations/examples/exampleList}
