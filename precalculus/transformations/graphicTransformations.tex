...
The new variable, $k$, is obtained from the old variable, $x$, by subtracting $B$ and then dividing by $A$ (or multiplying by $\frac{1}{A}$). 
...
\textbf{\textcolor{blue!55!black}{$\blacktriangleright$ Now define a new function based on $G$.}}
...
\begin{itemize}
 \item \textbf{Domain of $T$:}  $p = 2 (h+4)$ 
    \begin{itemize}
      \item (1) add $4$ to the domain of $G$
      \item (2) then multiply by $2$
    \end{itemize}

 \item \textbf{Range of $T$:}  $T = -2 G + 1$ 
    \begin{itemize}
      \item (1) multiply $G$ by $-2$
      \item (2) then add $1$
    \end{itemize}
\end{itemize}
...
The table above shows that the transformations follow the order of operations applied to the original function and graph.  The only thing to keep in mind is that you must phrase the arithmetic of the transformation as being applied to the original function notation.  
...
On the inside of the domain parentheses, we have $\frac{1}{2} p - 4$.  Remember, $h$ is the variable representing domain numbers of $G$ and $p$ is the variable representing domain numbers of $T$. The new function has $h = \frac{1}{2} p - 4$. Solving for $p$, gives us $p = 2(h+4)$.  Now we can read off the arithmetic according to the \textbf{order of operations}.  
...
Each relative graphical relationships within each graph is also a relationship in the other graph. 
...
What are the graphical effects?
...
The original corner was at $x=0$.  This moved right to $1$ and then was compressed to $\frac{1}{3}$.
