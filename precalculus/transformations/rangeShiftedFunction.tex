...
One way to create a new function from an existing function is to shift all the range numbers (function values) in the function pairs. 
...
\textbf{\textcolor{blue!55!black}{$\blacktriangleright$}} How do we describe a shifting range? 

We'll begin with a graphical description and then translate it to an algebraic description. 



Watching the effects on the graph is helpful to understanding the algebra.



\begin{example} The Initial Function
...
On the interval $(-4, -1]$, the graph is a line segment for $T(v) = \answer{2v-1}$.   

There should be a(n) \wordChoice{\choice[correct]{open} \choice{closed}} dot when $v = -4$.  


There should be an \wordChoice{\choice{open} \choice[correct]{closed}} dot when $v = -1$.
...
On the interval $[1, 7)$, the graph is another line segment for $T(v) = \answer{-v+3}$. 

There should be a(n) \wordChoice{\choice{open} \choice[correct]{closed}} dot when $v = 1$.  


There should be an \wordChoice{\choice[correct]{open} \choice{closed}} dot when $v = 7$.
...
Using $T$ as an initial function, let's shift the range.
...
\textbf{\textcolor{blue!55!black}{$\blacktriangleright$ Let's define a new function based on $V$.}} 
...
The places in the domain where characteristics and features occur did not change.  The maximums and minimums have all dropped by $2$.  The endpoints are all still solid or hollow.  Their vertical coordinates have all dropped by $2$. Their horizontal coordinates have remained the same.
...
Define $m$ by the graph of $y = m(f)$ below.
...
To see that $2$ is a local maximum at $-3$, imagine the interval $(-3.1, -2.9)$. For all the domain numbers in this interval, $T(-3)=2$ is the maximum.  This is automatically satisfied, since $3$ is the only domain element of $m$ inside this interval.  
...
\textbf{\textcolor{blue!55!black}{$\blacktriangleright$ Now, we'll shift the range.}} 
...
Adding or subtracting a constant from the function, as opposed to the domain, shifts the graph up and down.  The shape of the graph doesn't change.  All the characteristics and features of the function, like maximums and minimums, occur at the same places in the domain.  Their values just change accordingly.
...
Let $B(r) = |r-3| + 2$ with its natural domain.  


We have shifted the absolute basic value function to the \wordChoice{\choice{left} \choice[correct]{right}} by \wordChoice{\choice{-3} \choice{-2} \choice{2} \choice[correct]{3}} and \wordChoice{\choice[correct]{up} \choice{down}}  by \wordChoice{\choice[correct]{2} \choice{3}}.
