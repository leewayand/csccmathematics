...
\begin{itemize}
\item The \textbf{global maximum value} is the greatest value of the function on its whole domain.  
...
\begin{warning} \textbf{\textcolor{red!70!black}{Multiple Locations}}
...
\begin{warning} \textbf{\textcolor{red!70!black}{None}}
...
In contrast to a single maximum or minimum value, a function may also have values which are the greatest value in their own little neighborhood of the domain, but not necessarily the greatest overall value.  We see these visually encoded as tops of hills and bottom of valleys on the graph.  A function may have many of these \textbf{local} or \textbf{relative} extrema.
...
\textbf{Note:} We picked $0.3$, $0.4$, and $0.5$ for $\epsilon$, but we could have selected any small, positive numbers that work. In fact, once one positive number works, then any less positive number will also work. 
...
\begin{explanation} \textbf{\textcolor{red!75!green}{Local Extrema for $B(w)$}} 
...
However, the real numbers do not have a ``next'' property.  There is no ``next'' number for any real number.
