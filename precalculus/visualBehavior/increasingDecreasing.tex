...
We use functions to compare information (measurements). One aspect we are interested in is how one measurement changes compared to changes in the other. 
...
\item If the number of accidents goes up when the speed limit goes up, then we say that accidents increase with respect to speed limit.
\end{itemize}
...
When the domain values become greater, then the function values also become greater.  

When the domain values become lesser, then the function values also become lesser.  
...
\textbf{Note:} You cannot merely test the endpoints of an interval to show a function is increasing throughout the interval.  It must increase for EVERY possible pair of numbers in the interval.
...
When the domain values become greater, then the function values become lesser.  

When the domain values become lesser, then the function values become greater.  
...
\textbf{Note:} You cannot test the endpoints of an interval to show a function is decreasing throughout the interval.  It has to decrease for EVERY possible pair of numbers in the interval. 
...
In mathematics, the words \textbf{increase} and \textbf{decrease} are seemingly used in two different ways. 
...
As we step through the positions, the values of the numbers become greater, or become lesser, or stay the same. 

\begin{itemize}
    \item If the value of the numbers becomes greater as we step through the positions of the numbers, then we say the list (sequence) of numbers \textbf{increases}.  
    \item If the value of the numbers becomes lesser as we step through the positions of the numbers, then we say the list (sequence) of numbers \textbf{decreases}. 
\end{itemize}
...
A function is also kind of like a list of numbers with positions.  The domain numbers play the role of position and the function values play the role of list numbers sitting in those positions. 

We compare the change in the function numbers to the change in the domain numbers. 

\begin{itemize}
    \item If the function values become greater as the domain numbers become greater, then we say the function \textbf{increases}.  
    \item If the function values become lesser as the domain numbers become greater, then we say the function \textbf{decreases}.  
\end{itemize}

We tend to read left to right with graphs, but that is only half the story.  Everything can be read right to left. 


\begin{itemize}
    \item If the function values become greater as the domain numbers become lesser, then we say the function \textbf{decreases}.  
    \item If the function values become lesser as the domain numbers become lesser, then we say the function \textbf{increases}.  
\end{itemize}
...
\begin{itemize}
    \item \textbf{\textcolor{purple!85!blue}{If the function values and domain numbers change in the same way (both become greater or lesser), then we say the function \textbf{increases}}}.  
...
However, $m$ is not decreasing on the interval $(-8, 8)$.  To show that a function is not decreasing on an interval, we merely give ONE \textbf{counterexample}. That is we need to come up with ONE pair of numbers $a < b$ in the interval where $f(a) \ngtr f(b)$.  Let's select $a = -4$ and $b = -3$.
...
\begin{idea} \textbf{\textcolor{blue!55!black}{Counterexample}} 

A \textbf{counterexample} is one example where all the required conditions of a statement are met, yet the conclusion is not true.  
...
\begin{definition}  \textbf{\textcolor{green!50!black}{Rate of Change over an Interval}} 
...
$\blacktriangleright$ Rate of change is a function idea that we will connect to the geometric idea of slope.





The rate of change over an interval is an overall measurement.  It compares the changes occurring over the whole interval. It compares final values to initial values. 
...
Given an interval, we can measure the change of a linear function over this interval. 
...
Later in Calculus, we will more closely examine sequences.  We will describe and represent them as functions.  Their domains will be the natural numbers (positions), rather than the real numbers. 


That will bring all our versions of increasing and decreasing together under one umbrella.
