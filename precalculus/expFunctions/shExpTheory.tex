...
\begin{definition} \textbf{\textcolor{green!50!black}{General Shifted Exponential Functions}}
...
\textbf{For Instance,} consider $f(x) = 2^{x} + 1$. 
...
So, we examine shifted exponential functions as we examined expoential functions.




\begin{idea}  \textbf{\textcolor{blue!55!black}{Shifted Exponential Compositions}} 

We can view shifted exponential functions as compositions


\begin{itemize}
\item Let $L_{out}(u) = A \, u + D$, a linear function
\item Let $E(t) = r^t$, a core shifted exponential function
...
\[  Exp(x) =    A \cdot r^{B \, x + C} + D = (L_{out} \circ E \circ L_{in})(x) \]
...
Compared to a exponential function graph, where the horizontal axis is the horizontal asymptote, here, the horizontal asymptote has been moved down $\answer{7}$ units, because the constant term in the formula is $-7$.
...
\textbf{Zeros:}  Shifted exponential functions can have zeros.
...
$\blacktriangleright$ \textbf{Thinking of formula pieces...}
...
\begin{itemize}
\item Let $L_{out}(u) = \frac{1}{3} \, u - 7$, an increasing linear function, since the leading coefficient is positive.
\item Let $E(t) = 2^t$, an increasing core shifted exponential function, since the base is greater than $1$.
\item Let $L_{in}(v) = v + 2$, an increasing linear function, since the leading coefficient is positive.
\end{itemize}


\[
f(x) = \frac{1}{3} \cdot \, 2^{x+5} - 7 = (L_{out} \circ E \circ L_{in})(x)
\]
...
We now have a simplier shifted exponential function.  The base hasn't changed. It is still $2$. The new leading coefficient is $\frac{32}{3}$.
...
We now have a simplier shifted exponential function.  The new base is $\answer{9}$ and the new leading coefficient is $\answer{\frac{4}{3}}$.
...
Our exponent here is $3 - t$.  Our function's exponent is zero, $3-t=0$ when $t=3$. Our one anchor point for the4 graph is shifted over to $3$.  Multipying by $-2$, means the dot is $2$ away(below) from the horizontal asymptote, which is now $y=4$.  Our anchor point is $(3, 2)$.
...
$\blacktriangleright$ \textbf{Thinking about the formula pieces...}
...
\begin{itemize}
\item Let $L_{out}(u) = -2 \, u + 4$, a decreasing linear function, since the leading coefficient is negative.
\item Let $E(x) = \left( \frac{2}{3} \right)^x$, a decreasing core shifted exponential function, since the base is less than $1$.
...
\begin{example}  Shifted Exponential Function and Graph
...
more examples can be found by following this link\\ \link[More Examples of Exponential Functions]{https://ximera.osu.edu/csccmathematics/precalculus/precalculus/expFunctions/examples/exampleList}
