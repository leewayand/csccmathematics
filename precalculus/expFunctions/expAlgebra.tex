\documentclass{ximera}

\input{../../preamble.tex}

\title{Algebra}

\begin{document}

\begin{abstract}
exponential form
\end{abstract}
\maketitle











The template for the formula of the basic exponential function looks like



\[  A \cdot r^x    \, | \,  A \ne 0, \, r > 0 \text{ and } r \ne 1  \]




Exponential functions are those functions that \textbf{\textcolor{red!70!black}{CAN}} be represented by a formula of the forms $A \cdot r^x$, $A \cdot r^{B \, x}$, or $A \cdot r^{B \, x + C}$.
















\begin{example}  Exponential Functions 



$f(x) = 7 \cdot 2^{3x+1} $ represents an exponential function, because this formula can be rewritten in the form $A \cdot r^x$.   


\textbf{\textcolor{red!75!green}{explanation}} 


\[
f(x) = 7 \cdot 2^{3x+1}
\]


\[
f(x) = 7 \cdot 2^{3x}  \, 2^1
\]

\[
f(x) = 7 \cdot 2 \, 2^{3x} 
\]

\[
f(x) = 7 \cdot 2 \, (2^3)^x 
\]

\[
f(x) = 14 \cdot 8^x 
\]


}

\end{example}






















\begin{example}  Exponential Functions 



\[ g(t) = \frac{5 \cdot 3^{8 t + 9}}{7 \cdot 3^{6 t + 4}} \]

represents an exponential function, because this formula can be rewritten in the form $A \cdot r^t$.   


\textbf{\textcolor{red!75!green}{explanation}} 


\[ g(t) = \frac{5 \cdot 3^{8 t + 9}}{7 \cdot 3^{6 t + 4}} \]



\[ 
g(t) = \frac{5}{7} \cdot \frac{3^{8 t + 9}}{3^{6 t + 4}} 
\]



\[ 
g(t) = \frac{5}{7} \cdot 3^{(8 t + 9)-(6 t + 4)}
\]




\[ 
g(t) = \frac{5}{7} \cdot 3^{2 t + 5}
\]


\[ 
g(t) = \frac{5}{7} \cdot 3^5 \cdot 3^{2 t}
\]



\[ 
g(t) = \left( \frac{5}{7} \cdot 3^5 \right) \cdot (3^2)^t
\]




\[ 
g(t) = \left( \frac{5}{7} \cdot 3^5 \right) \cdot 9^t
\]










\end{example}


















\begin{example}  Exponential Functions 



$H(k) = 6 \sqrt{e^{8 k + 6}} $ represents an exponential function, because this formula can be rewritten in the form $A \cdot r^x$.   



\textbf{\textcolor{red!75!green}{explanation}} 


\[
H(k) = 6 \, \sqrt{e^{8 k + 6}}
\]


\[
H(k) = 6 \, \left( e^{8 k + 6} \right)^{\tfrac{1}{2}}
\]

\[
H(k) = 6 \, \left( e^{(8 k + 6 ) \cdot \tfrac{1}{2}} \right)
\]

\[
H(k) = 6 \, \left( e^{(4 k + 3)} \right)
\]

\[
H(k) = \left( 6 \cdot \answer{e^3} \right) \left( \answer{e^4} \right)^k
\]




\end{example}

















\begin{example}  Exponential Functions 



$W(m) = 5^{2 m} \cdot 7^{3 m}$ represents an exponential function, because this formula can be rewritten in the form $A \cdot \, r^x$.   


\textbf{\textcolor{red!75!green}{explanation}} 


\[
W(m) = 5^{2 m} \cdot 7^{3 m}
\]


\[
W(m) = \left( 5^2 \right)^m \cdot \left( 7^3 \right)^m
\]

\[
W(m) = \left( 5^2 \cdot 7^3 \right)^m 
\]






\end{example}














The template for the formula of the basic shifted exponential function looks like



\[  A \cdot r^x + D   \,  | \,  A \ne 0, \, r > 0 \text{ and } r \ne 1  \]




However, this is not how we usually encounter exponential and shifted exponential functions.  


\begin{template}

We usually encounter exponential and shifted exponential functions in the more general forms. 




\[  A \cdot r^{B \, x + C}     \]


\[  A \cdot r^{B \, x + C} + D    \]

$r$ is called the \textbf{base}. \\
$A$ is called the \textbf{leading coefficient} of the function. \\
$B$ is the \textbf{leading coefficient} of the linear function in the exponent. \\
$D$ is called the \textbf{constant term}. 

\end{template}









We can also view exponential and shifted exponential functions as compositions, which allows us to use the Chain Rule.



\begin{idea}  \textbf{\textcolor{blue!55!black}{Exponential Compositions}} 




\begin{itemize}
\item Let $L_{out}(u) = A \, u $, a linear function
\item Let $E(t) = r^t$, a core exponential function
\item Let $L_{in}(v) = B \, v + C$, a linear function
\end{itemize}

With these component functions, we can view exponential functions as compositions.


\[  Exp(x) =    A \cdot r^{B \, x + C} = (L_{out} \circ E \circ L_{in})(x) \]


Now the Chain Rule can give us the behavior.

\end{idea}





\begin{idea}  \textbf{\textcolor{blue!55!black}{Shifted Exponential Compositions}} 




\begin{itemize}
\item Let $L_{out}(u) = A \, u + D$, a linear function
\item Let $E(t) = r^t$, a core exponential function
\item Let $L_{in}(v) = B \, v + C$, a linear function
\end{itemize}

With these component functions, we can view exponential functions as compositions.


\[  Exp(x) =    A \cdot r^{B \, x + C} + D= (L_{out} \circ E \circ L_{in})(x) \]


Now the Chain Rule can give us the behavior.

\end{idea}


















\begin{onlineOnly}
\begin{center}
\textbf{\textcolor{green!50!black}{ooooo-=-=-=-ooOoo-=-=-=-ooooo}} \\

more examples can be found by following this link\\ \link[More Examples of Exponential Functions]{https://ximera.osu.edu/csccmathematics/precalculus/precalculus/expFunctions/examples/exampleList}

\end{center}
\end{onlineOnly}









\end{document}
