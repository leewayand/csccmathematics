...
We have two types of core exponential functions, $r^x$. 
...
$\blacktriangleright$ If $r > 1$, then $r^x$ is an increasing exponential function.

$\blacktriangleright$ If $0 < r < 1$, then $r^x$ is a decreasing exponential function.
...
General exponential functions insert a leading coefficient.
...
We can think of exponential functions as shifts and stretches of a core exponential function, accomplished through composition. 



The \textbf{\textcolor{purple!85!blue}{leading coefficient}}, $A$, controls vertical stretching or compression. The sign of $A$ dictates the sign of our function values, which is illustrated graphically as a vertical reflection. 
...
The \textbf{\textcolor{purple!85!blue}{base}}, $r$, dictates a growing or decaying function - the constant prcentage change could be an increase or decrease.
...
Exponential functions possess a constant percentage growth rate.
...
Graphs of the basic exponential functions have the horizontal axis as a horizontal asymptote in one direction or the other, which illustrates that they have no zeros.
...
\begin{itemize}
\item Let $L_{out}(u) = \frac{1}{3} \, u$, an increasing linear function.
\item Let $E(t) = 2^t$, an increasing core exponential function.
\item Let $L_{in}(v) = v + 5$, an increasing linear function.
\end{itemize}
...
This gets shortened to leading coefficients and the base:\\
...
Exponential functions can swallow up horizontal shifting into the leading coefficient.
...
Exponential functions swallow up stretching and compressing into the leading coefficient and base.
...
A general \textbf{exponential function} is any function that \textbf{\textcolor{purple!85!blue}{CAN}} be represented with a formula of the form
...
\begin{itemize}
\item Let $L_{out}(u) = 4 \,$, an increasing linear function
\item Let $E(x) = 3^x$, an increasing core exponential function
\item Let $L_{in}(v) = 2 \, v - 1$, an increasing linear function
\end{itemize}

With these component functions, we can view $G$ as a composition.


\[  G(t) = 4 \cdot 3^{2t-1} = (L_{out} \circ E \circ L_{in})(x) \]
...
$f(x) = 3 \cdot e^{x}$ is an increasing function.
...
Or, we can compare back to a basic exponential function...
