...
Rigor is the proper and precise use of mathematical language to explain your reasoning and how you know you have accounted for everything.



\textbf{$\blacktriangleright$} \textbf{\textcolor{green!50!black}{Rigor}} lives inside your reasoning. 

\textbf{$\blacktriangleright$} \textbf{\textcolor{green!50!black}{Rigor}} lives inside your explanations. 
...
We have already commented that graphs are inherently inaccurate, so they might seem an odd place to introduce \textbf{\textcolor{purple!85!blue}{rigor}}.  On the other hand, we have noted that graphs are communication tools. This makes them an easy entrance to the ideas of \textbf{\textcolor{purple!85!blue}{rigor}}.  We can begin by using auxiliary graph symbols to clarify our graphical communication. Their presence and absence communicate information about the function.
...
Points are dimensionless objects.  They are too small to actually see.  When we draw a graph, the thickness of the pencil or pen is already an exaggeration of the underlying points and makes the endpoints a mystery.  How would you tell if the very last point of a curve was included or not? Therefore, we exaggerate even more for the endpoints, so that the reader immediately understands whether or not they are plotted and included with the graph or not.
...
Arrows at the ends of curves signal that the curve continues with its current pattern. However, this may not be enough.  Many functions have curves that continue moving down and to the right, however, their movement has a boundary. We use dashed lines to indicate that a curve is approaching this line pattern.
...
The values of the function, $f(x)$, get closer (or approach) $2$ as we move through the domain, heading to infinity (the right tail of the domain).  

And, of course, we have algebraic notation to communicate this thought. Limits will be our language to talk about expected values, which are described graphically with arrows, asymptotes, holes, and jumps.
...
\end{tikzpicture}
\end{image}
Just an arrow may be interpreted as the line continuing down.
...
A graph is a communication tool. We already know that the accuracy of graphs has limitations. There are also other limitations that we must deal with.  To that end, a nice graph displays other symbols besides just the points.  These other graphical features help the reader understand the thoughts of the drawer. 



\begin{summary} \textbf{\textcolor{red!70!yellow}{Nice Graphs}}

\textbf{Nice Graphs} include plotted points representing function pairs.  They also include

\begin{itemize}
\item \textbf{Solid Dots} at endpoints, indicating that the endpoint is included in the graph.
\item \textbf{Hollow Dots} at endpoints, indicating that the endpoint is not included in the graph.
\item \textbf{Arrows}, indicating that the graph continues its pattern.
\item \textbf{Asymptotes}, indicating a boundary the graph approaches.
\item \textbf{Ellipses}, indicating that the graph continues its pattern.
\item \textbf{Axis Labels}, indicating domain and function measurements.
\end{itemize}
...
\begin{warning} \textbf{\textcolor{red!70!black}{Rigor}}

These graphical enhancements are not optional.  They are a requirement of our desire to be \textbf{\textcolor{red!70!black}{rigorous}}.

We know the author of the graph knows that these symbols are not optional. So, when the symbols are missing, then we know that was intentional.  The missing symbol communicates information about the function.
...
All these graphical symbols help the reader understand the graph.  When the symbols are not drawn, then we know the author of the graph does not want us to interpret the graph in that way.
...
The auxiliary pieces drawn on a graph give us information and their absence gives us information as well.
...
We are humans communicating with other humans, which is why we take care to create readable and informative graphs. Technology does not have this desire.
...
When working with technology, we must accept that it is a machine giving us what it is programmed to give.  We must interpret correctly. 
