...
In computer programming, there is an ide called \textbf{scope}. Scope decides how symbols are to be interpreted at any point in the program. Mathematics has the same idea.

When writing mathematics, the same symbols will be used at different places and mean different things.  Probably, this is most often seen when numbering questions.  Once a question has concluded and the next question begins, all definitions from the previous question are lost. 
...
\begin{example} Scope
	...
	\begin{enumerate}
	\item  $f(x) = 7x - 5$ and $g = 4$
		\begin{enumerate}
			\item $f(1) = 2$
			\item $g(3) = 12$
			\item $h(6) = 6h = 6 \cdot h$
		\end{enumerate}
	\item $h(x) = 3x + 6$
		\begin{enumerate}
			\item $f(3) = 3f = 3 \cdot f$
			\item $h(g) = h(g) \ne h(4)$  
		\end{enumerate}
	\end{enumerate}
...
This tells us that we should not be using the same symbol in the same scope.

Defining $f(x) = \sqrt{7-x}$ and $g(x) = \ln(x)$ is not a good idea.
...
One of the main learnign objectives of htis course is \textbf{\textcolor{blue!55!black}{rigor}}.


We want to use proper and precise mathematics to communicate our reasoning.

Why? So, that we can communicate!  Learning is going to be difficult if people dont say things correctly.
