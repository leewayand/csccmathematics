...
Functions are special kinds of \textbf{\textcolor{blue!55!black}{relations}}.  Let's say a few words about relations to get our story off on the right footing.  Then we can focus our attention on functions. 
...
\textbf{Note:} This third set does not seem to have an official title. Sometimes it is just called the relation.
...
\pdfOnly{
\textbf{$\mathcal{VIDEO}$: Introduction to Relations}
}

\begin{onlineOnly}
\begin{explanation} \textbf{\textcolor{red!75!green}{Video: Introduction to Relations}}  
...
\begin{idea} \textbf{\textcolor{blue!55!black}{If-Then}}

If you are holding the ticket numbered 23675, then you win the stuffed teddy bear. 
...
Ticket number first. Teddy bear second.

\end{idea}



\begin{idea} \textbf{\textcolor{blue!55!black}{Cause and Effect}}

Due to the arctic vortex, tomorrow's high temperature will be $30^{\circ}$.  
...
First, arctic vortex. Second, $30^{\circ}$.
...
The idea is for mathematics to describe how we view the world, so we give relations a characteristic of order. Therefore, relations come prepackaged with a feeling that the information is connected \textbf{\textcolor{purple!85!blue}{from the domain}} \textbf{\textcolor{blue!55!black}{to the codomain}}.
...
Mathematics has notation to reflect both an unordered structure as well as an ordered structure. 
...
Since domain and codomain pairs come with a feeling of order, parentheses are useful for describing them.  
...
A relation is a package of three sets. The domain and codomain are sets of information.  The third set is a set of pairs.  Each pair connects a member of the domain with a member of the codomain. 
...
Each pair has a domain item written on the left and a codmain item written on the right.  They are separated with a comma.  All of that is wrapped in parentheses.
...
It is the written order within the pairs that is important. The order communicates the domain and codomain membership.
...
\begin{example} The $KindergartenIceCream$ Relation
...
\begin{idea} \textbf{\textcolor{blue!55!black}{Mathematical Model}} 

The overall idea is that while investigating our world, we discover some structure (connections) between collections of information.  The structure probably is a connection between two measurements. Relations are our way of mathematically \textbf{\textcolor{blue!55!black}{modeling}} this connection.

We then use the relation model to think about the connections, mathematically.  We draw conclusions about the relation model, inside mathematics. These conclusions describe structure about the relation model, inside mathematics.
...
\begin{itemize}
\item \textbf{\textcolor{blue!55!black}{[Type 1]}} You know the domain item(s) and want the corresponding codomain partner(s).
\item \textbf{\textcolor{blue!55!black}{[Type 2]}} You know the codomain item(s) and want the corresponding domain partner(s).
\end{itemize}
...
That is basically all we ever do, address these two types of questions. This is the foundation of function analysis.

The descriptions of our relations will become more and more detailed, which will require some critical thinking to understand and investigate. Inside the mathematical world we may need some sophisticated tools to investigate the relation.  But, in the end, all we are ever doing is hunting down domain information connected to recognized codomain information, or hunting down codomain information connected to meaningful domain information. 
...
\begin{example} The $ActorsInMovies$ Relation
...
Thinking ahead, most of our questions will identify a single hypothesis (antecedent) and then expect a single associated conclusion (consequent).

This would translate into each domain member always connected to a \textbf{\textcolor{purple!85!blue}{single}} codomain member.

Let's focus our investigation on these types of relations.  These types of relations are called \textbf{\textcolor{purple!85!blue}{functions}}.
