...
We have whittled our investigation down to a study of real-valued functions.  For us, these are functions whose domain and range are both subsets of the real numbers.  When it comes right down to it, \textbf{Precalculus} is a study of our numbers. We will see in Precalculus that our numbers include more than we might think.
...
Naturally, we have a need to communicate about sets of real numbers.  We have several ways of communicating our ideas of real numbers, both algebraically and graphically.
...
Our subsets might include one or more individual isolated numbers (singletons).  

	\begin{itemize}
	\item Our set might be just the numbers -3, 2, and 5.

	\item Curly braces around a comma-separated list denotes a set of individual numbers $\{ -3, 2, 5 \}$
...
Our subsets might include whole, unbroken, pieces or segments of the real number line of a finite length.  These are called \textbf{finite intervals} or \textbf{bounded intervals}.   Finite intervals include \textbf{\textcolor{red!70!darkgray}{ALL}} the real numbers between one specific number and another specific number.  Questions arise over the inclusion or exclusion of the two ends of the interval. Therefore, our communication needs to address them specifically.
	...
	\item All the real numbers between $4$ and $7$, including $4$ but not $7$.  We use parenthese to indicate exclusion:  $[4, 7)$
	\begin{image}
	\begin{tikzpicture}
	\begin{axis}[
            %xmin=-25,xmax=25,ymin=-25,ymax=25,
	...
	\item All the real numbers between $4$ and $7$, not including $4$, but including $7$:   $(4, 7]$
	\begin{image}
	\begin{tikzpicture}
	\begin{axis}[
            %xmin=-25,xmax=25,ymin=-25,ymax=25,
	...
	\item All the real numbers between $4$ and $7$, including neither $4$ nor $7$:    $(4, 7)$
	\begin{image}
	\begin{tikzpicture}
	\begin{axis}[
            %xmin=-25,xmax=25,ymin=-25,ymax=25,
...
\pdfOnly{
\textbf{$\mathcal{VIDEO}$: Interval Notation for Bounded Intervals}
}


\begin{onlineOnly}
\begin{explanation} \textbf{\textcolor{red!75!green}{Video: Interval Notation for Bounded Intervals}} 
...
Our subsets might include the right or left half of the real number line.  These are called \textbf{infinite intervals} or \textbf{unbounded intervals}.   Infinite intervals include \textbf{\textcolor{red!70!darkgray}{ALL}} the real numbers less than a specific real number, or \textbf{\textcolor{red!70!darkgray}{ALL}} the real numbers greater than a specific real number. 

	\begin{itemize}
	\item All the real numbers less than $4$ and including $4$.  When writing, we use negative infinity, -$\infty$, to indicate the interval extends without end, $(-\infty, 4]$.  When drawing, we use a left arrow.
	\begin{image}
	\begin{tikzpicture}
	\begin{axis}[
            %xmin=-25,xmax=25,ymin=-25,ymax=25,
...
\item All the real numbers less than $4$ but not including $4$, $(-\infty, 4)$
	\begin{image}
	\begin{tikzpicture}
	\begin{axis}[
            %xmin=-25,xmax=25,ymin=-25,ymax=25,
...
\item All the real numbers greater than $4$ and including $4$.  When writing, we use positive infinity, $\infty$, to indicate the interval extends without end, $[4, \infty)$. When drawing, we use a right arrow.
	\begin{image}
	\begin{tikzpicture}
	\begin{axis}[
            %xmin=-25,xmax=25,ymin=-25,ymax=25,
...
\item All the real numbers greater than $4$ but not including $4$, $(4, \infty)$.
	\begin{image}
	\begin{tikzpicture}
	\begin{axis}[
            %xmin=-25,xmax=25,ymin=-25,ymax=25,
...
\pdfOnly{
\textbf{$\mathcal{VIDEO}$: Interval Notation for Unbounded Intervals}
}


\begin{onlineOnly}
\begin{explanation} \textbf{\textcolor{red!75!green}{Video: Interval Notation for Unbounded Intervals}} 
...
\begin{itemize}
\item The set $S$ is \textbf{bounded from above}, if there exists a real number, $B$, such that for all $s \in S$, we have $s \leq B$.  There is a real number greater than or equal to all the numbers in $S$.  

\item The set $S$ is \textbf{unbounded from above}, if for any selected real number, $B$, there exists $s \in S$, such that $B < s$. No matter how high the bar is set, $S$ contains a number greater than the bar.  There is no real number that is greater than or equal to all the numbers in $S$. 

\item The set $S$ is \textbf{bounded from below}, if there exists a real number, $B$, such that for all $s \in S$, we have $B \leq s$.  There is a real number less than or equal to all the numbers in $S$.  

\item The set $S$ is \textbf{unbounded from below}, if for any selected real number, $B$, there exists $s \in S$, such that $s < B$. No matter how low the bar is set, $S$ contains a number less than the bar.  There is no real number that is less than or equal to all the numbers in $S$.  
...
\begin{warning}  \textbf{\textcolor{red!70!black}{Numbers vs. Points}} 
...
We will attempt to keep the blur to a minimum in this course by our judicial use of the words \textit{number} and \textit{point}.
...
\begin{notation}  \textbf{\textcolor{red!50!green}{Membership}} 
...
\begin{warning}  \textbf{\textcolor{red!70!black}{Special Sets}}

$(-\infty, \infty)$ is both an open interval and a closed interval.

The empty set is also both open and closed.
...
\pdfOnly{
\textbf{$\mathcal{VIDEO}$: Interval Notation, Inequalities, and Graphs}
}


\begin{onlineOnly}
\begin{explanation} \textbf{\textcolor{red!75!green}{Video: Interval Notation, Inequalities, and Graphs}} 
...
\begin{warning}
\textbf{\textcolor{red!70!black}{ALL}} real numbers would include numbers such as $5$, $\frac{17}{4}$, $\sqrt{19}$, $6.354987263$, and $211^{\tfrac{1}{\pi}}$, not just whole numbers.
\end{warning}
...
This would be the set $\{ 4, 5, 6 \}$, since $n \in \mathbb{Z}$.
...
The \textbf{intersection} of two sets is a set containing all elements common in both sets.
...
\pdfOnly{
\textbf{$\mathcal{VIDEO}$: Sets}
}


\begin{onlineOnly}
\begin{explanation} \textbf{\textcolor{red!75!green}{Video: Sets}}
...
\pdfOnly{
\textbf{$\mathcal{VIDEO}$: Singletons}
}


\begin{onlineOnly}
\begin{explanation} \textbf{\textcolor{red!75!green}{Video: Singletons}} 
...
Having such a wide variety of descriptions is very valuable as we move through mathematics.  However, for communication purposes, it causes confusion. Therefore, we are establishing an \textbf{official reduced form} for interval notation.  We will always use the reduced form, unless there is a good reason not to.  (There are often good reasons.)

Our \textbf{reduced form} will adhere to some rules:





\begin{notation} \textbf{\textcolor{red!50!green}{Reduced Interval Notation}} 
...
\pdfOnly{
\textbf{$\mathcal{VIDEO}$: Reduced Interval Notation}
}


\begin{onlineOnly}
\begin{explanation} \textbf{\textcolor{red!75!green}{Video: Reduced Interval Notation}}
...
Our official reduced interval notation uses maximal intervals.
...
We use square brackets and parentheses for both written algebraic notation and graphical notation.  
...
\begin{example}  Half Open Half Close Interval
...
Select all valid statements.
    \begin{selectAll}
    \choice [correct] {$A$ is bounded from below}
    \choice [correct] {$A$ is bounded from above}
    \choice [correct] {$A$ is bounded}
    \choice {$A$ is unbounded from below}
    \choice {$A$ is unbounded from above}
    \choice {$A$ is unbounded}
    \end{selectAll}
\end{question}
...
Select all valid statements.
    \begin{selectAll}
    \choice {$B$ is bounded from below}
    \choice [correct] {$B$ is bounded from above}
    \choice {$B$ is bounded}
    \choice [correct] {$B$ is unbounded from below}
    \choice {$B$ is unbounded from above}
    \choice {$B$ is unbounded}
    \end{selectAll}
\end{question}
...
Select all valid statements.
    \begin{selectAll}
    \choice [correct] {$C$ is bounded from below}
    \choice {$C$ is bounded from above}
    \choice {$C$ is bounded}
    \choice {$C$ is unbounded from below}
    \choice [correct] {$C$ is unbounded from above}
    \choice {$C$ is unbounded}
    \end{selectAll}
\end{question}
...
\item Mathematics is a language we use to describe and explain our scientific discoveries.
\end{itemize}
...
As we accumulate descriptions of structure, we will notice commonalities. We will describe these commonalities and we will organizes our thoughts about this common structure. 

We will use mathematics to describe this common structure. We call this \textbf{\textcolor{blue!55!black}{abstraction}}.

This process of description and organization naturally makes our brains think of these descriptions as entities themselves. The language helps us think about structure and becomes structure itself.

Algebra is our first experience with this idea of abstraction.

Mathematicians are looking for structure inside the language we have invented to describe structure.

Mathematics is a language that lends itself to abstract thoughts, which leads to more structure, which leads to more mathematics.

Mathematicians are people who explore this structure.
