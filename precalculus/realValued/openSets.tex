...
\subsection*{Open Sets in $\mathbb{R}$}
...
\begin{idea}  \textbf{\textcolor{blue!55!black}{Space}}

The idea is that inside an open set, every number sits inside an open interval inside the set.  There is space between each number and the endpoint. There is a positive distance between each number and each endpoint. There is a positive distance between any two numbers inside the interval.
\end{idea}
...
Therefore, no matter which open interval you choose around $7$, it will always contain a number outside $(4, 7]$.
...
The reasoning is sort of reverse thinking.  

If $\emptyset$ is not open, then it MUST contain a number, such that there is no tiny open interval around it.  There is no such number.  Therefore, $\emptyset$ is an open set.
