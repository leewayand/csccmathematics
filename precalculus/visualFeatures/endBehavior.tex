...
There is the \textbf{\textcolor{red!70!black}{left tail}}, which is the part with very very very very large negative numbers. There is the \textbf{\textcolor{red!70!black}{right tail}}, which is the part with very very very very large positive numbers. Then, there is the \textbf{\textcolor{purple!85!blue}{middle}} part. 

All the individual characteristics of a function happen in the middle. 

That is, each function has its own middle bubble (subset of the domain) where all the important locations live.  The middle bubble contains the zeros, the discontinuities, the singularities, the critical numbers, the maximums, the minimums, the everything.  Once we are outside the middle part of the domain, the function settles down into a consistent pattern of behavior and maintains that predictable behavior all the way to $-\infty$ or $\infty$ (in the domain). 

This settled down pattern in the \textbf{\textcolor{red!70!black}{tails}} is called the \textbf{\textcolor{red!70!black}{end-behavior}} of the function. 
...
In the graph above, we see a vertical asymptote (function singularity).  This is inside the middle bubble.  It is not a part of end-behavior. 

In the graph above, we see an intercept (function zero).  This is inside the middle bubble.  It is not a part of end-behavior. 
...
\begin{notation} \textbf{\textcolor{red!50!green}{Building End-Behavior Limits}}  




\[
\lim\limits_{tail} Function = Expected Value
\]
...
\begin{itemize}
\item a number
\item $\infty$, indicating that the function is unbounded positively
\item $-\infty$, indicating that the function is unbounded negatively
...
From this, we deduce that as we move out in the domain towards $\infty$ (the right tail), the value of $H$ grows greater and greater - unbounded negatively. 

We also deduce that as we move out in the domain towards $-\infty$ (the left tail), the value of $H$ grows greater and greater - unbounded positively. 
...
\begin{warning} \textbf{\textcolor{red!70!black}{Bigger and Bigger}} 
...
We reserve $-\infty$ and $\infty$ to describe unbounded situations.
...
Increasing functions do not all become unbounded.  To conclude that a function is unbounded and the limit is $\infty$, you need a reason beyond ``it is getting bigger and bigger''.

A reason could be the type of function.  By recognizing the category of the function, you might automatically know that the funciton becomes unbounded.
...
It doesn't even make sense to write the symbol.



\[
\lim\limits_{v \to -\infty} K(v) 
\]
...
If there is no horizontal dashed asymptote, then the function's values \textbf{do not} eventually get close to a constant value and stay there. It doesn't matter what it ``looks like''.

The horizontal asymptotes tell the reader something and its absence also tells the reader something. 
...
\begin{tikzpicture}
     \begin{axis}[
        ...
        %\addplot [line width=0.5, gray, dashed,samples=100,domain=(-9:9)] ({3},{x});
        \addplot [line width=0.5, gray, dashed,samples=100,domain=(-9:9)] ({x},{3});
        \addplot [line width=0.5, gray, dashed,samples=100,domain=(-9:9)] ({x},{-4});
...
\textbf{Discontinuities and Singularities}
...
All of these involve expected values, which may or may not agree with actual values.


We would like to describe these expectations and discrepancies.
