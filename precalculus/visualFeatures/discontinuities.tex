...
Continuity is one of the characteristics, features, or properties of functions listed in our official analysis list.

The definition of continuity requires more experience with limits than we have so far. However, we can begin to get a feel for continuity by first examining functions that are not continuous.
...
\begin{itemize}
\item \textbf{\textcolor{blue!75!black}{Removeable:}}  The graph above has a very subtle break at $t=-6$. The graph is almost one piece near $-6$, except one point has been shifted vertically. This type of break gets the name \textit{removeable}, because you could remove the break by just sliding that one point back in place. 

\item \textbf{\textcolor{blue!75!black}{Asymptotic:}} The break at $t=3$ is not subtle at all.  One side is heading up to infinity, and the other side is heading down to negative infinity.  You can't get a bigger break than that. Graphically, the asymptote is communicating this behavior, hence, the break is called an \textit{asymptotic} break. 
...
In all three cases, the graph experiences a sudden vertical movement.  This sudden vertical movement may or may not be predictable from the points leading up to the sudden movement.
...
Our language for expected values is called \textbf{limits}, which is a main interest of Calculus.


\begin{example} Limits

We want algebraic descriptions of discontinuities.

Our algebraic descriptions are called limits. Limits communicate our expectations based on surrounding values.

For the function $g$ above, the surrounding points give us the expectation that $g(-6)=1.9$.  However, $g(-6)=5$.  Our expectation did not match the actual value at $-6$.

We would communicate our expectations with limit notation,


\[ \lim\limits_{t \to -6} g(t) = 1.9 \]
...
It is not enough to form a basic idea. We want to be exact, and we want to express our thoughts on exactness so that other people can understand what we see.
...
Therefore, we are going to adopt some language to separate these ideas dealing with breaks.


\begin{idea} \textbf{\textcolor{blue!55!black}{Language}}  


While graphs have breaks, functions will have \textbf{\textcolor{blue!55!black}{discontinuities}} and \textbf{\textcolor{blue!55!black}{singularities}}.

\begin{itemize}
\item If the domain of a function includes the number where the graph is experiencing a break, then we will say the function has a \textbf{\textcolor{blue!55!black}{discontinuity}} at this domain number.  

\item If the domain of a function does not include the number where the graph is experiencing a break, then we will say the function has a \textbf{\textcolor{blue!55!black}{singularity}} at this non-domain number.
\end{itemize}
...
We will soon algebraically define discontinuities to match these ideas.
...
Select all of the discontinuities of $K$.
\begin{selectAll}
\choice {$-8$}
\choice [correct]{$-3$}
\choice {$-1$}
...
There is a vertical jump at $-3$. And $4$ is  in the domain of $K$.  So, $4$ is a discontinuity.  
...
Select all of the discontinuities of $h$.
\begin{selectAll}
\choice {$-8$}
\choice {$-3$}
\choice {$-1$}
...
\textbf{Note:}  $ |f(c) - f(e)| > d $ says the distance between $f(c)$ and $f(e)$ is greater than $d$.
...
We need algebraic language for ``\textit{no matter how close you get in the domain}'' and `\textit{no matter how close you get in the codomain}''.
...
Select all of the discontinuities of $P$.
\begin{selectAll}
\choice [correct]{$-8$}
\choice [correct]{$-4$}
\choice {$-2$}
...
For the second condition, let $d = 1$. Any small open interval of real numbers containing $c = 4$ must also contain an interval of the form $(4-\epsilon, 4+\epsilon)$, where $0 < \epsilon < 0.1$.  Such an interval would \textbf{ALWAYS} contain the domain number $e = 4-\frac{\epsilon}{2}$.
...
\item $-6$ is a discontinuity of $T(y)$.  $-6$ is in the domain of $T$.  For the second condition, let $d = 0.5$. Any small open interval of real numbers containing $c = -6$ must also contain an interval of the form $(-6-\epsilon, -6+\epsilon)$, where $0 < \epsilon < 0.1$.   Such an interval would \textbf{ALWAYS} contain the domain number $e = -6+\frac{\epsilon}{2}$.
...
\item $4$ is a discontiuity of $T(y)$.   $4$ is in the domain of $T$.  For the second condition, let $d = 0.5$. Any open interval of real numbers containing $c = 4$ must also contain an interval of the form $(4-\epsilon, 4+\epsilon)$, where $0 < \epsilon < 0.1$.   Such an interval would \textbf{ALWAYS} contain the domain number $e = 4-\frac{\epsilon}{2}$.
...
No matter how close you get to $-6$ in the domain, there is \textbf{ALWAYS} a jump of more than $1$ in the value of the function.  
...
\begin{observation} \textbf{\textcolor{-red!75}{But, But, But, ...}}  
...
\begin{idea} \textbf{\textcolor{blue!55!black}{A Singularity}}  
...
\begin{itemize}
\item EVERY open interval containing $c$ contains a domain number and 
\item the function is behaving extreme/weird/unexpected near $c$. 
\end{itemize}
It turns out ``weird'' includes a lot of behavior.  It will take a while to state a \textbf{\textcolor{purple!85!blue}{rigorous}} definition of singularity, because it will involve the derivative.  In the meantime, we will keep noting weird behavior and building our feeling of weird.
...
\begin{example} Discontinuity and Singularity
...
If $c$ is in the domain of $f$ and $c$ is not a discontinuity, then $f$ is said to be \textbf{continuous} at $c$. 


If $S$ is a subset of the domain and $f$ is continuous at every number in $S$, then $f$ is said to be continuous on the subset. 

A function that is continuous on its whole domain is just said to be a continuous function. 
...
\begin{warning} \textbf{\textcolor{red!70!black}{Not Continuous}}
...
Continuity only refers to domain numbers.

A function can be continuous if it has singularities, because a singularity is not in the domain.

A function is certainly not continuous at a singularity, because the number is not in the domain. But it is not a discontinuity, which are the only places where a function could fail to be continuous.
...
Close in the domain does not result in close in the range.
...
You can get close enough to $c$ in the domain so that all the function values are close to $f(c)$...as close as you want...any closeness. 
...
\textbf{Note:} $| f(x) - f(c) | < \epsilon$ is an algebraic way of saying  

\[  f(x) \in (f(c) - \epsilon, f(c) + \epsilon)  \]

a small open interval around $f(c)$.
...
\subsection*{Core and Elementary Functions}
...
This makes studying discontinuities difficult using core and elementary functions.  The only tool we have to help us study discontinuities is piecewise defined functions.
