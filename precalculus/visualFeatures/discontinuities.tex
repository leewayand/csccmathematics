...
The graphs we have seen so far clearly show three kinds of breaks: holes, jumps, and asymptotes.  These breaks illustrate a drastic change in behavior for the function.
...
$\blacktriangleright$ Discontinuities are numbers in the domain of the function and singularities are not in the domain. Of course, all of this gets more complicated as we encounter more functions.  But this is a good start. 
...
$\blacktriangleright$ Endpoints make an exact definition of discontinuity messy, because we want to say there are domain numbers on one or both sides of the discontinuity and these domain numbers have no space between them.  There is no \textit{open interval} between domain numbers.  However, there is a gap in their function values - suddenly - right at the discontinuity.
...
Close in the domain does not mean close in the range.

Close in the domain does not mean close function values.
...
A function can be continuous if it has singularities, because the singularitiy is not in the domain.

The function is certainly not continuous at a singularity, because the number is not in the domain. But, it is not a discontinuity, which are the only places where a function could fail to be continuous.
