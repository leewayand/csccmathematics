...
\textbf{Best job} means the line shares the tangent point with the curve and the slope of the line is the same as the ``slope'' of the curve at the tangent point. If you zoomed in on the graph at the tangent point, then the curve would slowly begin to look just like the line. There can only be one tangent line.
...
\pdfOnly{\textbf{$\mathcal{Desmos}$: Secant Line}}
\begin{onlineOnly}
\textbf{\textcolor{teal}{$\blacktriangleright \mathcal{Desmos}$: Secant Line}} 
\begin{center}
...
The idea is that we can select two points on the graph and a secant line will run through them. Since we have two points, we can calculate the slope of the secant line. We can keep selecting two points that get closer to the tangent point. Each pair of new points on the graph defines a secant line. The slopes of these secant lines should be getting closer to the slope of the tangent line. 




\textbf{\textcolor{red!70!darkgray}{$\blacktriangleright$}} There are two changes happening as we shift a secant line over to the tangent point. First, the graph has its own twists and turns as the points on the graph approach the tangent point. Second, different secant lines are changing position, changing slopes, as they come closer to the tangent point. 



The graph is behaving (there is a pattern) and the secant lines are changing (there is a pattern). Hopefully, these separate behaviors will converge to the same behavior at the tangent point.
...
\begin{warning} \textbf{\textcolor{red!70!black}{Both Sides}}

Both sides means both sides of the tangent point. 

The points on the graph on both sides are approaching the tangent point. 

The secant lines connecting the points on both sides are approaching the tangent line
...
Instead, we are drawing two secant lines.

We are using the tangent point itself as a secant partner for each of the two selected points.


\begin{itemize}
\item We'll have one secant line through the selected left point and the tangent point.
\item We'll have another secant line through the selected right point and the tangent point.
\end{itemize}
...
Here is a secant line on the left side of the tangent line.




\pdfOnly{\textbf{$\mathcal{Desmos}$: Left Tangent Line}}
\begin{onlineOnly}
\textbf{\textcolor{teal}{$\blacktriangleright \mathcal{Desmos}$: Left Tangent Line}} 
\begin{center}
...
\pdfOnly{\textbf{$\mathcal{Desmos}$: Right Tangent Line}}
\begin{onlineOnly}
\textbf{\textcolor{teal}{$\blacktriangleright \mathcal{Desmos}$: Right Tangent Line}} 
\begin{center}
...
\textbf{Remember:} We are considering the situation where we have both sides, which means the domain number corresponding to the tangent line must be inside an open interval in the domain. 
...
Our secant lines are special secant lines. Rather than a secant line running through two points on either side of the tangent point, our secant lines will each use the tangent point as one of their points. 
...
We have a parabola and a point on the parabola. 

\textbf{First}, the graph has no breaks. The points on the graph are approaching the point $(4, -3)$ smoothly on both sides.  There is no break in the curve. $F$ is continuous at $4$.
...
The tangent line does the best job of modeling the curve right at the tangent point. If you were to zoom in close to the point $(4, -3)$, the line and the curve would look the same.


As it turns out, we know how to get the slope of lines tangent to a parabola.  Use $F'$ (the derivative of $F$) or $iRoC_F$.   
...
\textbf{Second}, for every other point on the parabola, there is a line through that point and the tangent point.  These are called \textit{secant lines}. These secant lines run through a point on the graph and the tangent point. The sequence of secant lines smoothly turns into the tangent line at the tangent point...on both sides.
...
\pdfOnly{\textbf{$\mathcal{Desmos}$: Both Sides Agree}}
\begin{onlineOnly}
\textbf{\textcolor{teal}{$\blacktriangleright \mathcal{Desmos}$: Both Sides Agree}} 
\begin{center}
...
The secant lines on the left smoothly turn into the tangent line as you move the left secant point to the right. The secant lines on the right smoothly turn into the tangent line as you move the secant point to the left. 



This is the best situation. 

The graph is nice at the tangent line and the secants lines on both sides smoothly turn into the tangent line at the tangent point. 



Now, let's look at kinks in the story. 
...
\textcolor{gray!90}{\textbf{Graph Description:} Cartesian plane. Horizontal axis labeled $x$. Vertical axis labeled $y$. The point $(4,-3)$ is plotted and is the intersection of two rays.  Both rays originate at this point forming a corner.  One ray moves from this point through the point $(-1,0)$.  The other ray moves from the intersection point through the point $(7,0)$.}
...
If there is a tangent line, then the secant lines should smoothly turn into the tangent line, \textbf{on both sides}.  
...
\textbf{Second}, the secant lines drawn through approaching points do not smoothly turn into a common line at the tangent point...\textbf{on both sides}.
...
\pdfOnly{\textbf{$\mathcal{Desmos}$: Both Sides Do Not Agree}}
\begin{onlineOnly}
\textbf{\textcolor{teal}{$\blacktriangleright \mathcal{Desmos}$: Both Sides Do Not Agree}} 
\begin{center}
...
The secant lines on the left smoothly turn into a line as you move to the right. The secant lines on the right smoothly turn into a line as you move to the left. 
...
So, there is no tangent line at the point $(3,-4)$. 
...
\textcolor{gray!90}{\textbf{Graph Description:} Cartesian plane. Horizontal axis labeled $x$. Vertical axis labeled $y$. The point $(2,0)$ is plotted on the curve.  The curve begins with an arrow pointing down and to the left at $(-5,-8)$. From there, the curve moves up and to the right slowly becoming vertical. The curve goes through the point $(2,0)$ vertically. The curve moves up and to the right slowly flattening out until it arrives at an arrow point up and to the right at $(9,8)$.}
...
\textcolor{gray!90}{\textbf{Graph Description:} Cartesian plane. Horizontal axis labeled $x$. Vertical axis labeled $y$. The point $(2,0)$ is plotted on the curve.  The curve begins with an arrow pointing down and to the left at $(-5,-8)$. From there, the curve moves up and to the right slowly becoming vertical. The curve goes through the point $(2,0)$ vertically. The curve moves up and to the right slowly flattening out until it arrives at an arrow point up and to the right at $(9,8)$. There is a vertical line running through $(2,0)$.}  
...
\pdfOnly{\textbf{$\mathcal{Desmos}$: Vertical Tangent}}
\begin{onlineOnly}
\textbf{\textcolor{teal}{$\blacktriangleright \mathcal{Desmos}$: Vertical Tangent}} 
\begin{center}
...
Can a graph have a tangent line at a discontinuity?

A function with a discontinuity has a break in the graph, which means the points on the graph on both sides of the point do not approach the same point.  This automatically results in the derivative not existing, because the derivative requires both sides to agree.
...
\textcolor{gray!90}{\textbf{Graph Description:} Cartesian plane. Horizontal axis labeled $x$. Vertical axis labeled $y$. The graph has two pieces.  Piece #1 begins with an arrow pointing up and to the left at $(0,5)$. The graph continues down to the point $(3,-4)$ and then turns upward to a solid dot at $(4,3)$. Piece #2 begins with a hollow dot at $(4,2)$ and moves up to an arrow pointing up and to the right at $(6,10)$.}
...
\pdfOnly{\textbf{$\mathcal{Desmos}$: Discontinuity}}
\begin{onlineOnly}
\textbf{\textcolor{teal}{$\blacktriangleright \mathcal{Desmos}$: Discontinuity}} 
\begin{center}
...
As we have seen in our graphs, functions change behavior. If a function changes behavior at a domain number, then this domain number is called a \textbf{critical number}.
...
A real number, $A$, is called a critical number of $F$ if

\begin{itemize}
\item $A$ is a domain number of $F$.
\item $F'(A) = 0$ or $F'(A)$ Does Not Exist
\end{itemize}
...
\textbf{\textcolor{blue!55!black}{$\blacktriangleright$}} A derivative implies that there are two sides and the two sides are agreeing. 

We need two sides of a tangent point to talk about a derivative.

But there is no need to just throw everything else away.  We can extend our idea of derivative to include just one side, which we will do. 
...
Can we extend our idea of derivative to include just one side? Yes!
...
\textbf{Graph Description:} Cartesian plane. Horizontal axis labeled $x$. Vertical axis labeled $y$. The graph begins with an arrow pointing up and to the left at $(0,5)$. The graph continues down to the point $(3,-4)$ and then turns upward to a solid dot at $(4,3)$.  There is a line running through the endpoint $(4,3)$. This line is tangent to the curve at $(4,3)$.  
...
\pdfOnly{\textbf{$\mathcal{Desmos}$: Endpoint}}
\begin{onlineOnly}
\textbf{\textcolor{teal}{$\blacktriangleright \mathcal{Desmos}$: Endpoint}} 
\begin{expandable} 
\begin{center}
...
\end{expandable}
\end{onlineOnly}
[ Click on the arrow to the right to expand for a Desmos version of the graph. ]
...
For our curve above, we have a \textbf{left tangent line} or \textbf{left tangent ray}.

For the function, we have a \textbf{left derivative}.
...
\[
iRoC_F(t_0) =F'(t_0) = \text{slope of tangent line}
\]
...
\begin{warning} \textbf{\textcolor{red!70!black}{Derivative Implies Two Sides}}
...
To describe such situations, we have a one-sided derivative, which is like half of a derivative. 
...
Isolated domain numbers are automatically critical numbers.

The term derivative implies that there is some sort of approaching.


We are just beginning to think about these types of mental exercises. We want to think about how to think about them.  Calculus will officially take over this story and give all the details.
