...
Since ``slope'' is a characteristic of a line and a curve be more than a line, curves do not have a slope as we know that word. We are inventing a meaning.

It is easy to see that curves have something that seems a lot like a slope at points. We will adopt the slope of the tangent as the slope of the curve. 





\begin{definition} \textbf{\textcolor{green!50!black}{Slope of a Curve at a Point}}


The \textbf{slope of a curve} at the point $(a, b)$ is the slope of the tangent line to the curve at $(a,b)$.
...
If we can quantify it, then we can build a function for it and use that to measure a rate of change for our curve. This is the first step towards Calculus. It will take us a while to quantify this idea of slope for a curve into a logical tool. A tool we will call the \textbf{derivative}.

We will first investigate this idea with quadratic functions and parabolas. 



Our plan is illustrated in the graph below.

We have a quadratic function $f(x) = \frac{1}{2} (x - 2)^2 + 1$.

We have a point on the graph and a line tangent to the curve at this point.

We have two other points on the graph. One on either side of the tangent point.

We have a line through these two other points.  This is called a \textbf{secant line}.


As you move the outer two points close to the tangent point, the secant line becomes closer to the tangent line.
...
We want an algebraic description of that story.
...
\end{tikzpicture}
\end{image}
\textcolor{gray!90}\textbf{Graph Description:} Parabola opening up, vertex labeled ``(h, k)'' at $(3,2)$.} 
...
Let's examine the interval $[2, 5]$, which includes $3$.
...
We are calling our invention the \textbf{instantaneous rate of change}.
...
\textbf{\textcolor{red!70!darkgray}{$\blacktriangleright$}} If the slope of a secant line corresponds to the function's rate of change over an interval, then the slope of a tangent line corresponds to the function's rate of change at a single number.
...
\textbf{Note:} We are beginning with a graphical definition, because that is a good introduction to the idea.  But, we really want an algebraic definition.  We want to be exact.  Getting to an exact definition of the derivative requires limits.  Therefore, we will take our time and slowly gain enough experience, so that Calculus can present a detailed story.

\textbf{Note:} We are leaving out vertical lines, because they do not have a slope.  But, we will eventually include them to complete the story of derivaties.
...
\end{tikzpicture}
\end{image}
\textcolor{gray!90}{\textbf{Graph Description:} Parabola opening up, vertex at the origin, and a point labeled ``$(x_0, y_0)$'' at $(2,4)$. There is a line drawn through this point, which is tangent to the parabola.}
