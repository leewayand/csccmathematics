...
Since ``slope'' is a characteristic of a line and a curve is not a line, curves do not have a slope as we know that word. We are inventing a meaning.
...
If we can quantify it, then we can build a function for it and use that to measure a rate of change for our curve. This is the first step towards Calculus. It will take us a while to quantify this idea of slope for a curve in a logical tool. A tool we will call the \textbf{derivative}.
...
\end{tikzpicture}
\end{image}
\textbf{Graph Description:} Parabola opening up, vertex labeled ``(h, k)'' at $(3,2)$. \\
...
\end{tikzpicture}
\end{image}
\textbf{Graph Description:} Parabola opening up, vertex at the origin, and a point labeled ``$(x_0, y_0)$'' at $(2,4)$. There is a line drawn through this point, which is tangent to the parabola.
