...
Then $\frac{1}{r} > 1$.  We would like to use $\frac{1}{r}$ as the new base, which we can accomplish through the change of base formula.
...
\textbf{\textcolor{red!70!darkgray}{$\blacktriangleright$}} We can switch any logarithm from a base less than $1$, to a base greater than $1$ just by negating the logarithm.
...
In this model, our core forms to memorize would be $e^x$ and there would be several alternative choices for you.
...
\begin{tikzpicture}
   \begin{axis}[name = leftgraph, 
          ...
          \addplot [color=penColor,only marks,mark=*] coordinates{(1,0)};
           ...
           \addplot [line width=1, magenta, dashed, domain=(-10:10),<->] ({10},{x});
           \addplot [line width=1, magenta, dashed, domain=(-10:10),<->] ({-10},{x});
           \addplot [line width=1, magenta, dashed, domain=(-10:10),<->] ({x},{10});
           \addplot [line width=1, magenta, dashed, domain=(-10:10),<->] ({x},{-10});
