...
Since our analysis rests on zeros of funcitons anc zeros of their derivatives, the standard form for a polynomial is just not that helpful.  Our analysis prefers a factored form. 
...
\textbf{\textcolor{blue!55!black}{$\blacktriangleright$}} If our polynomial is a constant function, then it always has a $0$ derivative and $0$ rate of change.


\textbf{\textcolor{blue!55!black}{$\blacktriangleright$}} If our polynomial is a linear function, then it has a constant rate of change. The leading coefficent tells us if the linear function is always increasing or always decreasing.


\textbf{\textcolor{blue!55!black}{$\blacktriangleright$}} If our polynomial is a quadratic function, then it has a linear derivative. The leading coefficent tells us if the quadratic function increases and then decreases or the other way around.  This coincides with the sign of the derivative.


This pattern follows for polynomials of higher degree. The critical numbers partition the real line into intervals where the polynomial function increases or decreases.  All of this depends on getting the critical numbers, which will require a derivative.  Calculus will provide algebraic tools for obtaining the derivative. In this course, the derivative of a higher order polynomial would have to be given.
...
Unless the polynomial is a constant function, polynomials are unbounded.  The range will be unbounded positively or negatively or both.  We would communicate this with $-\infty$ or $\infty$ or both.  

The range of a polynomial with an odd degree is always $(-\infty, \infty)$. 
...
We can't get the behavior exactly.  We need the derivative for that. However, we can get a working idea.
...
Graphical approximation is as close as we are going to get. 
...
We can get some information exactly and must approximate other information.
...
Once we have a function's derivative, we might be able to get the exact values of the critical numbers. 


Calculus will tell us how to get the derivative of any function.  In Precalculus, we can get the derivative of a linear function or a quadratic function.  We cannot get the derivative of any other polynomial.

But, you could be given the derivative and use it to find exact values of critical numbers. 




In the example above, Calculus would give us the derivative, $p'(w) = -\frac{1}{5}(3w^2 - 4w - 15)$. 

$p'(x)$ is another polynomial a quadratic. The zeros of this derivative would be critical numbers of $p(x)$.  $p'(w)$ is a quadratic.  Therefore, we can obtain its zeros, the critical numbers of $p(x)$, via the quadratic formula.
...
$p'(w)$ is a polynomial, which means it is continuous. It is certainly negative for very large negative values of $w$, because the leading coefficient is negative.  Then it changes sign over $-\frac{5}{3}$.  So, $p'(w)$ is positive between $-\frac{5}{3}$ and $3$.  Then, it changes sign to negative at $3$.


Another way to see this is by noting that $p'(w)$ is a quadratic with a negative leading coefficent, which means its values follow $pos - neg - pos$ pattern.
