...
In the world of functions, polynomials play a similar role as the rational numbers - you can find a polynomial as close as you want to any continuous function. 


In other words, no matter what small distance you think up...

\begin{itemize}
\item \textbf{Numbers:} Given any real number, there is a rational number within that small distance of the real number.

\item \textbf{Functions:} Given any continuous function on a closed interval, there is a polynomial within that ``small distance'' of the continuous function.
\end{itemize}

This makes polynomials extremely useful and thus very interesting. 

Polynomials are linear combinations of core power functions.
...
\begin{itemize}
\item $n$, a whole number, is called the \textbf{degree} of the polynomial.
...
When analyzing the behavior of polynomials, the characteristics depend on the zeros of the derivative, which Calculus will show us is another polynomial.  Analyzing polynomials rests on their zeros.


For this reason, we would prefer a product
...
All the characteristics and features of polynomials are nice.
...
Polynomials smoothly alternate between increasing and decreasing, which switch at locations of global and local maximums and minimums.
...
\textbf{Note:} Constant functions are technically polynomials. They have a completely different behavior than what we think of with polynomials.
...
$p(x) = a (x-r_n)(x-r_{n-1})  \cdots (x-r_2)(x-r_1)$ is the \textbf{factored form} of a polynomial (a product).  
...
In this version, the $r_k$ are distinct roots. i.e. they are all different. \\
...
If the factors are collected, then the multiplicity of $r$ is the exponent of $(x-r)$.

\end{definition}

As we saw with quadratics, this factorization into linear factors often requires complex numbers, which we will encounter later in this course. We are only using real numbers now.  Therefore, our factorizations will include linear factors as well as irreducible quadratic factors (quadratics with nonreal/complex roots).


\textbf{\textcolor{red!70!darkgray}{$\blacktriangleright$}}  Therefore, our discussion on polynomial roots will center around the linear factors.  Irreducible quadratics will have to stay that way until later in the course.
...
For a polynomial, we would run through each distinct root once.  As we ran from the left side to the right side of each root, the sign of the value of the polynomial would either change or stay the same.
...
The multiplicity or exponent tells us if the sign changes across the zero or singularity.
...
In between these zeros, the value of the polynomial function has a positive or negative sign. It can't change, since polynomials are continuous. 
...
\begin{itemize}
  ...
  \item $x-3 < 0$ 
\end{itemize}
In the interval; $(-\infty, -4)$, the whole polynomial is $pos \cdot neg \cdot pos \cdot neg = positive$.  We also know this because $p$ is a fourth-degree polynomial and the leading coefficient is positive. That tell us that the end-behavior is unbounded positively in both directions.
...
All the information about the sign of the polynomial is encoded in the zeros and their exponents. 
...
For the graph below, alternate the exponent for $(x+2)$ between $2$ to $3$.  

The sign of $(x+2)^3$ switches across $-2$ and the graph crosses over the axis.


The sign of $(x+2)^2$ stays the same across $-2$ and the graph does not cross over the axis.


If you change the sign of the leading coefficient, then the crossing and bouncing will jump to the other side of the $x$-axis.



\pdfOnly{\textbf{$\mathcal{Desmos}$: Multiplicities}}
\begin{onlineOnly}
\textbf{\textcolor{teal}{$\blacktriangleright \mathcal{Desmos}$: Multiplicities}} 
\begin{center}
...
$-2$ and $3$ are consecutive zeros or roots of $f$.  Since $f$ is continuous, it must remain positive or negative on the interval $(-2,3)$.

This means there must be a local maximum or minimum somewhere inside the interval $(-2,3)$.


\pdfOnly{\textbf{$\mathcal{Desmos}$: Approximating Extrema}}
\begin{onlineOnly}
\textbf{\textcolor{teal}{$\blacktriangleright \mathcal{Desmos}$: Approximating Extrema}} 
\begin{center}
\desmos{igck8p6w15}{400}{300}
\end{center}
\end{onlineOnly}
...
Calculus will give us the tools to obtain derivatives for ourselves, and that will help us locate where in the domain maximums and minimums occur.
...
Graphically, we can think of critical numbers through characteristics of tangent lines.
...
\begin{idea} \item \textbf{\textcolor{blue!55!black}{Critical numbers}}

\textbf{Critical numbers} are domain numbers, which correspond to points on the graph where 

\begin{itemize}
\item the tangent line has slope $0$ (is horizontal), or 
\item the tangent line has no slope (is vertical), or
\item there is no tangent line (corner, cusp, discontinuity).
\end{itemize}
...
Since graphs of polynomials do not have breaks or cusps or vertical tangent lines, critical numbers are domain numbers marking flat/horizontal places in the graph.  These include tops of hills and bottoms of valleys. Critical numbers include places where extreme values occur.


The next example shows that critical numbers also include places that are not marking maximums or minimums.
...
\pdfOnly{\textbf{$\mathcal{Desmos}$: Horizontal Tangent Line}}
\begin{onlineOnly}
\textbf{\textcolor{teal}{$\blacktriangleright \mathcal{Desmos}$: Horizontal Tangent Line}} 
\begin{center}
...
\textbf{\textcolor{red!70!black}{$\blacktriangleright$}} Critical numbers are \textbf{candidates} for locations of maximums and minimums.
...
\begin{example} Cubic Analysis


Let $p(t) = (x+4)(x+1)(x-3)$

Once we get to Calculus, you will be able to get the derivative of $p$ for yourself. Right now, the derivative will just be given.

$p'(t) = 3 \, t^2 + 4 \, t - 11$

We'll use the derivative to determine the behavior of $p$.
...
\begin{tikzpicture} 
  \begin{axis}[
            domain=-6:5, ymax=15, xmax=5, ymin=-25, xmin=-6,
            axis lines =center, xlabel=$t$, ylabel=$y$,
            ytick={-25,-20,-15,-10,-5,0,5,10,15},
            xtick={-6,-4,-2,0,2,4},
            ticklabel style={font=\scriptsize},
            every axis y label/.style={at=(current axis.above origin),anchor=south},
            every axis x label/.style={at=(current axis.right of origin),anchor=west},
          ...
          \addplot [line width=2, penColor, smooth, domain=(-4.7:3.4),<->] {(x+4)*(x+1)*(x-3)};
...
Let's step through our standard analysis list.




\textbf{Domain:} Since $p$ is a polynomial, its domain is $(-\infty, \infty)$.


\textbf{Zeros:} By the Zero Product Property, for $p = 0$, 

either $x+4=0$, $x+1=0$, or $x-3=0$.


Either $x=-4$, $x=-1$, or $x=3$.

All of these are in the domain.


The zeros of $p$ are $\{ -4, -1, 3 \}$.


\textbf{Continuity:} Since $p$ is a polynomial, it is contiuous.


\textbf{End-Behavior:} $p$ is a polynomial with degree $3$, which is odd. Therefore, the end-behavior of $p$ is opposite on either side.  Since the leading coefficient is positive, we have
...
We'll use the derivative, $p'$.

We'll need the sign of $p'$. 

So, we'll factor $p'$ to getthe critical numbers of $p$.

$p'(t) = 3 \, t^2 + 4 \, t - 11$ doesn't factor nicely, so we'll use the quadratic formula.


\[ 
\frac{-b \pm \sqrt{b^2 - 4 \, a \, c}}{2 \, a} = \frac{-4 \pm \sqrt{4^2 - 4(3)(-11)}}{2(3)}
\]

\[ 
\frac{-4 \pm \sqrt{148}}{6} = \frac{-4 \pm 2 \sqrt{37}}{6} = \frac{-2 \pm \sqrt{37}}{3}
\]

Just Checking...
For a cubic polynomial, these critical numbers should be in between the zeros.

$ \frac{-2 - \sqrt{37}}{3} \approx -2.69 $ \\
$ \frac{-2 + \sqrt{37}}{3} \approx 1.36 $
...
\[ p'(t) = 3 \left( t - \frac{-2 - \sqrt{37}}{3} \right) \left( t - \frac{-2 + \sqrt{37}}{3}\right)
\]

We want to know where $p'$ is positive and negative.

$p'$ is a quadratic function with a positive leading coefficient, so we know it is positive : negative : positive, switching at is zeros.


\begin{itemize}
\item $p'$ is positive on $\left(-\infty, \frac{-2 - \sqrt{37}}{3} \right)$
\item $p'$ is negative on $\left(\frac{-2 - \sqrt{37}}{3}, \frac{-2 + \sqrt{37}}{3} \right)$
\item $p'$ is positive on $\left(\frac{-2 + \sqrt{37}}{3}, \infty \right)$
\end{itemize}


\begin{itemize}
\item $p$ is increasing on $\left(-\infty, \frac{-2 - \sqrt{37}}{3} \right)$
\item $p$ is decreasing on $\left(\frac{-2 - \sqrt{37}}{3}, \frac{-2 + \sqrt{37}}{3} \right)$
\item $p$ is increasing on $\left(\frac{-2 + \sqrt{37}}{3}, \infty \right)$
\end{itemize}
...
\[  \lim\limits_{t \to -\infty} p(t) = -\infty   \, \text{ and } \,   \lim\limits_{t \to \infty} p(t) = \infty \]

we know there is not a global maximum nor a global minimum.



\textbf{Local Maximums and Minimums:} 

$p$ has two critical numbers, so two candidates for local maximum and/or minimum. We need the beahvior of $p$ around these critical numbers.



On $\left(-\infty, \frac{-2 - \sqrt{37}}{3} \right)$, $p$ is increasing. \\
On $\left(\frac{-2 - \sqrt{37}}{3}, \frac{-2 + \sqrt{37}}{3} \right)$, $p$ is decreasing. \\
$\frac{-2 - \sqrt{37}}{3}$ is the location of a local maximum.


On $\left(\frac{-2 - \sqrt{37}}{3}, \frac{-2 + \sqrt{37}}{3} \right)$, $p$ is decreasing. \\
On $\left( \frac{-2 + \sqrt{37}}{3}, \infty \right)$, $p$ is increasing. \\
$\frac{-2 - \sqrt{37}}{3}$ is the location of a local minimum.


Just Checking...this agrees with the graph.


\textbf{Range:}  Since $p$ is continuous and

 \[  \lim\limits_{t \to -\infty} p(t) = -\infty   \, \text{ and } \,   \lim\limits_{t \to \infty} p(t) = \infty \]


 the range of $p$ is $(-\infty, \infty)$.
...
\textbf{\textcolor{red!70!darkgray}{$\blacktriangleright$}} Zeros, Factors, and Intercepts are different things, but they all refer to the same idea.
...
The existence of any of these implies the other two
