...
Rational functions are constructed from polynomials. They are fractions of polynomials
...
\textbf{Polynomial-like Functions}
...
Strictly speaking, these are not power functions, but more like shifted power functions - in the denominator. They are compositions of power functions with linear functions.
...
\begin{tikzpicture} 
  \begin{axis}[
          ...
          \addplot [line width=2, penColor, smooth, samples=200, domain=(-2.68:19),<->] {((x-1)*(x-5))/(5*(x+3))};
...
And, since the derivative of a rational function is also a rational function, multiplicities are also helpful when determining where a rational function is increasing or decreasing, because the multiplicities of the derivative will tell us when the derivative changes sign. 
