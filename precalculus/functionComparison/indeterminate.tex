...
This goes for approaching $0$ as well. 
...
As $t$ approaches $\infty$, the exponential pieces will dominate the constant pieces.  
...
As $t$ approaches $-\infty$, we have a different story.  $\lim\limits_{t \to -\infty} e^t = 0$. Therefore, in this case, the constant terms dominate.
...
We have been examining functions that approach infinity (or $0$) similarly. To cpmare them, we combine similar functions into a new quotient function, this new function might have a contant end-behavior. 


Fractions help us compare values


\[ \frac{very \, small}{very \, big} = small\]

\[ \frac{very \, big}{very \, small} = big\]



But that is only half of the stroy.
...
Both the functions $S(t) = \sqrt{2t^3 - 5}$ and $L(t) = t\sqrt{9t+4}$ approach $\infty$ as $t$ approaches $\infty$. 
...
Both the functions $S(t) = \sin(t)$ and $L(t) = t$ approach $0$ as $t$ approaches $0$. 
...
\begin{tikzpicture}
  \begin{axis}[
            ...
            \addplot [line width=2, penColor2, smooth,samples=100,domain=(-1:1),<->] {sin(deg(x))/x};
...
\textbf{\textcolor{blue!55!black}{$\blacktriangleright$ desmos graph}} 
\begin{center}
\desmos{doncbpa943}{400}{300}
\end{center}
\end{onlineOnly}



The graph is very suggestive that $\sin(t)$ and $t$ approach $0$ in exactly the same way.  Their quotient approaches $1$ as $t$ appraoches $0$.
...
\begin{tikzpicture}
  \begin{axis}[
            ...
            \addplot [line width=2, penColor2, smooth,samples=100,domain=(0.1:8),<->] {(1+1/x)^x};
...
Calculus will investigate the ideas of indeterminacy in detail.  We are just getting started.
...
more examples can be found by following this link\\ \link[More Examples of Dominance]{https://ximera.osu.edu/csccmathematics/precalculus/precalculus/functionComparison/examples/exampleList}
