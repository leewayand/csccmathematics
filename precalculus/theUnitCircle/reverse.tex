...
The sine function associates every real number to a number in the interval $[-1,1]$.  This association comes from the unit circle.


Given an angle, we can interpret this angle as a counterclockwise rotation measurement from the positive $x$-axis. There is exactly one point on the unit circle positioned at that angle. The value of sine is the $y$-coordinate of that point.
...
What are all the zeros of $f(x) = 2 \sin(3x - \pi) + 1$.
...
We will mold these into functions later in the course, by restricting the range.  Restricting the range will bring us into compliance with the one and only function rule.  These functions will be called \textbf{arcsine} and \textbf{arccosine}.
...
The value of tangent is negative here, which is what should happen for an angle in quadrant II.
