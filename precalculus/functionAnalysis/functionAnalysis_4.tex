...
\begin{image}
\begin{tikzpicture}
    \begin{axis}[domain=-10:10, ymax=10, xmax=10, ymin=-10, xmin=-10,
          ...
          \addplot [line width=2, penColor, smooth,samples=100,domain=(-6:0)] ({x},{0.5*x+4});
          \addplot [color=penColor,fill=background,only marks,mark=*] coordinates{(-6,1)}; 
          \addplot [color=penColor,fill=background,only marks,mark=*] coordinates{(0,4)}; 
...
The domain of $f$ is $(-6, 0) \cup (0, 5)$.

At each domain number $f'(a) > 0$. In Calculus, we would say that $f$ is increasing at every domain number.


However, algebraically, consider the domain number $-1$ and $1$.
...
$f$ is not an increasing function on $(-6, 0) \cup (0, 5)$.
...
more examples can be found by following this link\\ \link[More Examples of Function Analysis]{https://ximera.osu.edu/csccmathematics/precalculus/precalculus/functionAnalysis/examples/exampleList}
