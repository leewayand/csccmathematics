...
\begin{observation} \textbf{\textcolor{-red!75}{Principal Interval}}
...
Since the values of $T$ come from the values of $Out$, we are first looking for zeros of $Out$.  

$Out(x) = 2x - 1$ is a linear function and has only one zero, $\frac{1}{2}$. 
...
$\blacktriangleright$ $T$ switches from increasing to decreasing at $\frac{\pi}{2}$, which makes $\frac{\pi}{2}$ a critical number and the location of a local maximum. 




$\blacktriangleright$ $T$ switches from decreasing to increasing at $\frac{5\pi}{6}$, which makes $\frac{5\pi}{6}$ a critical number and the location of a local minimum. 
...
Since $T$ is continuous with a global maximum of $1$ and a global minimum of $-3$, the range is $[-3, 1]$.  
...
more examples can be found by following this link\\ \link[More Examples of Trig Functions]{https://ximera.osu.edu/csccmathematics/precalculus/precalculus/trigFunctions/examples/exampleList}
