...
Since the values of $K$ come from the values of $Out$, we are first looking for zeros of $Out$.  

$Out(x) = -x$ is a linear function and has only one zero, $0$. 
...
$K$ is monotonic (its behavior doesn't change) and it is unbounded.  There are no local minimums or minimums. 
