...
Linear functions are those functions that have a constant rate of change, which can be described with formulas like
...
\textbf{Restricting:}  We can restrict the domain of any function.  Restricting the domain just means that the stated domain is not all the numbers that could be used in the formula.  For linear functions, we will alert the reader by titling such functions as restricted linear functions.
...
\textbf{Restricting:} If we restrict the domain and remove the zero, then it is possible our linear function will not have a zero.
...
\pdfOnly{
\textbf{$\mathcal{DESMOS}$: End-Behavior}
}
\begin{onlineOnly}
...
Our notation for end-behavior is called \textit{limit notation}. 
...
We say ``leading coefficient'' for $A$.


We say ``the constant term'' for $B$.
...
The graphs above vividly suggest that linear functions are always increasing or always decreasing (unless they are constant).
...
\textbf{Restricting:} If we restrict the domain, then we might have a maximum or minimum. 
...
If the linear function is a constant function, $L(x) = B$, then the range is just a single number $\{ B \}$. 

Otherwise, all linear functions have the same range, $(-\infty, \infty)$ 

\textbf{Restricting:} Restricting the domain will also restrict the range.
...
\textbf{Rigor:} Reasons are given for each conclusion. That is rigor. 
...
\textbf{Reasons:} Reasons are given for each conclusion.  The quickest reason is to cite the category of the function, because we know lots of characteristics for functions in each category.  This makes it very helpful to study categories of functions. 
...
\begin{itemize}
\item If $A > 0$ and $C > 0$, then $A C > 0$
\item If $A > 0$ and $C < 0$, then $A C < 0$
\item If $A < 0$ and $C > 0$, then $A C < 0$
\item If $A < 0$ and $C < 0$, then $A C > 0$
\end{itemize}
...
This gives us a map that connects the rates of change of linear functions to the rate of change of their composition, which we call the \textbf{Chain Rule}.


\begin{idea} \textbf{\textcolor{blue!55!black}{Chain Rule}}
