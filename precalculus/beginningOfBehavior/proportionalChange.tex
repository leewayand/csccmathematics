...
Two related measurements change \textbf{\textcolor{blue!55!black}{proportionally}} when a change in one measurement is a constant multiple of the change in another measurement. 


\begin{warning} \textbf{\textcolor{red!70!black}{The Change is Proportional}}


The values of the measurements are not constant multiples of each other. 
...
The growth of the two proportional measurements has a constant rate, a constant conversion factor. 
...
\begin{notation}  \textbf{\textcolor{red!50!green}{$\Delta$}} 
...
Water freezes at $0^{\circ}$ C and $32^{\circ}$ F.  These are the two starting points.

From here, if the Celsius measurement changes by $100^{\circ}$, then the Fahrenheit measurement changes by $180^{\circ}$.  $18$ each of $5$'s and $9$'s. We get water boiling at $100^{\circ}$ celsius and $212^{\circ}$ degrees.
...
\[
\frac{\Delta Distance}{\Delta Time} = \frac{60 \, miles}{1 \, hour} 
\]
...
Two measurements share a \textbf{constant rate of change} of $\tfrac{A}{B}$ if whenever measurement \#1 changes by $B$, then measurement \#2 changes by $A$ and vice versa.
...
This rate is not dependent on the amount of each measurement present.  The rate is the same throughout the situation.  It is a constant.
...
\begin{idea} \textbf{\textcolor{blue!55!black}{Multiplier}}


A constant growth rate is a multiplier.
...
The rate converts change in one measurement to the equivalent change in the other measurement.
...
\begin{example} Constant Rate of Change
...
What are the units of $0.450808$ in the equation?
...
Let $L$ be a linear function.  That means it has its own unique constant rate of change.  Let's call it $m$. 
...
Since $(A, B)$ is a pair in $L$, we know that $B = L(A)$.  And, if $(x, y)$ represents any other pair in $L$, we know that $y = L(x)$.  Replacing these in the equation for constant slope gives
...
\textbf{Note:} In advanced mathematics, a similar idea called \textit{linear transformations} are required to include the pair $(0,0)$.  This would result in $L(0) = 0$. \\  
This is not required for Calculus. 




\begin{example} A Pair and a Rate-of-Change
...
\begin{formula} \textbf{\textcolor{purple!65!blue}{Slope}} 
...
\begin{formula} \textbf{\textcolor{purple!65!blue}{Point-Slope Form for a Line}} 


In practice, along with the slope, $m$, we almost always have a point on the line, $(a,b)$, that is not an intercept. The point-slope form of a line describes all of the points on the line.
...
\begin{center}
The point-slope form is the one you want to memorize.
\end{center}
...
\pdfOnly{
\textbf{$\mathcal{DESMOS}$: Graph of B(t)}
}
\begin{onlineOnly}
\textbf{\textcolor{blue!55!black}{$\blacktriangleright$ desmos graph}} 
\begin{center}
\desmos{dia4922qxi}{400}{300}
\end{center}
\end{onlineOnly}
...
Depending on how we use the graph, we will interpret geometric information differently.  This is common when using mathematics to model. This idea will occur many times in mathematics. Context is everything.
...
\begin{explanation} \textbf{\textcolor{red!75!green}{Vertical Axis}}


When switching our thinking from geometry to functions, which variable is the function, and which is the domain? 
