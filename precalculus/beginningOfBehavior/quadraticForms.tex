...
We have seen that the trajectories of projectiles are described by quadratic equations.  

In fact, quadratic equations and formulas are used everywhere.
...
$A \, t^2 + B \, t + C $ is officially known as the \textbf{standard form}.

\end{definition}


Quadratic functions describe relationships between many different common measurements. 

For us, heading to Calculus, quadratics are the simplest complex function.  We want to understand rates of change and linear functions just don't change their behavior. Quadratics change their behavior. 


Being the simplest complex function makes quadratics our favorite for constructing compositions and illustrating the chain rule (Calculus). 

We will see shortly that involving quadratics in our investigation of rates of change will make their zeros of the utmost importance, because when the rate of change is zero, that marks a change in behavior. 
...
For a quadratic, all this information is directly connected to its leading coefficient and zeros.  So, we have many perspectives and viewpoints of a quadratic function's zeros (also called roots).
...
The quadratic formula tells us that this quadratic function has two roots.  In general, there are several possibilities for zeros of quadratic functions.
...
\begin{example} Two Real Solutions 
...
To use the Quadratic Formula, the quadratic must be in standard form. The equation has to have everything on one side and $0$ on the other.  Our equation is already in that form.
...
\begin{example} One Real Solution
...
\begin{example} No Real Solutions
...
\item If $b^2 - 4 a c$ \, \wordChoice{\choice{$>$}\choice[correct]{$=$}\choice{$<$}} \, $0$, then there is one real solution.
...
The vertex form of a quadratic function, $F(x) = A \, (t-H)^2 + K$ presents some function information quickly.


For instance, the maximum or minimum value is the constant term, $K$.  This extreme value of the function occurs in the domain at $H$. 
...
\begin{idea} \textbf{\textcolor{blue!55!black}{Two Occurrences to One}} 


The idea is to rearrange  $0 = A \, t^2 + B \, t + C$ to look like $0 = A \, (t-H)^2 + K$.  Then there will only be  one occurrence of the $t$ and we can solve for it.


To accomplish this, we need to investigate $A \, (t-H)^2 + K$. For the moment, let's multiply it back out.


\begin{align*}
A \, (t-H)^2 + K & = A \, (t-H)(t-H) + K \\
& = A \, (t^2 - \answer{2 t H} + H^2) + K  \\
& = A \, t^2 - \answer{2 t A H} + A \, H^2 + K
\end{align*}

We want this last line, $A \, t^2 - 2 t A H + A \, H^2 + K$ to be $A \, t^2 + B \, t + C$, our original quadratic.
...
\[ A \, t^2 - 2 t A H + A \, H^2 + K = A \, t^2 + B \, t + C \]


The leading coefficients match, $A = A$.

As long as we know the leading coefficients match, we can factor it out and work with what is left.

Then, let's pretend our first step is factoring out $A$ and let's start over. 
...
\textbf{\textcolor{blue!75!black}{Step 1}} 
 ...
 In this way we can pretend that we had a \textbf{monic} quadratic from the beginning.  That means we can pretend the leading coefficient was $1$.
...
We want to complete the square on $t^2 + B \, t + C$.  
...
This last line $t^2 - 2 \, t \, H + H^2 + K$ should turn out to be $t^2 + B \, t + C$.

We want


\[   t^2 - 2 \, t \, H + H^2 + K = t^2 + B \, t + C   \]



\begin{explanation} \textbf{\textcolor{red!75!green}{Linear Term}} 


The leading term is $1$ in both (since we factored out $A$). 

Now, the linear terms need to be the same: $ - 2 \, t \, H = \answer{B t}$.  

That tells us that $H = \answer{\frac{B}{-2}}$.

\end{explanation}


To complete the square with a monic quadratic, we need $H = \frac{B}{-2} = -\frac{B}{2}$.  Let's put that in.


\[ t^2 - 2 \, t \, H + H^2 + K = t^2 - 2 t \left( -\frac{B}{2} \right) + \left( -\frac{B}{2} \right)^2 + K = t^2 + B \, t + \left(-\frac{B}{2}\right)^2 + K \]
...
Now there is a mess for the constant term.  We have $\left( -\frac{B}{2}\right )^2 + K$, when we just wanted $C$.  

Then let's just pick $K$ to be $-\left( -\frac{B}{2}\right )^2 + C = -\left(\frac{B}{2}\right)^2 + C$
...
The whole point was to change from an expression with two occurrences of the variable to an expression with only one occurrence of the variable. 
...
From this equation, we can see that this function has no zeros. Squares cannot equal negative numbers.
...
We have already seen that a quadratic equation can have at most two solutions.  So, we must have all the solutions.
...
The graph of $y = 2 \, x^2 - 12\, x + 18$ is a parabola touching the horizontal axis at only one point (its vertex).





There is another viewpoint to the example above.  We arrived at this equation
...
$3$ is the only solution to the equation, but the equation has this solution twice. \textbf{\textcolor{red!70!black}{A double root}}.
...
\begin{idea} \textbf{\textcolor{blue!55!black}{Double Root}} 
...
The previous two examples illustrated that quadratic equations could have one solution or two solutions.  And a quadratic equation can have no real solutions.
...
It feels like that quadratic equation should have a solution.  


All we needed was for $(x-3)^2 = -1$ 

There are no real numbers that will do this.  The square of a real number cannot be negative. 




We can see that the real numbers are not enough for all our equations.  Eventually, we will fill in the missing pieces with the \textbf{Complex Numbers}.  They will include $\sqrt{-1}$.  Then our last example will have two complex solutions.  
...
Eventually, we'll fill in the holes.
...
It presented a step-by-step procedure for applying algebra that would transform a quadratic polynomial in standard form to one in vertex form.  The procedure gathers terms that could be rewritten as a square.  
...
Let's think of these as functions.
...
The discussion for the vertex form of a quadratic function tells us that the extreme value of the function happens at $-\frac{B}{2A}$.
...
\begin{explanation} \textbf{\textcolor{red!75!green}{Quadratic Formula}} 
...
$\blacktriangleright$ If two quadratics have the same two roots, then they must be constant multiples of each other.
...
\[ f(t) = A \, \left(t - \frac{-B + \sqrt{B^2 - 4 A C}}{2A}\right)   \left(t -  \frac{-B - \sqrt{B^2 - 4 A C}}{2A}\right)   \]
...
Second, $2$ is a prime number, therefore, its factors are $2 \cdot 1$ or $-2 \cdot -1$.
...
$3$ is a prime number, therefore, its factors are $3 \cdot 1$ or $-3 \cdot -1$. However, $-3 \cdot -1$ is actually redundant.  If we picked $-3 \cdot -1$, then we could just factor out $-1$ from both and get back to $3 \cdot 1$.
...
All the factorizations above involve integers, because we are fast with those. 



If the roots of the quadratic involve square roots, then we probably are not guessing them.  In any case, the quadratic formula will find all the roots.
