...
\item If you collect vertical height and time, then the function's graph is an arc (parabola opening down).
\item If you collect vertical velocity and time, then the function's pairs include negative values (when coming down).
\end{itemize}
...
Actually, the story above is true only if you assume that the Earth is pretty much flat and that the gravitational force points down at all times. Oh, and neglect stuff like air resistance.
...
If you want, we can add more details like gravity changes as the object gets higher.




\begin{center}
\textbf{\textcolor{blue!55!black}{Let's Not Do That}} 
\end{center}
...
This is the acceleration we feel on Earth, a downward acceleration, $-9.81 \frac{m}{s^2}$.   

That means, for most of our investigations, the acceleration due to the Earth can be considered a constant.
...
This velocity is a linear function. We will see that the accumulated vertical distance traveled would be represented by the accumulated area under its graph. We will see that this area (distance) is a described by a quadratic. 
...
All those quantities are vectors.  They are actually holding two pieces of information simultaneously. They are holding a vertical measurement and a horizontal measurement. 
...
If we select a random point, $(x,y)$, on the parabola above, then the distance from this point to the focus must equal the distance to the directrix.
