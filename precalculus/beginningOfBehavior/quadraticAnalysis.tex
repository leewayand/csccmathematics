...
We want both graphical and algebraic thinking.  A graph is inherently inaccurate and so we want our analysis reasoning to have algebraic and functional explanations.  But we still want to use graphs to help our algebra.
...
\textcolor{gray!90}{\textbf{alternate text:} Parabola opening down. Top point (vertex) at $(1,6.5)$. Intercepts are $(-4,0)$ and $(6,0)$.}
...
\textcolor{gray!90}{\textbf{alternate text:} Parabola opening up. Bottom point (vertex) at $(-1,-6.5)$. Intercepts are $(-6,0)$ and $(4,0)$.}
...
The squared term, $A \, (x - H)^2$ has the same sign as $A$, except when it equals $0$.  That happens at $H$.  When $x = H$, then $V(h) = K$, which is either the minimum or maximum value of $V$.  The vertex is the graphical representation of the extreme value of the quadratic function and where this extrema occurs in the domain.
...
\item the function has exactly $0$, $1$, or $2$ zeros
\item the function is continuous with no singularities
\item the end-behavior on both sides is the same and it is either $-\infty$ or $\infty$ depending on the sign of the leading coefficient
\item the function either increases and then decreases or decreases and then increases depending on the sign of the leading coefficient
\item the function either has a global maximum or minimum depending on the sign of the leading coefficient
\item the function has one local extrema and it is the same as the global extrema
\item the range looks like either $(-\infty, m]$ or $[m, \infty)$ depending on the sign of the leading coeffcient
\end{itemize}
...
$f$ is a quadratic, so we can use the quadratic formula to get the zeros.
...
\textbf{Note:}  We are just beginning, so parentheses are ok.  Really, $f$ decreases on $(-\infty, -2]$ and increases on $[-2, \infty )$.
...
$f$ is a quadratic function and has a parabola for a graph.


From any of the three forms, we can see that the leading coefficient is $1$, which is positive.  Thus, our parabola opens up.
