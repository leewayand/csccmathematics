...
\outcome{Plot the inverse of a basic functions.}
...
\begin{definition}
  ...
  \begin{image}
    ...
    \end{tikzpicture}
  \end{image}
  We say that $f$ and $g$ are \textbf{inverses} of each other if $f(g(b))
...
\begin{definition}
A function is called \textbf{one-to-one} if each output value corresponds
...
\begin{question}
  ...
  \begin{selectAll}
    ...
    \choice[correct]{Given a runner racing forward on a straight path,
      mapping time to the position of the runner.}
    \choice{Mapping people to their birth date.}  \choice{Mapping
...
the \textbf{vertical line test}. Similarly, a function is one-to-one
if every horizontal line crosses the plot at most once, and we call this 
the \textbf{horizontal line test}.
...
\begin{question}
  ...
  \begin{multipleChoice}
    \choice[correct]{$f^{-1}(0.5)>1.5$}
    \choice{$f^{-1}(0.5)\le1.5$}
    \choice{$f^{-1}(0.5)$ is not defined}
  \end{multipleChoice}
    \end{enumerate}
  \end{question}
\end{question}
...
f(x) = x^3.
\]
Does the function $f$ have an inverse? If so, what is it? If not, attempt to
restrict the domain of $f$ and find an inverse on the restricted
domain.
\begin{explanation}
