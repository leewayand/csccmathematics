...
\begin{document}
\begin{abstract}
The limit of a continuous function at a domain number is equal to the value of the function at that domain number. 
\end{abstract}
\maketitle
...
\begin{definition}
  A function $f$ is \textbf{continuous at a domain number} $a$ if
  \[
  \lim_{x\to a}f(x) = f(a).
  \]
\end{definition}
...
\begin{center}
  ``a function $f$ is continuous at a domain number $a$''
\end{center}
...
\begin{example}
Find the discontinuities (the domain numbers $x$ where a function is not
...
Building from the definition of \textit{continuity at a domain number}, we can
now define what it means for a function to be \textit{continuous} on
an open interval.

\begin{definition}
  A function $f$ is \textbf{continuous on an open interval} $I$ if
...
\begin{definition}
  A function $f$ is \textbf{left continuous} at a domain number $a$ if
  $\lim_{x\to a^-} f(x) = f(a)$.

  A function $f$ is \textbf{right continuous} at a domain number $a$ if
  $\lim_{x\to a^+} f(x) = f(a)$.
\end{definition}
...
\begin{definition}
  A function $f$ is
  \begin{itemize}
    \item \textbf{continuous on a closed interval} $[a,b]$ if $f$ is
      continuous on $(a,b)$, right continuous at $a$, and left
      continuous at $b$;
    \item \textbf{continuous on a half-closed interval} $[a,b)$ if $f$ is
...
\begin{selectAll}
  \choice{$I=[0,10]$}
  \choice[correct]{$I=[0,4]$}
  \choice{$I=[4,6]$}
 \choice{$I=[4,6)$}
  \choice{$I= (4,6]$}
  \choice[correct]{$I= (4,6)$}
...
so $6$ is not included in any intervals.  Our function is right
continuous at $x=0$ and left continuous at $x=10$ so we included
