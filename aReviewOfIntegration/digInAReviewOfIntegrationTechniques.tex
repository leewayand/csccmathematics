...
Let $u= x^2+1$.  We now need to write everything in the integral in terms of this new variable.  Recalling that if $u = f(x)$, the differentials are related by $\d u = f'(x) dx$, we can write $du = \answer[given]{2x} dx$.
...
\begin{image}
  \begin{tikzpicture}
        \node at (0,0) {
          $\int 2x e^{x^2+1} dx = \int e^{\overbrace{x^2+1}} \underbrace {2x dx} = \int e^u du$.};
        \node at (1.4,-.8) {\small{This is $du$.}};
