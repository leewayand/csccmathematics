...
\begin{definition}\index{slant asymptote}\index{asymptote!slant}
  If there is a nonhorizontal line $\ell(x) = m\cdot x+b$ such that
  \[
  \lim_{x\to \infty}\left(f(x) - \ell(x)\right) = 0 \qquad\text{or}\qquad \lim_{x\to -\infty} \left( f(x) - \ell(x)\right) = 0,
  \]
  then $\ell$ is a \textbf{slant asymptote} for $f$.
\end{definition}
\begin{question}
...
f(x) = \frac{3x^2+x+2}{x+2}.
  \]
  \begin{explanation}
    We are looking to see if there is a line $\ell$ such that
    \[
    \lim_{x\to \infty}\left(f(x) - \ell(x)\right) = 0\qquad\text{or}\qquad \lim_{x\to -\infty} \left( f(x) - \ell(x)\right) = 0.
    \]
...
\]
    We know that $f(x)$ is continuous everywhere except at $x = -2$ and $\ell(x)$ is continuous everywhere, so 
    ...
    \]
   In other words, $\ell(x) = \answer[given]{3x-5}$ is a slant asymptote
   for our function $f$.  You should check that we get the same slant
   asymptote $\ell(x) = \answer[given]{3x-5}$ when we take the limit to
