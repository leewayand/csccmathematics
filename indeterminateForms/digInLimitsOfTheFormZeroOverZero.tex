...
\begin{question}
  ...
  \begin{selectAll}
    \choice[correct]{$\lim\limits_{x\to 0}\frac{\sin(x)}{x}$}
    \choice{$\lim\limits_{x\to 0}\frac{\cos(x)}{x}$}
    \choice{$\lim\limits_{x\to 0}\frac{x^2-3x+2}{x-2}$}
    \choice[correct]{$\lim\limits_{x\to 2}\frac{x^2-3x+2}{x-2}$}
    \choice{$\lim\limits_{x\to 3}\frac{x^2-3x+2}{x-3}$}
  \end{selectAll}
\end{question}

\begin{warning}
  The symbol $\boldsymbol{\tfrac{0}{0}}$\ is \textbf{not} the number $0$ divided by
  $0$. It is simply short-hand and means that a limit $\lim\limits_{x\to a}
