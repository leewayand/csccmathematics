...
\choice{$\ensuremath{\boldsymbol{0^0}}$}
\choice[correct]{$\ensuremath{\boldsymbol{1^\infty}}$}
\choice{$\ensuremath{\boldsymbol{\infty^0}}$}
\end{multipleChoice}

Now
\[
\lim_{x\to0^+} (1+x)^{-\ln(x)} = e^{\lim_{x\to0^+}\answer{-\ln(x) \ln(1+x)}}.
\]

The value of the limit in the exponential above is $\answer{0}$, so
\[
\lim_{x\to0^+} (1+x)^{-\ln(x)} = e^{\answer{0}} = \answer{1}.
\]
