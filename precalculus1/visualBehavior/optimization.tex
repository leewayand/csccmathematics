...
When using functions to analyze situations, we are often interested in the maximum and minimum values that a function takes on. The maximum or minimum value the function attains. These are often called \textbf{extreme} values. We would like to know the extreme values and where, in the domain, they occur.
...
A function has at most, one maximum value and one minimum value, but each can occur at multiple domain numbers.
...
\textbf{\textcolor{blue!55!black}{$\blacktriangleright$}}  $g$ has a local minimum of $-2.3$, which occurs at $0.9$. 
...
\textbf{\textcolor{blue!55!black}{$\blacktriangleright$}} $g$ has a local maximum of $2.4$, which occurs at $-4.9$. 
...
\textbf{\textcolor{blue!55!black}{$\blacktriangleright$}} $g$ has a local minimum of $-3.3$, which occurs at $-8$. 
...
\textbf{Note: } This is not true for closed intervals.  For example, the closed interval $[3, 3]$ contains only one number.  This is one reason we use open intervals in our reasoning.
