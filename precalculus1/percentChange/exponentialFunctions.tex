...
Our base is less than $1$.  Therefore, as its exponent gets large and positive, we multiply by more $\frac{2}{3}$'s and the values gret smaller.


However, the variable, $t$, in the exponent is multiplied by $-1$.  Therefore, we need $t$ to get large and negative in order for the exponent to get large and positve.


\begin{itemize}
\item $\left( \frac{2}{3} \right)^{3-t}$ grows when $t$ becomes more negative.
\item $\left( \frac{2}{3} \right)^{3-t}$ decays when $t$ becomes more positive.
\end{itemize}
