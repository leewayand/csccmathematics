...
\begin{observation} \textbf{\textcolor{-red!75}{Slope tells us how a line is tilted.}}
...
Suppose we have a line that contains the point $(a,b)$ and has slope $m$.  Let $(x,y)$ represent any other point on the line.  Then,
...
This can be rewritten as $y - b = m(x - a)$. We call this the \textbf{\textcolor{purple!85!blue}{point-slope}} form of a line.  This equation can be rewritten as $y = m(x - a) + b$, which can be viewed as a formula for a function: $y(x) = m(x - a) + b$ 
...
\begin{itemize}
\item upper side : $\sqrt{(a + 1 - a)^2 + (n + b) - b)^2} = \sqrt{\answer{1 + n^2}}$
\item lower side : $\sqrt{(a + 1 - a)^2 + (m + b) - b)^2} = \sqrt{\answer{1 + m^2}}$
...
Horizontal lines have equations of the form $y = y_0$, where the $y$ is the name of the vertical axis. You can view this equation as $y = 0 \cdot x + y_0$ to see that horizontal lines have $0$ slope. Every point on this horizontal line is of the form $(x, y_0)$. The horizontal coordinates changes. The vertical coordinates do not.
...
Vertical lines have equations of the form $x = x_0$, where the $x$ is the name of the horizontal axis. 

If you want to see both variables, then you can view this equation as $x = 0 \cdot y + x_0$ to see that every point on this line is of the form $(x_0,y)$, making it vertical.

Vertical lines do not have slope, since the formula for slope would look like $\frac{r}{0}$.


Since, the graph is a vertical line, there are points with the same horizontal coordinate but different vertical coordinates. Interpreting this into function language, we have a domain number with multiple function values. This is not allowed for functions.

Vertical lines are not graphs of functions.
