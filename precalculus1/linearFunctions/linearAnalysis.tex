...
\textbf{Restricting:}  We can restrict the domain of any function.  Restricting the domain just means that the stated domain is not all of the numbers that could be used in the formula.  For linear functions, we will alert the reader by titling such functions as restricted linear functions.
...
Our notation for end-behavior is be called \textit{limit notation}. 
...
\textbf{Note:} It is the fact that $g(t)$ CAN be written in the official template form that makes it a linear funciton. 
...
\textbf{Reasons:} Reasons for each conclusion.  The quickest reason is to cite the category of the function, because we know lots of characteristics for functions in each category.  This makes it very helpful to study categories of functions. 
