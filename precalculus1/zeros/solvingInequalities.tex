...
For instance, when solving $\frac{1}{x} < 2$, you cannot multiply both sides by $x$, because you do not know if $x$ is positive or negative, so you do not know whether to flip the inequality sign or not.

For instance, when solving $x^2 - 3x < 2$, there is nothing to do with the inequality.


Instead, it is much better to switch from an algebraic viewpoint to a function viewpoint. 


When we swicth from an algebraic to a function viewpoint, then we can bring all of our knowledge about functions to help us. Chacarteristics and features like continuity and zeros are very helpful when thinking about inequalities. 
...
\textbf{\textcolor{blue!55!black}{Quick Analysis}} 


The square root function is never negative.  Therefore, when $x > 0$ each of the terms would be positive.  So, at least this inequality has $(0, \infty)$ in the solution set.  
...
Incidentally, $\sqrt{x+9} + x \cdot \frac{1}{2} (x+9)^{-\tfrac{1}{2}}$ is the derivative of $x \sqrt{x+9}$. 

Therefore, $f(x) = x \sqrt{x+9}$ is increasing on $(-6, \infty)$. 
...
$x^2$ is a common factor of the numerator and denominator.
...
\textbf{\textcolor{red!70!darkgray}{$\blacktriangleright$}} We have an algebraic point of view: the function, $f$, is increasing on the set $S$, if whenever $a < b$, then $f(a) < f(b)$.



\textbf{\textcolor{red!70!darkgray}{$\blacktriangleright$}} We have a pointwise view: the function, $f$, is increasing at the number $a$, if $f'(a) > 0$. 
