...
From the factored for, we can see that $-5$ and $1$ are the zeros of $f(x)$.  These will be represented as the intercepts $(-5, 0)$ and $(1,0)$.


From the vertex form, we can see that the lowest point of the parabola will be $(-2, 9)$.  The funciton $f$ has a global minimum value of $-9$, which occurs at $-2$ in the domain.
...
\begin{tikzpicture}
     \begin{axis}[
        ...
        \addplot [draw=penColor, very thick, smooth, domain=(-9:7),<->] {(x+5)*(x-1)};
        %\addplot [line width=1, gray, dashed,samples=100,domain=(-9.5:9.5)] ({3},{x});
