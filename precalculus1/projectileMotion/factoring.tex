...
We have seen that one of our goals for analysis is to identify zeros of functions. 
...
$\blacktriangleright$ Hence, we have discovered that if two quadratics have the same two roots, then they must be multiples of each other.
...
The quadratic formula always works...for quadratics!  But, many times it is slow with lots of steps and reducing.  Sometimes it is just easier to guess the factors.  ``Easier'' usually means integers.
...
First, the coefficients, $2$, $-5$, and $-12$, have no common factors.
