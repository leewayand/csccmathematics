...
We can create another quadratic function from these two zeros.
...
This quadratic function has the same two zeros as $Q(t)$.  If we multiply by $a$, we get 
...
The real zeros can be obtained via the quadratic formula - provided the discrimanant is positve.  If the discriminant is $0$, then $r_1 = r_2$ and we get a square.  If the discriminant is negaqitve, then $Q(t)$ doesn't factor with real numbers.  
...
First, the coefficients have no common factors.
...
First, the coefficients have a common factor.  Factor out $2$ to get $p(y) = 2 (3 y^2 - 8 y - 11)$.


Now factor $p(y) = 3 y^2 - 8 y - 11$.
