...
Parabolas can have intercepts.

Quadratic functions can have zeros.
...
\textbf{\textcolor{green!50!black}{Zero Product Property}} 
...
Like when solving equations. 


\begin{procedure}  \textbf{\textcolor{purple!30!blue}{Solving Equations}}
...
We have already seen that factors correspond to zeros of a function. Thus, it comes as no surprise that identifying zeros of functions is a top priority.  Factoring is a top priority. Spotting horizontal intercepts in the graph is a top priority.

As we can see from their parabolic graphs below, quadratics can have $0$, $1$, or $2$ real zeros.  
...
How do we find them? Factoring is one way.  

\begin{example} Factoring and Zeros
...
Zeros of linear functions were easy to identify.  Simply apply a little algebra to the equation to get the variable by itself on one side of the equation.  But, this is not always possible with equations of the form $0 = a \, t^2 + b \, t + c $, because there are two occurrences of the variable and they have different degrees. They cannot be combined.
...
To accomplish this, we need to investigate $A \, (t-B)^2 + C$. 

For the moment, let's multiply it back out.
...
We want $A \, t^2 - 2 t A B + A \, B^2 + C$ to look like $a \, t^2 + b \, t + c$, our original quadratic.
...
\textbf{\textcolor{blue!75!black}{Step 1}} 
 ...
 Factor out the leading coefficent.   In this way we can pretend that we have a \textbf{monic} quadratic from the beginning.  That means we can pretend the leading coefficient was $1$.
...
Take half of the linear coefficient, $\frac{2}{2} = 1$, square that $1^2 = 1$. Add and subtract it, so that we have just added $0$ to the expression and not changed its value.
...
Remember, this was inside parentheses from factoring out the leading coefficient.
...
If the vertex of the parabola is the only intercept, then the corresponding quadratic function has a double root (zero).

That is the quadratic is of the form  $A (x - r)(x - r) = A (x - r)^2$

The zero (root), $r$, occurs twice in the factorization.

\end{idea}



In the example above, we arrived at this equation
...
\begin{itemize}
\item Our first example had two distinct real solutions.  
