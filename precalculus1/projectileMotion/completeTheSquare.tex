...
\textbf{Quadratic Domain and Range} \\

The implied domain of a quadratic function is all real numbers.  Of course, any quadratic function could come with a stated domain that restricts the domain.

From the graphs above we can see that the implied range of a quadratic comes in two types.  

\begin{itemize}
\item The range could be all real numbers greater than or equal to some particular real number.
\item The range could be all real numbers less than or equal to some particular real number.
\end{itemize}
...
WE have already seen that factors correspond to zeros of our funciton. Thus, it comes as no surprise that identifying zeros of functions is a top priority.  Factoring is a top priority. Spotting intercepts in the graph is a top priority.
...
\textbf{Step 1}  Factor out the leading coefficent.   In this way we can pretend that we had a \textbf{monic} quadratic from the beginning.  That means we can pretend the leading coefficient was $1$.
...
Take half of the linear coefficient, $\frac{2}{2} = 1$, squre that $1^2 = 1$, and add and subtract it, so that we have just added $0$ to the expression and not changed its value.
...
\end{example}
The graph would be a parabola touching the horizontal axis at only one point (its vertex).
...
$3$ is the only solution to the equation, but the equation has this solution twice.
...
The above example illustrates that there are numbers missing from teh real numbers.  


$\blacktriangleright$  \textbf{A Peek Ahead}
...
Then our last example will have two complex solutions.  Our first example had two real solutions.  Our second example had two identical solutions.  All quadratics will have two solutions. \\
...
But, for now, we a staying inside the real numbers.



 For now, we note that there are no \textbf{real} solutions.
