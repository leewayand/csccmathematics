...
We have already seen if we write a quadratic function in squared form, $f(x) = a (x - h)^2$, and examine the interval $[h, x]$, then the rate of change of $f$ is the linear function $RoC_f(x) = a (x - h)$.


On the interval $[x,h]$, the rate of change would look like 



\[
\frac{0 - a (x - h)^2}{h - x} = a (x - h)
\]


Same linear function works to the right and left. 



$\blacktriangleright$ With $a > 0$, $a (x - h) > 0$ to the right of $h$.  A positive rate of change and $f(x) = a (x - h)^2$ is increasing. \\


$\blacktriangleright$ With $a > 0$, $a (x - h) < 0$ to the left of $h$.  A negative rate of change and $f(x) = a (x - h)^2$ is decreasing. \\



We can even add a constant onto our function, because the rate of change of a constant function is $0$.



This fits well with our graph.



$\blacktriangleright$ \textbf{Quadratic Function:} $f(x) = a (x - h)^2 + k$ \\

$\blacktriangleright$ \textbf{Rate of Change:} $RoC_f(x) = a (x - h)$ \\
...
$\blacktriangleright$ When the linear rate of change function $RoC_f(x) < 0$, then $f(x)$ is decreasing. \\

$\blacktriangleright$ When the linear rate of change function $RoC_f(x) > 0$, then $f(x)$ is increasing. \\

$\blacktriangleright$ When the linear rate of change function $RoC_f(x) = 0$, then $f(x)$ is neither increasing nor decreasing and the graph of $y = f(x)$ is flat.
...
We can also look at the linear $RoC_f(x)$ function.  Where $RoC_f(x) = 0$ is where the vertex is located, which encodes the maximum or minimum value of $f$.
