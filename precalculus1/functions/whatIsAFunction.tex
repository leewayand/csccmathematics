...
This particular rational number from the domain is paired with both $\tfrac{2}{3}$ and $\tfrac{4}{6}$. That isn't allowed for a funciton. Each domain item must be paired with exactly one codomain number. \textit{RationalAdd} is not well-defined and not a function.
...
\textit{functionName(d)} is called ``the image of d under functionName''. \\
