...
\begin{onlineOnly}
\begin{explanation} \textbf{\textcolor{red!75!green}{Video: Introduction to Functions}} 
...
\begin{example} The $SSN$ function

The $SSN$ function has a domain consisting of all U.S. citizens and a codomain consisting of all 9-digit numbers.  $SSN$ pairs a U.S. citizen in the domain with the 9-digit number in the codomain that was issued to them as their social security number. 


This function is not well-defined, because there are U.S. citizens without a social security number, like many new-born babies.   $SSN$ is not a function.
\end{example}


\begin{example} The $ReverseSSN$ function
...
This function is not well-defined, because there are 9-digit numbers that have not been issued as social security numbers.  $ReverseSSN$ is not a function.
\end{example}


\begin{example} The $RationalAdd$ function
...
This particular rational number from the domain is paired with both $5$ and $10$. That isn't allowed for a function. Each domain item must be paired with exactly one codomain number. 

$RationalAdd$ is not well-defined and not a function.

\end{example}



\begin{warning} \textbf{\textcolor{red!70!black}{Numbers vs. Notation}}
...
\begin{example} The $SuperBowlWinner$ function  
...
$functionName(d)$ is also called ``the \textbf{\textcolor{purple!85!blue}{IMAGE}} of $d$ under functionName''. 
...
$SuperBowlWinner$(Superbowl 13) represents the winning team from Superbowl 13, which is the Pittsburgh Steelers.  The following equality is communicating this connection with an equation.

\begin{itemize}
\item $SuperBowlWinner$(Superbowl 13) = Pittsburgh Steelers  
\end{itemize}
...
The \textbf{range} of a function is the collection of items in the codomain which are paired with some item in the domain.  
...
\begin{notation}\textbf{\textcolor{red!50!green}{Set Symbols}} 
...
$f^{-1}$ is the symbol for the reverse of $f$. 


$f^{-1}$ pairs members of the codomain with members of the domain.  It is just reading the function $f$ backwards or in reverse.
...
Mathematics has many ideas that involve reverse, opposite, backwards, undo, inverse, back up, turn around, etc. In one way or another, $-1$ seems to be involved in the notation for this reverse action.  
