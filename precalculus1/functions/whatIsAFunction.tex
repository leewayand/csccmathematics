...
We finally found a function. That one little rule actually narrows the pool of relations quite a bit.
...
Each domain item is paired with \textbf{\textcolor{red!25!blue!75!}{EXACTLY}} one codomain item.  Not 0. Not 2.  Not 3.  Exactly 1!  Thus, when you pick a domain item, you have automatically selected its partner in the codomain.  We could certainly hunt it down inside the codomain.  But, we can also just talk about "the domain member's partner in the codomain."  We have function notation for this thought.
...
functionName(d) is called "the value of functionName \textbf{\textcolor{red!25!blue!75!}{AT}} d". \\
functionName(d) is pronounced as "functionName \textbf{\textcolor{red!25!blue!75!}{OF}} d".
\end{notation}
...
The rule doesn't refer to the range.
...
\begin{example} \textit{Actor}

Let's name the following function \textit{Actor}.
...
\begin{example} \textit{ActorAgain}

Let's name the following function \textit{ActorAgain}.
...
Is \textit{ActorAgain} well-defined?
...
\end{question} 
What kind of sets of real numbers are there?
