...
\end{multipleChoice}
\begin{feedback}
Remeber that every domain element must be used in exactly one ordered pair. Toy Story is in the domain and not in any pair.
\end{feedback}
...
\[
f^{-1}(T) = \{ d \in D \, | \, f(d) \in T  \} \subseteq T \subseteq C
\]
...
\begin{itemize}
\item $f^{-1}(x)$ might mean the reciprocal, $\frac{1}{f^{-1}(x)}$
