...
\begin{definition}   \textit{Open Set in $\mathbb{R}$} \\

$S \subset \mathbb{R}$ is an \textbf{open set in $\mathbb{R}$} provided 

\[
\text{For each } s \in S, \text{ there exists an } \epsilon > 0 \text{ such that }  (s - \epsilon, s + \epsilon) \subset S
\]
...
The interval $(4,7)$ is an open set in $\mathbb{R}$.
...
Since this is true for all of the numbers in $(4,7)$, then interval is an open set in $\mathbb{R}$.
...
\textbf{\textcolor{purple!50!blue!90!black}{Reasoning:}} \\
...
Let $c \in S$. \\
Then $c$ is in one of the one intervals. \\
Then there exists an $\epsilon > 0$, such that $(c - \epsilon, c + \epsilon)$ is a subset of this one interval.  This interval is a subset of $S$.  Therefore, $(c - \epsilon, c + \epsilon) \subset S$. \\
...
$\blacktriangleright$ These types of statements are oftern said to be \textbf{vacuously true}.  They are true, because the contain nothing that violates the definition - because they don't contain anything at all. 
