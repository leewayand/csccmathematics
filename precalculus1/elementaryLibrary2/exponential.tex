...
We have core exponential functions: $r^x$.  The Elementary Functions include a more general form constructed from a special function property.
...
Let's build such a function up from a given value of $g(0)$ and a percentage growth of $r$ for every change of $1$ in the domain. 
...
Let's build such a function up from a given value of $g(0)$ and a percentage growth of $r$ for every change of $3$ in the domain. 



\begin{procedure}
Moving from $0$ to $3$ gives

\[      \frac{\frac{g(1)-g(0)}{g(0)}}{3-0}  = r  \]

\[      g(1) =  g(0) (3r + 1)  \]



Moving from $3$ to $6$ gives
...
\begin{idea} \textbf{\textcolor{blue!55!black}{Exponential Functions}}
...
\[
A \cdot r^{x + \Delta x} - A \cdot r^x = A \cdot r^x (r^{\Delta x} - 1) = f(x) (r^{\Delta x} - 1)
\]
...
Exponential functions can be thought of as compositions our our core exponential functions with linear functions.
...
Let $L_{out}(y) = A \, y$ be a linear function with zero constant term.


General exponential functions are constructed by composing these component functions.


\[
(L_{out} \circ E \circ L_{in})(x) = A \, a^{B \,x + C}
\]
...
\begin{idea} \textbf{\textcolor{blue!55!black}{Practical Purposes}}
...
\begin{idea} \textbf{\textcolor{blue!55!black}{Coefficients}}


We have four general behaviors of exponential functions, which we will investigate in this course. By changing the sign of the leading coefficent or the leading coefficient in the linear exponent, you can get model of the four types.  
...
This all fits into the Chain Rule story.
...
\item $e^{-x} = (E \circ L)(x)$
\item $-e^x = (L \circ E)(x)$
\item $-e^{-x} = (L \circ E \circ L)(x)$
\end{itemize}


Since $L$ is a decreasing function, every time we compose with it, the behavior reverses.
