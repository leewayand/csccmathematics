...
The Core absolute value function, $|x|$, gives the distance on the number line between a number, $x$, and $0$.  Since distance cannot be negative, it appears that the function just ``makes numbers positive''.


To do this, the Core absolute value function just returns nonnegative numbers unharmed, and makes negative numbers turn positive.  Algebraically, this is accomplished through a piecewise defined function.
...
Technically speaking, $|-3| = -(-3)$ and then $-(-3) = 3$.  The Core absolute value function negates negative numbers.


\textbf{Note:} Just because you see a negative sign doesn't mean you have a negative number.
...
Using the $iRoC$, or derivative, to say where $A(x) = | x |$ increasing and decreasing. 
...
Since the absolute value function is an unbounded increasing linear function on $(0, \infty)$, we know that
...
There is no tangent line at $(0,0)$, which means there is no slope. 
...
Rewrite $A(y) = | y - 3|$ without absolute value bars.
...
Each piece is a (restricted) linear function.  The leading coefficients tell us that


\begin{itemize}
\item $|y-3|$ is decreasing on $(-\infty, 0)$
\item $|y-3|$ is increasing on $(0, \infty)$
\end{itemize}
