...
\begin{procedure}
We begin with several functions with their own defining sets of domain-range pairs.  \\
Select some pairs from each of those sets. \\
Collect them into a new set of pairs defining a new function.  
\end{procedure}
...
The graph of a piecewise defined function may not seem like pieces.
...
\begin{cases}
  ...
  x + \frac{9}{4} & \text{ if }  -7 \geq x \leq \frac{5}{2} \\
  -(x+3)^2 + 5   & \text{ if } \frac{5}{2} \le x 
\end{cases}
\]
...
\begin{tikzpicture}
  \begin{axis}[
        ...
        \addplot [draw=penColor,very thick,smooth,domain=(-7:2.5)] {x+8.25};
