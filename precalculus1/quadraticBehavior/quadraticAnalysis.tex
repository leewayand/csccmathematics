...
Remember, we began with graphical analysis, because that is a familiar jumping off point for students.  But, graphs are inherently inaccurate tools.  That isn't what we want. We are taking our familiarity with graphical descriptions and moving them over to algebraic descriptions, because algebra is our exact tool. 
...
Thsi allows us to factor the quadratic.



\[ a \, \left( x -  \frac{-b - \sqrt{b^2 - 4 a c}}{2a}, 0 \right) \, \text{ and } \, \left( x -   \frac{-b + \sqrt{b^2 - 4 a c}}{2a}, 0 \right)   \]
...
Let $f$ be a function. Let $x_0$ be a number in the domain of $f$ such that 

\begin{itemize}
\item $iRoC_f(x_0) = f'(x) = 0$, or
\item $iRoC_f(x_0) = f'(x)$ does not exist
\end{itemize}
...
We can see that the range of a quadratic comes in two types.  
