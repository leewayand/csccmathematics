...
We can be totally exact and algebraic about linear and quadratic functions.


Quadratic functions are those functions which can be described with formulas like

\begin{itemize}
     \item $Q(x) = A \, x^2 + B \, x + C$
     \item $Q(x) = A \, (x - H)^2 + K$
     \item $Q(x) = A \, (x - r_1) (x-r_2)$
\end{itemize}
...
Quadratic functions have the same unbounded end-behavior on both sides of the domain, which is given by the sign of the leading coefficient. 
...
We will want some mathematical notation for end-behavior.  \textbf{Limit notation} will be our way of algebraically describing end-behavior. 
...
The end-behavior is described as 
...
The leading coeffcient of $p$ is \wordChoice{\choice{positive} \choice[correct]{negative}}.
...
The leading coeffcient of $W$ is \wordChoice{\choice[correct]{positive} \choice{negative}}.
...
Our linear rate of change function now informs us about the behavior of the quadratic function. 
...
Maximums and minimum values of a function occur when the behavior of the function changes.
...
\textbf{Note: } Domain numbers where $iRoC_f$ or $f'$ doesn't exist are also places where a function's behavior can change drastically, which is why we included them as critical numbers.
...
The maximum or minimum value of a quadratic function, $f$, is visually encoded in the coordinates of the highest or lowest point on the graph, which is the vertex of the parabola.
...
We can also look at the linear $iRoC_f(x) = f'(x)$ function.  Where $iRoC_f(x) = 0$ is the critical number and corresponds to the vertex on the parabola which visually encodes information about the maximum or minimum value of $f$.
...
The leading coeffcient of $F$ is \wordChoice{\choice[correct]{positive} \choice{negative}}, which means $F$ has a \wordChoice{\choice[correct]{minimum} \choice{maximum}}.
...
If $a>0$, then the parabola opens upward and the extreme point is at the bottom.
...
The vertex visually encodes the minimum value of the function. 
...
Now, the vertex visually encodes the maximum value of the function. 
...
The leading coeffcient of $g$ is \wordChoice{\choice{positive} \choice[correct]{negative}}, which means $g$ has a \wordChoice{\choice{minimum} \choice[correct]{maximum}}.
