...
The graph vividly suggests that quadratic functions switch from increasing to decreasing (or vice versa) at the ``vertex'' number in the domain. Any domain number where a functions switches its behavior is called a \textbf{\textcolor{blue!55!black}{critical number}}.
...
We have seen if we write a quadratic function as $f(x) = a (x - h)^2 + k$, then the instantaneous rate of change of $f$ (the derivative of $f$) is the linear function $iRoC_f(x) = 2 a (x - h)$. The values of $iRoC$ are the slopes of the lines tangent to the parabola.
...
We can also look at the linear $iRoC_f(x) = f'(x)$ function.  Where $iRoC_f(x) = 0$ is the critical number and corresponds to the vertex on the parabola which visually encodes Tinformation about the maximum or minimum value of $f$.
