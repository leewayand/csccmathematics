...
\subsection*{Extending Beyond Continuity} 
...
This situation might occur if we had a restricted domain for a function.
...
A different situation involves a domain number that is the end of an interval in the domain.  The domain number only has one side in the domain.  This automatically disqualifies the function from having a derivative value at this domain number.  The domain number is automatically a critical number.  However, this is still a nice situation, which we would like to describe. 
...
\textbf{Note:} It is possible that we have a domain number where there is both a left and right derivative value and they are different values.  We saw this with an absolute value function. In this case, there would be no value of the derivative.  If there is a derivative value, then the left and right sides must match.  
