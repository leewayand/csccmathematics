...
\begin{tikzpicture}
     \begin{axis}[
        ...
        \addplot [draw=penColor, very thick, smooth, domain=(-5:9),<->] {abs(x-3) - 4};
...
An absolute value ``Vee'' and a point. \\
...
\begin{tikzpicture}
     \begin{axis}[
        ...
        \addplot [draw=penColor, very thick, smooth, domain=(-5:9),<->] {4*(x-2)^(0.3333)};
...
\begin{tikzpicture}
     \begin{axis}[
        ...
        \addplot [color=penColor2,only marks,mark=*] coordinates{(2,0)};
          ...
          \addplot [line width=2, penColor2, smooth,samples=200,domain=(-6:6)] ({2},{x});
...
Here, the tangent line described by the equation $x = 2$. It is a vertical line. \\
...
$f$ has no derivative value at $2$.  $f'(2)$ does not exist.
...
A function with a discontiuity has a break in the graph, which means the graph points do not approach the same point.  This automatically results in the derivative not existing. \\
