...
\begin{notation}  \textbf{\textcolor{red!50!green}{Parentheses}}  

Mathematics overuses symbols and allows the context in which the symbol is used to affect its meaning. Parentheses are not immuned to this overuse.
	...
	\begin{itemize}
		...
		\item \textbf{Intervals:} $(4, 5)$ might repesent an open interval of real numbers.
...
The actual letter ``$F$'' itself is not part of function notation.  The first part of function notation is the name of the function, which in most instances is not ``$F$.'' The parentheses are always there.  The letter $d$ is in the domain position and can be replaced by any expression representing domain elements.  


Unfortunately, mathematics overuses the letter ``$x$'', which often results in students reacting to the letter itself, rather than what it is representing.

We will strive not to overuse $F$ and $d$, or $x$.
\end{warning}
...
\pdfOnly{
\textbf{$\mathcal{VIDEO}$: Formulas are Procedures}
}

\begin{explanation} \textbf{\textcolor{red!75!green}{Video: Formulas are Procedures}}
