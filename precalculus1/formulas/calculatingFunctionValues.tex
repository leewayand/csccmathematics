...
A \textbf{formula} for a function is an algebraic expression involving  domain numbers, that produces the function value at each domain number.
\end{definition}
...
A variable is a letter or word or symbol that represents any and all values in a designated set.


\end{definition}
The ``any and all'' is crucial to a variable, as we will see. 


The simplest instance of function notation looks like $f(d)$.  The $d$ is a variable.  Since $d$ is the entire expression inside the parentheses, $d$ is representing any and all domain values for the function $f$.


\textbf{Note:} The variable involved in function notation does not always represent the domain values, as we shall see. It will become very important to identify when the variable is representing the domain values and when it is not. 



\pdfOnly{
\textbf{$\mathcal{VIDEO}$: Formulas Provide Assembly Instructions}
}
\begin{explanation} \textbf{\textcolor{red!75!green}{Video: Formulas Provide Assembly Instructions}}
...
\pdfOnly{
\textbf{$\mathcal{VIDEO}$: Formulas are Templates or Algorithms}
}
\begin{explanation} \textbf{\textcolor{red!75!green}{Video: Formulas are Templates or Algorithms}}
...
When defining a function, you must give the pairs.  Here, the formula is serving as the description of the pairs. The formula describes how to build any and all pairs of the function.

$P(k)$ represents the function's value at $k$.  $k$ is representing domain values. $3k - 2$ is the expression involving the domain number, which calculates the range partner. 
...
\begin{procedure} \textbf{\textcolor{purple!30!blue}{Evaluation}}   
...
\pdfOnly{
\textbf{$\mathcal{VIDEO}$: The Operator's Manual}
}

\begin{explanation} \textbf{\textcolor{red!75!green}{Video: The Operator's Manual}}
...
\begin{procedure}   \textbf{\textcolor{purple!30!blue}{Replacement}}    
...
\end{procedure}
\textbf{Note:} ``DNE'' or ``dne'' stands for Does Not Exist. 



\begin{warning}  \textbf{\textcolor{red!70!darkgray}{Formula $\ne$ Function}}   
A formula is not a function.  A function is a package containing three sets, following one rule.  A formula describes the connection between the domain and range numbers. A formula is not used for any number. A formula is only used with the domain numbers.
...
\begin{procedure}   \textbf{\textcolor{purple!30!blue}{Substitution}}      
...
A formula describes the connection between domain numbers and their range partners (function values).  Given a domain number, substitution, and then direct calculation gives the function value at that domain number.  We can also go the other way.  Given a range value or function value, we can find the domain numbers connected to it.
...
\begin{remark}  \textbf{\textcolor{yellow!35!black}{The Rule}} 
...
\begin{remark}  \textbf{\textcolor{yellow!35!black}{Language}} 
...
\pdfOnly{
\textbf{$\mathcal{VIDEO}$: Evaluating Functions (Image)}
}
\begin{explanation} \textbf{\textcolor{red!75!green}{Video: Evaluating Functions (Image)}}
...
\pdfOnly{
\textbf{$\mathcal{VIDEO}$: Pre-Images}
}
\begin{explanation} \textbf{\textcolor{red!75!green}{Video: Pre-Images}}
...
\begin{warning} \textbf{\textcolor{red!70!darkgray}{Shorthand Notation}}  
