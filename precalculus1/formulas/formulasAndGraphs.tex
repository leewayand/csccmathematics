...
Linear functions have a constant growth rate or rate of change.  Whenever the domain changes by a given amount, then the function changes by a constant amount. \\

Linear functions have a constant growth rate while \textbf{exponential functions} have a constant percentage growth rate.  Whenever the domain changes by a given amount, then the function changes by a constant percentage. A constant percentage growth rate results in a formula of the form $E(t) = a \cdot r^t$, where $a$ is a nonzero real number and $0 < r$.
...
Whenever $t$ changes by $1$, $E$ grows by $25\%$, because the function value is multiplied by another $1.25 = 1 + 0.25$.
...
\textbf{Note:} If we change $r$ to be $0 < r < 1$, the exponential function, $E(t) = a \cdot r^t$, becomes a decreasing function and its graph is reflected horizontally.
...
In particular, we have the \textit{Zero Product Property}.  This states that the only way a product of two real numbers is zero is if one of the numbers is $0$.
