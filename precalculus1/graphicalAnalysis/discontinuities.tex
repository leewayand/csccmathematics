...
The graphs we have seen so far clearly show all kinds of breaks.  The function represented by the graph below is exhibiting some drastic behavior around some individual domain numbers.  Let's examine some of these breaks and classify them into a few types.
...
Of course, this more complicated as we encounter more function.  But this is a good start. \\
...
The intervals in the domain of $h$ are separated by unused intervals of real numbers.
...
For the second condition, let $d = 1$. Any open interval of real numbers containing $c = 4$ must also contain an interval of the form $(4-\epsilon, 4+\epsilon)$, where $0 < \epsilon < 0.1$ is small enough.  Such an interval would ALWAYS contain the number $e = 4-\frac{\epsilon}{2}$.
...
\item $8$ is a discontinuity of $g(k)$.  First $8$ is in the domain of $g$. For the second condition, let $d = 1$. Any open interval of real numbers containing $c = 8$ must also contain an interval of the form $(8-\epsilon, 8+\epsilon)$, where $0 < \epsilon < 0.1$ is small enough.   Such an interval would ALWAYS contain the number $e = 8-\frac{\epsilon}{2}$.
...
\item $-6$ is a discontinuity of $T(y)$.  $-6$ is in the domain of $T$. Let $d=0.5$. For the second condition, let $d = 0.5$. Any open interval of real numbers containing $c = -6$ must also contain an interval of the form $(-6-\epsilon, -6+\epsilon)$, where $0 < \epsilon < 0.1$ is small enough.   Such an interval would ALWAYS contain the number $e = -6+\frac{\epsilon}{2}$.
...
\item $-5$ is a discontinuity of $f(x)$.  For the second condition, let $d = 0.5$. Any open interval of real numbers containing $c = -5$ must also contain an interval of the form $(-5-\epsilon, -5+\epsilon)$, where $0 < \epsilon < 0.000001$ is small enough.    Such an interval would ALWAYS contain the number $e = -5-\frac{\epsilon}{2}$.
...
$\blacktriangleright$ \textbf{Idea:}  $c$ is not in the domina, however, EVERY open interval containing $c$ does contain a domain number. \\


Perhaps the function behaves weirdly around the singularity or not. \\
