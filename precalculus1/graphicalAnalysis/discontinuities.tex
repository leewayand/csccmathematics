...
\item \textbf{\textcolor{blue!75!black}{Asymptotic:}} The break at $x=3$ is not subtle at all.  One side is heading up to infinty and the other side is heading down to negative infinity.  You can't get a bigger break than that. Since this type of unbounded behavior is due to the asymptote. Hence, the break is called an \textit{asymptotic} break. \\
...
In all three cases, the graph experiences a sudden vertical movement.  This movement does not seem to follow the pattern leading up to the sudden change.
...
Of course, all of this gets more complicated as we encounter more function.  But this is a good start. \\
...
The intervals in the domain of $h$ are separated by unused intervals of real numbers.  There is no abrupt jump.  There is just an end and then a new beginning further along.
...
$\blacktriangleright$ Endpoints make an exact definition of discontinuity messy, because we want to say there are numbrs on both sides of the discontinuity.
...
\begin{tikzpicture}
     \begin{axis}[
        ...
        \addplot [draw=penColor, very thick, smooth, domain=(-1:3)] {sin(deg(2*x))};
        \addplot [draw=penColor, very thick, smooth, domain=(4:8)] {-2*x+10};
        ...
        \addplot[color=penColor,fill=penColor,only marks,mark=*] coordinates{(-1,-0.909) (-8,4) (-2,-6)};
        \addplot[color=penColor,fill=white,only marks,mark=*] coordinates{(3,-0.279)};
...
For the second condition, let $d = 1$. Any open interval of real numbers containing $c = 4$ must also contain an interval of the form $(4-\epsilon, 4+\epsilon)$, where $0 < \epsilon < 0.1$.  Such an interval would ALWAYS contain the number $e = 4-\frac{\epsilon}{2}$.
...
\item $8$ is a discontinuity of $g(k)$.  First $8$ is in the domain of $g$. For the second condition, let $d = 1$. Any open interval of real numbers containing $c = 8$ must also contain an interval of the form $(8-\epsilon, 8+\epsilon)$, where $0 < \epsilon < 0.1$.   Such an interval would ALWAYS contain the number $e = 8-\frac{\epsilon}{2}$.
...
\item $-6$ is a discontinuity of $T(y)$.  $-6$ is in the domain of $T$.  For the second condition, let $d = 0.5$. Any open interval of real numbers containing $c = -6$ must also contain an interval of the form $(-6-\epsilon, -6+\epsilon)$, where $0 < \epsilon < 0.1$.   Such an interval would ALWAYS contain the number $e = -6+\frac{\epsilon}{2}$.
...
\item $-5$ is a discontinuity of $f(x)$.  For the second condition, let $d = 0.5$. Any open interval of real numbers containing $c = -5$ must also contain an interval of the form $(-5-\epsilon, -5+\epsilon)$, where $0 < \epsilon < 0.000001$.    Such an interval would ALWAYS contain the number $e = -5-\frac{\epsilon}{2}$.
...
A real number, $c$, is said to be a \textbf{singularity} of the function $f$, if $c$ is not a member of the domain of $f$, however, the domain surrounds $c$.  \\



$\blacktriangleright$ \textbf{Idea:}  $c$ is not in the domain, however, EVERY open interval containing $c$ does contain a domain number and the function is behaving weird there. \\

It turns our "weird" includes a lot of behavior.  It will take a while to state a rigorous definition of singularity.  In the mean time, we will keep noting weird behavior and building our feeling or weird.
...
If $S$ is a subset of the domain and $f$ is continuous at every number in $S$, then $f$ is said to be continuous on the set.
