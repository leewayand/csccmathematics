...
We can easily define a function via a graph.  However, it is not so easy to decipher exact information from a graph, especially when the curve is drawn by hand.  Therefore, in addition to the points on the curve, we include other symbols to help people read the graph and understand the function.  These are communication symbols.  They help the reader understand what is drawn and what is not drawn.  This is the beginning of our attention to \textbf{rigor}.



\section{Rigor}

$\blacktriangleright$ \textbf{What is Rigor in a Mathematics Classroom?} \\


\begin{quote}
Rigor is when students use mathematical language to communicate effectively and to describe their reasoning with clarity and precision.  Students make understood "How do we know we have accounted for everything?"
\end{quote}



Rigor is one of the main threads in this course along with function structure.  It will take a lot of communication back and forth to learn how to present rigorous explanations. \\


We have already commented that graphs are inherently inaccurate, so they might seem an odd place to introduce rigor.  On the other hand, we have noted that graphs are communcation tools. This makes them an easy entrance to the ideas of rigor.  We clarify our graphical communication by the use of auxillary graph symbols. Their presence and absence communicate information about the functi
...
Points are dimensionless objects.  They are too small to actually see.  When we draw a graph, the thinkness of the pencil or pen is already an exaggeration of the underlying points and makes the endpoints a mystery.  How would you tell if the very last point of a curve was included or not? Therefore, we exaggerate even more for the endpoints, so that the reader immediately understands whether or not they are plotted.

We use big solid dots to illustrate an endpoint is a part of the drawing. We use big hollow dots when the actual last point is not included. 
...
Here, the arrows tell the reader that the curve continues its current linear pattern indefinitely to the right.  The domain is $[-3, \infty)$.
...
Arrows at the ends of curves signal that the curve continues with its current pattern. However, this may not not be enough.  Many functions have curves that continue moving up and to the right, however, their movement has a boundary. We use dashed lines to indicate that a curve is approaching this line pattern.
...
Horizontal asymptotes give similar information.  The function graphed above gets closer to the value of $2$ as the values of the domain get bigger.   Unlike with vertical asymptotes, the graph of a function can oscillate back-and-forth across a horizontal asymptote.
...
The values of the function, $f(x)$, get closer (or approach) $2$ as we move through the domain, heading to infinity.  Of course, we have algebraic notation to communicate this thought.
...
These graphical enhancements are not optional.  They are a requirement of our desire to be rigorous.
...
\begin{itemize}
\item If there is no horizontal asymptote, then we should not read that the graph is becoming horizontal.
\item If there are no ellipses, then we should not assume the pattern repeats.
\end{itemize}



The auxillary pieces drawn on a graph give us information and their absence gives us information.
