...
We have a core function called a power function. The Elementary Functions version of the power function includes a leading coefficient.
...
\begin{enumerate}
\item When $p$ is even, the behavior around the singularity is the same.  Depending on the sign of the coeficient, $k$, the function is unbounded positively or negatively for domain numbers close to $0$. 
...
In either case, the end-behavior is to be closer to $0$ further out in the domain.
...
For power functions with a negative power, the function values are smaller for larger domain numbers.
...
The discussion above does not say $x^p =  \infty$.  That is impossible.

The discussion above does not say $\infty^p =  \infty$.  That is impossible.


The discussion above does not say anything about reaching $\infty$ in the domain or in the range. That is impossible, since $\infty$ is not a real number and is not in the domain or in the range.

$\infty$ is our shorthand for ``becomes unbounded''.


$\lim\limits_{x \to \infty} k \, x^p =  \infty$ means that as $x$ becomes unbounded positively, so does $k \, x^p$.
...
We would like a precise and proper explanation of $\lim\limits_{x \to \infty} k \, x^p = 0$, when $p$ is a negative integer. 
...
For the limit to equal $0$, $f$ must ``eventually'' be smaller than any real number, and stay smaller. 
...
Given any small $\epsilon > 0$, let $M = \frac{| k |}{\epsilon}$, which is a positive real number.
...
Given any small positive size, called $\epsilon$, eventually $| f(x) |$ is eventually smaller than $\epsilon$ and stays smaller. 



The ``eventually'' part is $M$, a place far out in the real numbers. 

The ``stays there'' part is, for any $x > M$, all further places.
...
For a power function, $f(x) = k \, x^p$, with a negative power we have seen that there is a singularity at $0$, where the function value is unbounded and tends to infinity or negative infinity.  This time we want our limit notation to algebraically specify the direction we are approaching, which is $0$.  
...
The discussion above does not say $x =  0$.  That is impossible. When $p<0$, then the domain does not include $0$.



$\lim\limits_{x \to 0^+} k \, x^p =  \infty$ means that $x^p$ becomes unbounded positively as $x$ becomes closer to $0$.
...
Polynomial functions are constructed from special power functions.
...
We engage in two very different investigations involving polynomials.  One is an algebraic investigation and the other is an analytic investigation - our investigation.  Polynomials are functions for us and we are interested in analyzing them as functions.  This means, among other features, we are interested in their zeros.  For this, and other reasons, we prefer to write polynomials in factored form.
