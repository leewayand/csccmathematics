...
Logarithmic functions are functions that can be represented by formulas of the form
...
\begin{itemize}
\item Let $CL(t) = \log_r(t)$ be a core logarithmic function.
...
\[
 A \log_r(B \, x + C) + D = (L_{out} \circ CL \circ L_{in})(x)
\]
...
\begin{tikzpicture} 
  \begin{axis}[
          ...
          \addplot [line width=2, penColor, smooth,samples=200,domain=(-5:4.98),<->] {-3*ln((5-x)/ln(2)+1};
...
\begin{itemize}
\item Let $CL(t) = \log_2(t)$, is an increasing Core logarithmic function.
...
\[
 L(x) = -3 \log_2(5-x)+1 = (L_{out} \circ CL \circ L_{in})(x)
\]
...
Just like with exponential functions, it is nice to have a mental model in your head of a basic logarithmic function and its characteristics. 
...
\textbf{Zeros:} Every logarithmic function has exatly one zero.
...
\textbf{Global Maximum and Minimum:} Logarithmic functions do not have a global maximum or minium.
