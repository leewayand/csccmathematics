...
\begin{itemize}
\item Let $|t|$ be the Core absolute value function.
...
\[
 A \, |B \, x + C| + D = L_{out}(v) \circ |t| \circ L_{in}(x)
\]
...
Either $t = \frac{1}{2} \left( -\frac{5}{3}-4 \right)$ or $t = \frac{1}{2} \eft(\frac{5}{3} - 4 \right)$
...
\begin{itemize}
\item Let $|x|$ be the Core absolute value function.
...
\textbf{Global Maximum and Minimum:}  Absolute value functions dhave either a global maximum or a global minimum.
