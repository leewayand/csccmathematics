...
$\blacktriangleright$  When the power, $p$, is a positive integer, then power functions have two types of behavior.
...
$\blacktriangleright$  When the power, $p$, is a negative integer, then we have two types of behavior around the singularity at $0$. 

\begin{enumerate}
\item When $p$ is even the behavior around the singularity is the same - both up or down on the graph depending on the sign of the coeeficient, $k$.
...
For pa ower function with a negative power, the function values become smaller and smaller as the domain tends to positive or negative infinity.
...
Write $(y - 3)(2y + 1)$ in standard form, i.e. as a sum.
...
In the example above, $g(t)$ has a minimum value.
...
In the example above, $g(t)$ has a maximum value.
...
The end behavior of a polynomial function is the same as the leading term (a power function), which depend on the sign of the leading coefficient.
