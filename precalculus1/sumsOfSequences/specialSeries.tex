...
\begin{example}
  Evaluate the sum
  \[
  \sum\limits_{k=1}^\infty\left(\frac{1}{k}-\frac{1}{k+1}\right).
  \]
  \begin{explanation}
...
We can conclude that $s_n =\answer[given]{\ln (n+1)}$ and analyze $\lim\limits_{n \to \infty} s_n$.  

Since $\lim\limits_{n\to\infty}s_n=\answer[given]{\infty}$, $\sum\limits_{k=1}^\infty \ln\left(\frac{k+1}{k}\right)$ diverges.
\end{explanation}
\end{example}
