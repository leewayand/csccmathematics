...
Suppose a car is travelling at $5 \, mph = \frac{5 \, miles}{1 \, hour}$. This meanns every hour it moves another $5 miles$.
...
Our rate is a constant function: $r(t) = 5$. \\
...
Simce time is measured by the horizontal axis. our $distance = rate \times time$ calculation can be represented visually as the area under the rate curve.
...
The area of the rectnagle represents the distance travelled. \\
...
Let $M(t) = $ the accumulated miles travelled after $t \, hours$.

Graphically, this is the area of the rectangle below the graph.  The formula would be $M(t) = 5 t$.
...
$M(t)$ is the accumulated distance travelled in $t \, hours$, which makes $r(t)$ its derivative.
...
$L(k)$ is piecewise linear, which means we can calculate area.
...
This is the area under $y = L(x)$, so it is the approximate arae under $y = p(k) = -k^2 + 7$.
