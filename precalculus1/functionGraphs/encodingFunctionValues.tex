...
\end{tikzpicture}
\end{image}
Horizontal and vertical mdistances.

From the origin, we measure horizontally a distance of $d$.  Then we measure a vertical distance of $f(d)$. Plot a dot.



\begin{example}

SUppose we have a funciton $f$ and we know that $f(3)=2$. This information is encoded with a dot at $(3, 2)$.

\begin{image}
\begin{tikzpicture}
\begin{axis}[
            domain=0:4, ymax=5, xmax=5, ymin=-5, xmin=-5,
            axis lines =center, xlabel=$range$, ylabel=$codomain$,
            every axis y label/.style={at=(current axis.above origin),anchor=south},
            every axis x label/.style={at=(current axis.right of origin),anchor=west},
            axis on top
          ]
...
Graphs of functions are just a collection of a bunch of dots (a curve).  Each dot represents a pair in the function.  Each dot has two coordinates.  The horizontal (first/left) coordinate is the domain number and the vertical (second/right) coordinate is the function value at that domain value.
...
\begin{question}
The domain of the function, $p$, only contains positive numbers.  Which quadrants cannot not contain points on the graph of $p$?
...
The answer is yes.  It says all of those, because the dot is too big.  Our graph should consist of points, but points are dimensionless.  We wouldn't be able to see them.  So, we draw dots that we can see and accept the inaccuracy of the graph.
...
Below is the graph of $h(f)$. We can identify its domain, by collecting all of the first coordinates of all of the dots.   We can identify its range, by collecting all of the second coordinates of all of the dots.  
