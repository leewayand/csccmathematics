...
\item There is a real number, which is not a domain number. So, the function has no value at this real number. When  domain numbers are close to this real number, their function values may or may not be close to each other.
\end{itemize}
...
\textbf{Technically:}  $(a,a)$ would represent the empty set, which is techically an open set. 
...
\begin{explanation} \textbf{\textcolor{red!75!green}{No Greatest or Least Number}} 

Suppose $(a,b)$ does contain a greatest number. Then the greatest number would be some number, $c \in (a,b)$.  Why can't that happen?  
...
If we have the greatest number in the interval, $c$, then we can find a greater number also in the interval. That is not logical. Therefore, $c$ cannot be the greatest number in the interval.  As a result, there is no greatest number in an open interval. 


\textbf{Note:} This is called a proof by contradiction.
