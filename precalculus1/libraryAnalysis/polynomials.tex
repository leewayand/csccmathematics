...
\begin{definition}
\textbf{Polynomials} are functions that can be described as sums of power functions
...
Polynomials smoothly alternate between increasing and decreasing, which switched at global and local maximums and minimums.
...
The extreme values of the function occur at numbers in the domain called \textbf{critical numbers}.  Critical numbers play an important role in function analysis.  


We'll start with critical numbers pointing out horizontal tangent lines in the graph.
...
$f$ has two critical numbers: $-2$ and $1.75$. There are two places where the graph has a horizontal tangent line.
...
\textbf{Note:} $-2$ is a critical number, because the graph is flat there.  However, $f$ the sign of the rate of change does not switch there, since $f$ does not have an extreme value there.
...
\item If $(z_0,0)$ is an intercept of the graph of the polynomial $p(x)$, means $(z,0)$ is on the graph.
