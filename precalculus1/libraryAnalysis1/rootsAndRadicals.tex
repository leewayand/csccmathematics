...
\begin{tikzpicture} 
  \begin{axis}[
            ...
            axis on top
 			...
 			\addplot[color=penColor,fill=penColor,only marks,mark=*] coordinates{(2.67,-4)};
...
The cube root function is the most popular Core root function
...
\begin{tikzpicture} 
  \begin{axis}[
            ...
            %\addplot [line width=2, penColor, smooth, domain=(-9:0),<->] {(x-1)/((x+3)*(x-4))};
            \addplot [line width=2, penColor, smooth, samples=200,domain=(-9:9),<->] {(x)^(0.333))};
...
Let $CR(x) = \sqrt[3]{k}$ be the Core Cube Root function.
