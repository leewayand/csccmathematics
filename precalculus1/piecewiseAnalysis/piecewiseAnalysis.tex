...
\begin{itemize}
  ...
  \item if $4 < d < 6$, then use the formula $V(h) = -2h - 3$
\end{itemize}
...
The left line would have an intercept, except the domain does not include it.  The middle parabola has a left intercept, except the domain does not include it. 

From the graph, we might estimate these zeros to be around $3$ and $4.5$.  What are the exact values?
...
\item \textbf{$-(h+3)(h-3)$} is a quadratic on $(-2, 4]$. The vertex on the graph occurs at the midpoint of the zeros of the quadratic, which are $-3$ and $3$.  Therefore, $0$ is where the function switches behavior.  On $(-2, 0]$, $V$ is increasing.  On $[0,4]$, $V$ is decreasing.
\end{itemize}


\textbf{Technically:} $V$ is increasing on $[-2, 4]$, since the dot at $-2$ is lower than the left side of the included parabola.
...
Lastly, \\
  ...
  \item \textbf{$\frac{7h}{4} - 8$} is an increasing linear function on $[4, 6)$.  Its maximum would be encoded in the left endpoint, which is excluded. Its maximum would be encoded in the right endpoint, which is excluded. This piece is contributing no extreme values to $V$.
\end{itemize}
...
We also include domain numbers where the $iRoC_V$ does not exist.  That would be domain numbers where there is no corresponding tangent line.  That would include $-6$, $-2$, and $4$.
...
Algebraically, $V$ is continuous on $[-2, 4]$.  Therefore, $V$ assumes all of the values between $V(-2)$ and $V(4)$, which tells us algebraically, that the range is $[-7, -9]$. \\
