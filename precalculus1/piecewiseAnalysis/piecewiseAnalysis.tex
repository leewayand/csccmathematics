...
However, the graph suggests that the domain restrictions only permit two of them.  We can find the zeros for each component function and then see if they are members of the corresponding domain interval. 
...
However, the left piece of $V$ is restricted to the domain numbers inside $[-6,-2]$ and $-\frac{3}{2} \notin [-6,-2]$.
...
\begin{explanation} \textbf{\textcolor{red!75!green}{Discontinuity at $-2$}} 


Remember, a domain is a discontiuity if there is a gap in the range at that domain number.  We can see a gap in the graph of $V$ at $-2$.  We need to establish the gap algebraically.

Our plan is show that to the left of $-2$, $V$ is less than $2$ and to the right of $-2$, $V$ is greater than $\frac{9}{2}$. That will be our gap in the function values.  



Let's examine $V$ on the left side of $-2$.


Let $\epsilon > 0$ be any very small positive number.  We are considering domain numbers in the interval, $(-2 - \epsilon, -2 + \epsilon)$.


\begin{itemize}
\item Claim: On $(-2 - \epsilon, -2)$, the values of $V$ are near $1$. 


On $(-2 - \epsilon, -2) \subset [-6, -2]$, which tells us that $V(h) = -2h-3$.

$V$ is a linear function on $[-6, -2]$.

$V$ is decreasing on $[-6, -2]$, because its linear formula has a negative leading coeffcient.


Since, $V(h)$ is decreasing on $[-6, -2]$, we have  

\[ V(-6) \geq V(a) \geq V(b) \geq V(-2) \, \text{ for all } \, a < b \in [-6, -2] \]
...
So, we have $1 + 2\epsilon \geq V(a) \geq 1$ for $a \in (-2 - \epsilon, -2)$.

Since $\epsilon > 0$ is a very small positive number, we know that $1 + 2\epsilon < 2$.


For $a \in (-2 - \epsilon, -2)$, we have $1 \leq V(a) < 2$,



That tells us that to the left $-2$, $V$ is less than $2$.

Now for the other side. 

Now for the right side of $-2$.
...
\item On $(-2, -2 + \epsilon)$, the graph suggests that the values of $V$ are not near $2$.  They are around $7$. Let's verify this algebraically. 



On $(-2, -2 + \epsilon)$, $V(h) = 9 - h^2$. It is a quadratic function.  


Let's think for a moment.  If $V(h) = -(h + 3)(h - 3) = 9 - h^2$ was going to be near $2$, then we would need $h \approx -\sqrt{7}$.  

But, $-\sqrt{7} \notin (-2, -2 + \epsilon)$, since $-\sqrt{7} < -2$. Therefore, $V$ cannot get down to $2$. We just need to explain this rigorously. That means details.  
...
The critical number of $V$ is $\frac{-b}{2a} = \frac{0}{-2} = 0$.


$V$ is increasing on $(-2, -1]$, because its formula has a negative leading coeffcient and we are considering numbers less than the critical number $0$.
...
$V(h)$ is increasing on $(-2, -1]$, which means 



\[ V(a) \leq V(b) \leq V(-1) \, \text{ for all } \, a < b \in (-2, -1] \]
...
\[
5 -  \epsilon \geq 5 -  \frac{1}{2} = \frac{9}{2}
\]

That tells us that

\[ V(-2 + \epsilon) =  (1 + \epsilon) (5 - \epsilon) \geq \frac{9}{2} \]
...
\[
V(h) \geq 1 \cdot \frac{9}{2} = \frac{9}{2}
\]



On $(-2, -2 + \epsilon)$, all of the values of $V$ are greater than $\frac{9}{2}$.


We have $V(h) \geq  \frac{9}{2}$ on $(-2, -2 + \epsilon)$.
...
\textbf{\textcolor{red!70!darkgray}{$\blacktriangleright$}} On $(-2 - \epsilon, -2)$, $2 \geq V(h) \geq 1$. 

\textbf{\textcolor{red!70!darkgray}{$\blacktriangleright$}} On $(-2, -2 + \epsilon)$, we have $V(h) \geq  \frac{9}{2}$. 

The left side is always less than $2$.

The right side is always greater than $\frac{9}{2}$.


There is always a gap in the function values.


Following the definition of discontinuity, let's pick a distance of $1$.


No matter how small $\epsilon$ is, $(-2 - \epsilon, -2 + \epsilon)$ \textbf{\textcolor{red!70!darkgray}{ALWAYS}} contains a domain number, $a \in (-2, -2 + \epsilon)$, such that the distance between $V(a)$ and $V(-2)$ is greater than $1$.
...
We are trying to think in terms of types of numbers. 

We are trying to think of ``types'' as algebraic conditions or relationships. 


This is part of our new way of thinking.  We are trying to get away from specific numbers and, instead, think of \textbf{types} of numbers.  This is called algebra.

Our first encounter with this type of thinking is $\epsilon$ (epsilon).  It is representing a very small type of number, much smaller than the usual small numbers we have encountered in the past.  But, it is not any particular small number.  It is representing all of them. Kind of weird. 


But we need better language and notation to talk about discontinuities. 

It is coming. 

Limits will replace a lot of this kind of long-winded explanation. 
...
\begin{explanation} \textbf{\textcolor{red!75!green}{Discontinuity at $4$}}  


Let $\epsilon > 0$ be a small positive number.  Consider the interval, $(4 - \epsilon, 4 + \epsilon)$.

\begin{itemize}
\item On $(4 - \epsilon, 4]$, the values of $V$ are less than or equal to $-6$.
...
No matter how small $\epsilon$ is, $(4 - \epsilon, 4 + \epsilon)$ \textbf{\textcolor{red!70!darkgray}{ALWAYS}} contains (at least) one domain number, $a \in (4, 4 + \epsilon)$, such that the distance between $V(a)$ and $V(4)$ is greater than $1$.
...
Our algebraic notation to talk about function behavior around discontinuities and singularities is called \textbf{\textcolor{purple!85!blue}{limits}}.  Limits have already made an appearance helping us describe end-behavior, algebraically. They will slowly take over our thinking for Calculus.
\end{notation}
...
\item \textbf{$-(h+3)(h-3)$} is a quadratic on $(-2, 4]$. Here, $iRoC_V(h) = V'(h)= -2h$. The critical number occurs when $iRoC_V(h) = V'(h) = -2h = 0$, which happens when $h=0$. This agrees with the midpoint of the zeros, which are $-3$ and $3$.  Therefore, $0$ is where the function switches behavior.  

\begin{itemize}
\item On $(-2, 0)$, $iRoC_V(h) = V'(h) = -2h > 0$, therefore, $V$ is increasing.  
\item On $(0,4)$, $iRoC_V(h) = V'(h) = -2h < 0$, therefore, $V$ is decreasing.
\end{itemize}
...
And, we can look at the instantaneous rate of change. The middle piece of $V$ is a quadratic, therefore, the vertex form $-(h-0)^2 + 9$ gives us $iRoC_V(h) = V'(h) = -2(h-0) = -2h$.  This equals $0$ at $0$, which means $0$ is a critical number and a possible location of changing behavior.
...
Each of these global extrema are also a local extrema.  



In addition, we have a local minimum value of $1$ occuring at $-2$.  
...
\begin{explanation} \textbf{\textcolor{red!75!green}{Local Minimum}}  
...
This would also include domain numbers which are endpoints of intervals in the domain, like $-6$. Even though there exists a tangent line at $(-6, 9)$, $-6$ is automatically a critical number, since there are not two sides.
...
An extreme value can occur at domain numbers that correspond to points where a tangent line is vertical (and so has no slope - no derivative).  These would correspond to cusps in the graph. 
...
\textbf{\textcolor{red!70!darkgray}{Candidates for locations of extrema values are critical numbers, which are domain numbers where the derivative is $0$ or Does Not Exist.}}
