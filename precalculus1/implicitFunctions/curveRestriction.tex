...
Suppose we have two quanitities, $A$ and $B$ and in our situation, it is helpful to think of $A$ as dependent on $B$.  So, we would like to think in terms of $A(B)$ and use our knowledge of functions.
...
Pretend, we learn in our situation, that when $B$ is negative, then $A \leq 4$ and when $B \geq 0$, then $A > 4$ 
...
Since didn't have a formula for $y$, we are just stating the relationship via an equation with appropriate restrictions.  Functions defined this way, without an explicit formula, are said to be defined \textbf{implicitly}.
