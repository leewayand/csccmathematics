...
In our defining equation, $B(k) = T(k+3)$, $k+3$ is representing the domain values of $T$.  We also have $v$ representing the domain values of $T$.

\[ v = k+3   \]



$v=k+3$, so $v \in \left(\answer{-4},\answer{-1}\right]$ and on this interval $T(v) = 2v - 1$.

To get the formula for $B(k)$, we need to replace $v$ with $k+3$. 
...
$v=k+3$, so $v \in \left[\answer{1},\answer{7}\right)$ and on this interval $T(v) = -v + 3$.
