...
\item When $k \in (-7,-4]$, then $k+3 \in \left(\answer{-4},\answer{-1}\right]$.  
...
\item When $k \in [-2,4)$, then $k+3 \in \left[\answer{1},\answer{7}\right)$.  
...
Notice that four different variables, $v$, $y$, $k$, and $z$, were been used to describe the domain and range values of $T$ and $B$.  We are using separate names for the different types of values to keep everything clear. If we use the same variable to represent both domains, then we run into problems.
