...
Call the critical numbers of $f$ $a$, $b$, and $c$ with $a<b<c$.  Then
\begin{align*}
a &= \answer{-1}\\
b &= \answer{1}\\
c &= \answer{4}
...
The critical number $a$ corresponds to a \wordChoice{\choice[correct]{local max} \choice{local min} \choice{neither a local max nor a local min}}.

The critical number $b$ corresponds to a \wordChoice{\choice{local max} \choice[correct]{local min} \choice{neither a local max nor a local min}}.

The critical number $c$ corresponds to a \wordChoice{\choice[correct]{local max} \choice{local min} \choice{neither a local max nor a local min}}.
