...
We call the derivative of the derivative the \textbf{second
  derivative}, the derivative of the second derivative (the derivative of the derivative of the derivative) the
\textbf{third derivative}, and so on. We have special notation for
higher derivatives, check it out:
\begin{description}
\item[First derivative:] $\frac{d}{dx} f(x) = f'(x) = f^{(1)}(x)$.
\item[Second derivative:] $\frac{d}{dx^2} f(x) = f''(x) = f^{(2)}(x)$.
\item[Third derivative:] $\frac{d}{dx^3} f(x) = f'''(x) = f^{(3)}(x)$.
\end{description}
