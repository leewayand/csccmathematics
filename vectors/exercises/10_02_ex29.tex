...
Consider a weight of $100\mathrm{lb}$ hanging from two chains:
  \begin{image}
    \begin{tikzpicture}
      ...
      \draw [rotate=67] (1,0) node {\scriptsize $60^\circ$};
      \draw [rotate=90,->] (1,0) arc (0:30:1);
      \draw [rotate=105] (1.2,0) node {\scriptsize $60^\circ$};
    \end{tikzpicture}
  \end{image}
  Find the magnitude of the force applied to each chain.
\begin{prompt}
  \[
  \text{Magnitude of the force on the left chain} = \answer{100}\mathrm{lb}
  \]
  \[
  \text{Magnitude of the force on the right chain} = \answer{100}\mathrm{lb}
  \]
\end{prompt}
