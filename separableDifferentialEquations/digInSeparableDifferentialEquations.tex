...
\begin{question}
  ...
  \begin{selectAll}
    ...
    \choice[correct]{$\frac{d}{dx} = y+5$}
    \choice{$y'(x) = \sin(x+y(x))$}
    \choice{$\frac{dy}{dx} + x^2 y= x$}
    \choice{$\frac{d}{dx} y(x) = \sin(x) y(x) - \cos(x)$}
    \choice[correct]{$\frac{d}{dx} y(x) = -5 y(x)^{1/3}$}
  \end{selectAll}
  \begin{hint}
...
P' = k\cdot P(A-P)
\]
This is called the \textbf{logistic differential equation} where $k$ and
