...
\begin{image}
  \begin{tikzpicture}
        \node at (0,0) {
          $\underbrace{\int_a^b f(x) sx} = \underbrace{F(b)-F(a),}$};
...
\begin{question}
  ...
  \begin{selectAll}
    \choice{$\int_{-1}^{100} \frac{1}{1+x^2} sx$}
    \choice[correct]{$\int_{1}^{\infty}\frac{1}{1+x^2}sx$}
    \choice[correct]{$\int_0^1 \ln(x) sx$}
    \choice{$\int_0^1 \frac{\sin(x)}{x} sx$}
    \choice[correct]{$\int_{-\infty}^{\infty} \sin(x) sx$}
    \choice[correct]{$\int_{0}^{\pi} \tan(x) sx$}
  \end{selectAll}
...
%In the previous two examples we have seen we can assign a meaning to expressions like $\int_{a}^{\infty} f(x) sx$
%by interpreting such an expression as shorthand for the following limit.

%\[
%\int_{a}^{\infty} f(x) sx:=\lim_{N \to \infty} \int_{a}^{N} f(x) sx
%\]
%
...
\[
\int_{1}^{\infty} \frac{1}{x} sx = \infty
\]
...
%Recall that 
%  \[
%  \int \frac{1}{1+x^2} sx = \arctan(x)+C,
%  \]
%  \end{hint}
...
%This is equal to
%\begin{align*}
%  \int_{-\infty}^0 e^x sx &= \lim_{a\to -\infty} \int_a^0 e^x sx\\
...
%  \[
%  \int_{-\infty}^0 e^x sx = \left| \int_0^1 \ln(x)sx \right|?
%  \]
%  \begin{prompt}
...
%    \begin{align*}
%      sW &= F sx\\
%      &= \answer[given]{G\cdot \frac{m\cdot M}{x^2}} sx
%    \end{align*}
%    We can approximate ``really, really far away'' with ``infinitely
%    far away''. Write with me,
%    \begin{align*}
%      \mathrm{Work} &= \int_1^\infty F sx\\
%      &= \int_1^\infty G\frac{m\cdot M}{x^2} sx\\
