...
\[
V=\int_{x=a}^{x=b} \pi(R^2-r^2) dx \qquad \textrm{ or } \qquad V=\int_{y=c}^{y=d} \pi(R^2-r^2) dy
\]
...
\[V = \int_{x=a}^{x=b} \pi\left(R^2-r^2\right) dx \]
...
Thus, the volume integral to be evaluated is:
	...
	\[
	V= \int_{x=\answer[given]{0}}^{x=\answer[given]{2}} \answer[given]{\pi \left[\left(2+\sqrt{x}\right)^2-(1)^2\right]} dx
	\]
...
\[
V = \int_{x=0}^{x=1} 2 \pi x(2-x^2) dx
\]
...
\[
V=\int_{x=a}^{x=b} 2\pi \rho h dx \qquad \textrm{ or } \qquad V=\int_{y=c}^{y=d} 2 \pi \rho h dy
\]
