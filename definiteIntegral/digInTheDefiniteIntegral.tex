...
It follows that
 \[
\int_0^5 f(x) dx= \int_0^3 f(x) \d x+ \int_3^5 f(x) dx.
\]
...
\begin{example}
  ...
  \]
    ...
    and height $3$.  Hence
    \begin{align*}
    \int_0^6 |x-3| dx &= \int_0^3 \answer[given]{3-x} \d x + \int_3^6 \answer[given]{x-3} dx\\
...
\begin{align*}
  \int_0^9 |f(x)| dx = \int_0^1 |f(x) |\d x &+ \int_1^5 |f(x)| dx\\
  &+ \int_5^8 |f(x)| dx\\
  &+ \int_8^9 |f(x)| dx 
\end{align*}
