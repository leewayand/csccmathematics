...
\[
\int_0^{10}-x dx=\lim_{n\to\infty}\sum_{k=1}^n f(x_k^*)\Delta x=\lim_{n\to\infty}\sum_{k=1}^n -x_k^*\Delta x
\]
It does not matter what  type of a Riemann sum we use, so we choose a right Riemann sum.

\[
\int_0^{10}-x dx=\lim_{n\to\infty}\sum_{k=1}^n -x_k\Delta x
\]
...
\item $\int_a^bf(x) dx = -\int_b^a f(x) dx$
\item $\int_a^bk \cdot f(x) dx = k\cdot\int_a^bf(x) dx$
\item $\int_a^b \left(f(x)\pm g(x)\right) dx = \int_a^bf(x) dx \pm \int_a^bg(x) dx$
\end{enumerate}
\begin{explanation}
  ...
  \begin{image}
    \begin{tikzpicture}[
      ...
      \begin{axis}[
        ...
        \node at (axis cs:2.25,1) {\large$\int_a^c f(x) dx$};
        \node at (axis cs:4.75,1) {\large$\int_c^b f(x) dx$};
      \end{axis}
    \end{tikzpicture}
  \end{image}		
...
\begin{example}
  ...
  \]
    ...
    and height $3$.  Hence
    \begin{align*}
    \int_0^6 |x-3| dx &= \int_0^3 \answer[given]{3-x} dx + \int_3^6 \answer[given]{x-3} dx\\
...
\begin{definition}
  A function $f$ is an \textbf{odd} function if
  \[
  f(-x) = -f(x),
  \]
  and a function $g$ is an \etxtbf{even} function if
  \[
  g(-x) = g(x).
  \]
\end{definition}
...
\begin{question}
  ...
  \[
  \int_{-2}^2 f(x) dx \begin{prompt}=\answer{0}\end{prompt}
  \]
  \begin{hint}
...
\begin{question}
  ...
  \]
  \begin{selectAll}
    \choice{$\int_{4}^{2} f(x) dx$}
    \choice{$\int_{-4}^{-2} f(x) dx$}
    \choice[correct]{$\int_{-2}^{-4} f(x) dx$}
    \choice[correct]{$\int_{-2}^{4} f(x) dx$}
    \choice{$\int_{4}^{-2} f(x) dx$}
    \choice[correct]{$\int_{2}^{-4} f(x) dx$}
    \choice{$\int_{-4}^{2} f(x) dx$}
    \choice[correct]{$-\int_{-4}^{2} f(x) dx$}
    \choice[correct]{$-\int_{-4}^{-2} f(x) dx$}
  \end{selectAll}
\end{question}
...
\begin{align*}
  \int_0^9 |f(x)| dx = \int_0^1 |f(x) | dx &+ \int_1^5 |f(x)| dx\\
  &+ \int_5^8 |f(x)| dx\\
  &+ \int_8^9 |f(x)| dx 
\end{align*}
Rewriting the absolute value signs:
\begin{align*}
  = \int_0^1 (-f(x)) dx &+ \int_1^5 f(x) dx\\
  &+ \int_5^8 (-f(x)) dx\\
  &+ \int_8^9 f(x) dx
\end{align*}
...
\begin{question}
  ...
  \begin{feedback}
    ...
    \[
    \int_a^b |f(x)| dx = 1 \qquad\text{but}\qquad \left|\int_a^b
    f(x) dx\right| = 0.
    \]
  \end{feedback}
  \end{prompt}
