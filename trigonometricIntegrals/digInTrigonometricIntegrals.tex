...
\begin{example}
  ...
  \begin{explanation}
    ...
    \begin{center}%% used center instead of image since there is no reason for a LARGE integral in the text
      \begin{tikzpicture}
        \node at (0,0) {
          $\int \underbrace{(1-\cos^2(x))^2 \cos^2(x)} \underbrace{\sin(x) dx}$
        };
        \node at (-.5,-.7) {\small{polynomial in $u$}};
        \node at (1.8,-.7) {\small{``$du$''}};
      \end{tikzpicture}
    \end{center}
...
\begin{example}
  ...
  \begin{explanation}
    ...
    \begin{center}%% used center instead of image since there is no reason for a LARGE integral in the text
      \begin{tikzpicture}
        \node at (0,0) {
          $\int \underbrace{(1-\cos^2(x))^2} \underbrace{\sin(x) dx}$
        };
        \node at (-.5,-.7) {\small{polynomial in $u$}};
        \node at (1.2,-.7) {\small{``$du$''}};
      \end{tikzpicture}
    \end{center}
...
\begin{example}
  ...
  \begin{explanation}
    ...
    \begin{center}%% used center instead of image since there is no reason for a LARGE integral in the text
      \begin{tikzpicture}
        \node at (0,0) {
          $\int \underbrace{\tan^3(x)(1+\tan^2(x))} \underbrace{\sec^2(x) dx}$
        };
        \node at (-.5,-.7) {\small{polynomial in $u$}};
        \node at (1.8,-.7) {\small{``$du$''}};
      \end{tikzpicture}
...
%\]
%
%Doing the substitution $u=\tan(x)$ and $du=\sec^{2}(x)$, our integral becomes 
%
%\[
...
\] 
    ...
    \begin{center}%% used center instead of image since there is no reason for a LARGE integral in the text
      \begin{tikzpicture}
        \node at (0,0) {
          $\int \underbrace{(\sec^2(x)-1)\sec^4(x)} \underbrace{\sec(x)\tan(x) dx}$
        };
...
%
%\[
%\int \tan^{2k}(x) \sec^{m-1}(x) \sec(x)\tan(x) dx
%\]
%
%Now we can rewrite the $\tan^{2k}(x)$ as the power of a square:
%
%\[
%\int (\tan^{2}(x))^{k} \sec^{m-1}(x) \sec(x)\tan(x) dx
%\]
%
%We use the identity $\tan^{2}(x)=\sec^{2}(x)-1$ to express the $\tan^{2}(x)$ factor
%
%\[
%\int (\sec^{2}(x)-1)^{k} \sec^{m-1}(x) \sec(x)\tan(x) dx
%\]
%
%Now we do the substitution $u=\sec(x)$ and $du=\sec(x)\tan(x)$. 
%
%\[
%\int (u^{2}-1)^{k} u^{m-1} du
%\]
%
...
%    \begin{align*}
%      \int &\cot^3(x) \csc^4(x) dx \\
%    &= \int\cot^3(x) \csc^2(x) \csc^2(x) dx
%    \end{align*}
...
%      \begin{tikzpicture}
%        \node at (0,0) {
%          $\int \underbrace{(\cot^3(x)(\cot^2(x)+1)} \underbrace{\csc^2(x) dx}$
%        };
%        \node at (-.5,-.7) {\small{polynomial in $u$}};
%        \node at (1.8,-.7) {\small{``$du$''}};
%      \end{tikzpicture}
...
%    \begin{align*}
%      u &= \cot(x)\\
%      du &=\answer[given]{-\csc^2(x)} dx
%    \end{align*}
%    yields
%    \[
%    \int \answer[given]{-u^3(u^2+1)} du
%    \]
%    This is a polynomial in $u$, and so we can easily finish the
%    integration by just expanding the polynomial out and integrating
...
%    \begin{align*}
%      \int &\cot^3(x) \csc^5(x) dx \\
%    &= \int\cot^2(x) \csc^4(x) \csc(x)\cot(x) dx
%    \end{align*}
...
%      \begin{tikzpicture}
%        \node at (0,0) {
%          $\int \underbrace{(\csc^2(x)-1)\csc^4(x)} \underbrace{\csc(x)\cot(x) dx}$
%        };
%        \node at (-1,-.7) {\small{polynomial in $u$}};
%        \node at (1.8,-.7) {\small{``$du$''}};
%      \end{tikzpicture}
...
%    \begin{align*}
%      u &= \csc(x)\\
%      du &=\answer[given]{-\csc(x)\cot(x)} dx
%    \end{align*}
%    yields
%    \[
%    \int \answer[given]{-(u^2-1)u^4} du
...
\begin{explanation}
 There are no powers of $\tan(x)$ immediately available here, so letting $u=\tan(x)$ may seem like a bad idea at first.  However, by saving a copy of $\sec^2(x)$ for the differential, we have $2$ powers of $\sec(x)$ left, so let's give it a try:
    ...
    \begin{center}%% used center instead of image since there is no reason for a LARGE integral in the text
      \begin{tikzpicture}
        \node at (0,0) {
          $\int \sec^{4}(x) dx  =  \int  \underbrace{(\tan^2(x)+1)} \underbrace{(\sec^2(x) dx)} = \int u^2+1 du$
        };
        \node at (-1,-.7) {\small{polynomial in $u$}};
        \node at (1.2,-.7) {\small{``$du$''}};
      \end{tikzpicture}
...
%The example of $\int \sec^{3}(x) dx$ as well as $\int \sec^{n}(x) dx$ gives us one possible strategy to handle integrals that involve powers of $\tan(x)$ and $\sec(x)$ 
...
%\]
%
%Here the power of $\sin(x)$ is odd so we can separate off a $\sin(x)$ to be used for $du$ and convert
