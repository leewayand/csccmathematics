...
Polynomials may seem like easy functions to analyze, but they are very resistant to algebra.


To analyze polynomials, we almost always need a derivative.  



If the polynomial is special or we just happen to know unusual information about the polynomial, then we might be able to analyze it fully. 



To have any chance at analyzing a random polynomial, our first thought is factoring. What are the zeros? 
...
Using the Rational Roots Theorem, we have some guesses on roots.  The smaller factors of $24$ are $\pm4$, $\pm3$, $\pm2$, $\pm1$.  
...
With this factorization, we are ready to begin our analysis. 
...
$P$ is not a constant polynomial, which means is is unbounded. 
...
Since $P$ is continuous with no singularities, the end-behavior tells us that the range is $(-\infty, \infty)$.
