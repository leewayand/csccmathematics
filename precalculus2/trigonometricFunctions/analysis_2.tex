...
Along with $\cos(\theta)$, these three component functions give
...
This includes any value of $\cos(\theta)$. That measn we can use any value of $\cos(\theta)$. That means we can use the entire domain of $\cos(\theta)$, which is $(-\infty, \infty)$.  
...
We need the values of $y$ where $In(y) = \pi - 4y = \frac{2\pi}{3}$ and $In(y) = \pi - 4y = \frac{4\pi}{3}$

$y = \frac{\pi}{12}$  and   $y = -\frac{\pi}{12}$.

They are both in the principal interval for $H$. 
...
$In(y) = \pi - 4y$ is a linear function with a negative leading coefficient, so it is a decreasing function.
...
Since $H$ is continuous on $(-\infty, \infty)$ with a global maximum of $3$ and a global minimum of $-1$, the range is $[-1, 3]$.  
