...
For the graph, this means intercepts at the whole-$\pi$'s and vertical asymptotes at the half-$\pi$'s.
...
Along with $\tan(\theta)$, these three component functions give
...
\[
\lim\limits_{y \to -3\pi^+} In(y) = \lim\limits_{y \to -3\pi^+} \frac{y}{2} + \pi = \left( \frac {5\pi}{2} \right)^+
\]
...
\[
\lim\limits_{x \to -\infty} Out(x) = \lim\limits_{x \to -\infty} (-x) = \infty
\]
...
$\blacktriangleright$ Additionally, a $\pi$ has been added to the inside.  This would indicate a horizontal shift left in the graph, which has happened.
...
It either has no end-behavior or the end-behavior is that it is periodic and oscillates.
...
\[
K(t) = decreasing \circ increasing \circ increasing = decreasing
\]
