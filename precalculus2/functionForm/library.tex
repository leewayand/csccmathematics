...
First, we have core exponential functions, which have a formula like, $f(x) = r^x$ with $r > 0$, $r \ne 0$. 

This gives us a basic feeling for the behavior of exponential functions.



\textbf{For Instance,} $f(x) = e^x$:
...
Secondly, we have four basic forms of exponential functions that illustrate the basic variations of the basic behavior.
...
Vertical shifts are not allowed for exponential functions.  We call such functions \textit{shifted exponential functions}.  
...
Shifted exponential functions do not have a constant \textit{percentage} growth rate. That makes them different from exponential functions.

However, it is often just easier to analyze the exponential term of the shifted exponential function and think of adding/subtracting a constant to it.



\textbf{For Instance,} $f(x) = e^x - 3$:
...
\begin{tikzpicture}
  \begin{axis}[
      ...
      \addplot [line width=2, penColor, smooth,samples=200,domain=(-9:2),<->] {2.7^x-3};
      \addplot [line width=1, black, dashed,samples=200,domain=(-9:9),<->] {-3};
...
Shifting and stretching is again algebraically described by compositions with linear functions.
...
The natural domain of a shifted exponential function is all real numbers.  However, we can always state a restricted domain. 
...
First, we have core logarithmic functions, which have a formula like, $f(x) = \log_r(x)$ with $r > 0$. 

This gives us a basic feeling for the behavior of exponential functions.



\textbf{For Instance,} $f(x) = \ln(x)$:
...
\begin{tikzpicture}
  \begin{axis}[
      ...
      \addplot [line width=2, penColor, smooth,samples=200,domain=(0.02:9),<->] {ln(x)};
...
Secondly, we have four basic forms of the logarithmic function that illustrate the basic variations of the basic behavior.
