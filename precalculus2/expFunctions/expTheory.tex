...
When $r$ \wordChoice{\choice{$<$} \choice[correct]{$>$}} $1$ we have a growing function.
...
Graphs of exponential functions have one strategic point, whcih occurs when the exponent equals zero. For the basic exponential function, this point is $\left( \answer{0}, a \right)$.
...
When $r$ \wordChoice{\choice[correct]{$<$} \choice{$>$}} $1$, the horizontal axis is still a horizontal asymptote, just in the other direction. The function now decays.
...
Our function's exponent is zero, $3-t=0$ when $t=3$. Our one anchor point is shifted over to $3$.  Multipying by $-2$, means the dot is $2$ away(below) from the horizontal asymptote, which is now $y=4$.  Our anchor point is $(3, 2)$.
