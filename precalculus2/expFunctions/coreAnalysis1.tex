...
\textbf{Global Maximum and Minimum:} Since a constant function has only one value, that value is the global maximum and global minimum and they both occur at every domain number.

\textbf{Local Maximum and Minimum:}  Since a constant function has only one value, that value is a local maximum and local minimum and they both occur at every domain number.
...
\textbf{Behavior (Inc/Dec):} The identity function is an increasing function.
...
Power functions are powers of the identity function.


\[ Power(x) = x^n  \]

The power makes a big difference here.  As a beginning, we will only consider two types of powers: positive integers and negative integers. We don't include $0$ as a power, since that just makes a constant function and we will group fractional powers in with radical functions. 

\textbf{Domain:} 

If $n$ is a natural number, then the natural domain is $(-\infty, \infty)$.
...
If $n$ is a natural number, then $0$ is the only zero.
...
If $n$ is a natural number, then there are two different cases.

\begin{itemize}
\item If $n$ is odd, then the power function is unbounded negatively when the domain is unbounded negatively. On the other side, the power function is unbounded positively when the domain is unbounded positively.


\[
\lim\limits_{x \to -\infty} x^{odd} = -\infty   \, \text{   } \, \lim\limits_{x \to \infty} x^{odd} = \infty
\]

\item If $n$ is even, then the power function is unbounded positively in both directions.

\[
\lim\limits_{x \to -\infty} x^{even} = \infty   \, \text{   } \, \lim\limits_{x \to \infty} x^{even} = \infty
\]
...
\[
\lim\limits_{x \to -\infty} x^{neg} = 0   \, \text{   } \, \lim\limits_{x \to \infty} x^{neg} = 0
\]
...
A power function has a global minimum only if the power is an even natural number. In that case, $0$ is the global minimum, which occurs at $0$.
...
\begin{itemize}
\item If $n$ is even, then the root function has $0$ as its global minimum value and no global maximum value.
\item If $n$ is odd, then there is neither a global minimum nor a global maximum.
\end{itemize} 
...
We'll leave analysis of irrational powers for Calculus.
