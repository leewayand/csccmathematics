\documentclass{ximera}

\input{../../../preamble.tex}


\author{Lee Wayand}

\begin{document}
\begin{exercise}






\begin{idea} \textbf{\textcolor{blue!55!black}{Multiplicity}}


If we have a polynomial written in factored form

\[
p(x) = A (x-r_k)^{e_k} (x-r_{k-1})^{e_{k-1}}  \cdots ((x-r_1)^{e_1}
\]

then the multiplicity of the root $r_m$ is $e_m$. \\





\end{idea}



\begin{question}


The multiplicity of the root $2$ in $p(x) = -4 (x - 2)^3 (x + 5)^4 (x + 2)^8$ is $\answer{3}$. \\



The sign of $p$ will \wordChoice{\choice[correct]{change} \choice{stay the same}} as the domain runs over $2$.

\end{question}





\begin{question}


The multiplicity of the root $-2$ in $p(x) = -4 (x - 2)^3 (x + 5)^4 (x + 2)^8$ is $\answer{8}$. \\



The sign of $p$ will \wordChoice{\choice{change} \choice[correct]{stay the same}} as the domain runs over $-2$.

\end{question}





\begin{question}


The multiplicity of the root $-6$ in $M(t) = 5(t - 4) (t + 6)^2 (t - 1)$ is $\answer{2}$. \\


The sign of $M$ will \wordChoice{\choice{change} \choice[correct]{stay the same}} as the domain runs over $-6$.



\end{question}








\begin{question}


The multiplicity of the root $1$ in $M(t) = 5(t - 4) (t + 6)^2 (t - 1)$ is $\answer{1}$. \\



The graph will \wordChoice{\choice[correct]{cross} \choice{bounce back}} at the intercept $(1, 0)$.



\end{question}








\end{exercise}
\end{document}