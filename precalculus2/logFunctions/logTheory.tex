...
$\log_A(B)$ is the thing you raise $A$ to, to get $B$, gives us some simple algebraic rules.
...
\begin{itemize}
\item Let $L_{out}(u) = u - 4$, an increasing linear function, since the leading coefficiewnt is positive.
\item Let $L(x) = \log_2(x)$, an increasing core logarithmic function, since the base is greater than $1$.
\item Let $L_{in}(v) = v + 3$, an increasing linear function, since the leading coefficiewnt is positive.
\end{itemize}
...
\textbf{\textcolor{-red!75!green}{Algebraic Analysis}} 
