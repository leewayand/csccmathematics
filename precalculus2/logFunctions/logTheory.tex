...
From these core logarithmic function, we can transform and analyze more general logarithmic functions.
...
We are comparing back to our choice for a basic basic logarithmic function.  If we chose $\ln(x)$, then we can inventory the pieces of the formula for $M$ and compare back to $\ln(x)$, which is an increasing function (both bases are greater than $1$).



The base of $M$ is $2 > 1$, so this does not change the behavior from $\ln(x)$.

The leading coefficient of $M$ is $1$, which is positive, so this does not change the behavior, from $\ln(x)$. 

The leading coefficient of the linear inside function is $1$, which is also positive, so this does not change the behavior, from $\ln(x)$.
...
Logarithmic functins are either always increasing or always decreasing.
...
\begin{itemize}
\item Let $L_{out}(u) = u - 4$, an increasing linear function
\item Let $L(x) = \log_2(x)$, an increasing core logarithmic function
\item Let $L_{in}(v) = v + 3$, an increasing linear function
\end{itemize}
...
\item Let $L(k) = \log_3(k)$, an increasing core logarithmic function
\item Let $L_{in}(v) = 4 - v$, a decreasing linear function
\end{itemize}
