...
Logarithmis are exponents.  They are the things we raise bases to, to get targeted outputs. Therefore, to understand logarithms, we should investigate the exponents in exponential functions. \\
...
$\bigstar$ For $E(t) = 2^t$, every positive function value has \textit{exactly one} associated domain number, one exponent, one logarithm. (That is eeriely similar to the function rule for pairs in the exponential function.) \\ 
...
$\blacktriangleright$ The inside of the logarithm is $4-x$, and this is positive for $x<4$.  The graph must move to the left.
