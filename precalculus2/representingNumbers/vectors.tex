...
\begin{notation}  \textbf{\textcolor{red!50!green}{Vectors}} 
...
\begin{itemize}
\item We can think of adding a single vector to itself several times.
\item We can think of stretching or compressing a vector.
...
A \textbf{scalar} is just another name for our old real numbers.
...
Each real number $r$ is now either represented by the point $(r,0)$ on the old real line or by the horizontal vector, $\langle r, 0 \rangle$ in our new number plane.  All of our operations are duplicated with these new visual tools along the horizontal axis.


We can think of real numbers as dots on the old real number line or we can think of them as horizontal vectors in our new 2D number line.
...
Both viewpoints can be seen in our our new 2D arithmetic plane.
...
\textbf{\textcolor{red!70!darkgray}{$\blacktriangleright$}} Remember, $1$ has been encoded into vectors as $\langle 1, 0 \rangle$.  We would like $\langle 1, 0 \rangle$ to act like a multiplicative unit.
...
For instance, we would like $\langle 0, 1 \rangle * \langle 1, 0 \rangle    = \langle 0, 1 \rangle $.
...
\textbf{\textcolor{red!70!darkgray}{$\blacktriangleright$}} Multiplication by $\langle 0, 1 \rangle$ turns vectors counterclockwise $90^{\circ}$.
...
\begin{center}
\textbf{\textcolor{red!70!black}{Multiplication by $\langle 0, 1 \rangle$ turns $\langle a, b \rangle$ into $\langle -b, a \rangle$.}}
\end{center}
...
$\bullet$ We want the distributive property to hold.\\
$\bullet$ We want scalar multiplication.\\
$\bullet$ We want multiplication by $\langle 0, 1 \rangle$ to result in $90^{\circ}$ turning.
...
Demanding that the Distributive Property hold in our 2-D number system has forced a form for vector multiplication.
...
\textbf{Claim:} Our new number system contains $\sqrt{-1}$.


\[  \sqrt{-1} = \langle 0, 1 \rangle  \]


How do we know $\langle 0, 1 \rangle$ is $\sqrt{-1}$?


Let's square it with our 2D-multiplication.  We'll know it is $\sqrt{-1}$ if $(\sqrt{-1})^2 = -1$



\[   (\sqrt{-1})^2 =  \langle 0, 1 \rangle * \langle 0, 1 \rangle  \]

\[ = \langle 0 \cdot 0 - 1 \cdot 1, 0 \cdot 1 + 0 \cdot 1 \rangle  = \langle -1, 0 \rangle\]



And, $\langle -1, 0 \rangle = -1$ in our new 2D-Number system.



\begin{conclusion}  \textbf{\textcolor{green!25!black}{$\sqrt{-1}$}}



\[  \langle 0, 1 \rangle \, \text{ is } \,  \sqrt{-1}  \]


\end{conclusion}


We have invented a number system, which contains all of our usual real numbers, plus it contains $\sqrt{-1}$.
