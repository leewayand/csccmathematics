...
\textbf{Note:} So far, we have restricted our investigation to quadratics with real roots (or zeros).  Therefore, we encounter quadratics that do not factor for us with real numbers. We call these \textbf{irreducible quadratics}.  



In this course, we will bring in complex numbers and complete our characterization of quadratic functions. Complex numbers will allow us to factor all quadratics.




\begin{notation} \textbf{\textcolor{red!50!green}{Standard Form}} 
...
$\blacktriangleright$ \textbf{\textcolor{red!10!blue!90!}{More about Zeros}} 
...
Given the factored form, $Q(t) = A (t - r_1)(t - r_2)$, we can read off the roots or zeros as $r_1$ and $r_2$. Many times the leading coefficient is not factored out, if that makes the factorization look nicer.  The zero product property allows us to set each factor equal to $0$ and solve.
...
\begin{observation} \textbf{\textcolor{-red!75}{Complex Roots}}



From the vertex form, we see that the zeros or roots include the term $\sqrt{-\frac{k}{A}}$.  The inside of the square root is $-\frac{k}{A}$.  

This inside is negative if $A$ and $k$ have the same sign. 


This inside is positive if $A$ and $k$ have opposite sign. 

There are real roots when $A$ and $k$ have opposite sign in vertex form. 

Otherwise, there are complex roots, which we will examine.
...
$M$ is a quadratic function with a negative leading coefficient. It is unbounded from below.
...
$M$ is a quadratic function with a negative leading coefficient. $M$ increases and then decreases, switching at the critical number, which is $3$. 
