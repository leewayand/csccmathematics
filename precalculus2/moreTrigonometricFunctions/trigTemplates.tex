...
There is one \textbf{core sine function}. 
...
Our category of sine functions is built from composing this core sine function with linear functions
...
There is one \textbf{core cosine function}. 
...
Our category of cosine functions is built from composing this core cosine function with linear functions
...
The reason that this cannot happen is that $\sin(t) + 2$ can never equal $0$, which means that the domain of $\frac{1}{\sin(t) + 2}$ is all real numbers.
...
If we just stay with a $0$ added constant terms, then the reciprocal of sine functions are cosecant functions.
