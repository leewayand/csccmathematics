...
$\blacktriangleright$ \textbf{Continuity:} The denominator is a shifted exponential function, so it is continuous everywhere. The numberator is a constant function, so it is continuopus everywhere.  $L$ is the quotient of two continuous functions.  Therefore, it is continuous except where the denominator is $0$.  The denominator is never $0$. $L$ is continuous everywhere.  No discontinuities or singularities.
...
If there was a bump in the graph, then there would be a top point.  On either side of the top point the other points would be lower on the graph.  That would be the graph moves downhill on the right side of the top point, which would say the funciton is decreasing there.  But, we know $L$ is strictly increasing.  So, there can be no bumps.  $L$ has no local extrema.
