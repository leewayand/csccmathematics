...
When $n$ is odd, then $n = 2k+1$, $x^{\tfrac{1}{n}} = x^{\tfrac{1}{2k+1}}$ is defined to be the unique real number, $r$, such that $r^n = x$. \\
...
When $n$ is even, then $n = 2k$, $x^{\tfrac{1}{n}} = x^{\tfrac{1}{2k}}$ is defined to be the unique nonnegative real number, $r$, such that $r^n = x$. \\
...
\textbf{End-Behavior:} Root or Radical functions have end-behavior in only one direction. The domain tells us which direction.  Here, the domain is $\left(-\infty, \frac{5}{3} \right]$, whic tells us that the end-behavior is the negative tail of the domain.

$f$ is unbounded in the left tail of the domain. It is either unbounded positively or negatively. The leading coefficient tells us the end-behavior. Here the leading coefficient of $f$ is $\frac{1}{3}$, which is positive.
...
\begin{itemize}
\item $f$ is continuous, with no singularities
...
\textbf{End-Behavior:} Root or Radical functions have end-behavior in only one direction. The domain tells us which direction. Here, the domain is $(-\infty,5]$ and the end-behavior is in the left tail.

The leading coefficient tells us the end-behavior. Here the leading coefficient of $T$ is $1$, which is positive.
...
\begin{itemize}
\item $SQ(x) = \sqrt{x}$, an increasing core function
\item $L_{in}(t) = -k + 5$, a decreasing linear function
\end{itemize}


\[  T(k) =  \sqrt{-k+5} = (SQ \circ L_{in})(k) \]
...
\textbf{Continuity:} Square root functions are continuous functions.  They are continuous on their domain.
...
\[
A \sqrt[3]{B \,x + C} + D = (L_{out} \circ CR \circ L_{in})(x)
\]
...
\textbf{Continuity:} Cube root functions are continuous functions.   
