...
If we view this point on a circle of radius $r$, then we can connect polar coordinates up to rectangular coordintes and our trigonometric functions.
...
\begin{question}
  ...
  \begin{prompt}
    \[
    P = \left( \answer{2\cos (2\pi/3)}, \answer{2\sin (2\pi/3)} \right)
    \]
  \end{prompt}
  \begin{question}
  ...
  \begin{prompt}
    \[
    Q = \left( \answer{-1\cos (5\pi/4)}, \answer{-1\sin (5\pi/4)} \right)
    \]
  \end{prompt}
\end{question}
\end{question}
...
Then you continue turning counterclockwise and moving the radial measurement in and out until you get around the whole circle.
...
\begin{example}
  ...
  \begin{explanation}
	...
	\answer[given]{\frac{\sin(\theta)}{\cos^2(\theta)}}  &= r.
      \end{align*}
      We have found that $r=\sin\theta/\cos^2\theta = \tan\theta\sec\theta$. 
