...
\begin{example}
  ...
  \begin{explanation}
	...
	\answer[given]{\frac{\sin(\theta)}{\cos^2(\theta)}}  &= r.
      ...
      \begin{warning}
      When $\theta$ is inside $\left( \frac{-\pi}{2}, 0)$, then our direction is into the fourth quadrant.  However, $\sin(\theta) < 0$ in the fourth quadrant, which means $r < 0$.  This means we are plotting points in the second quadrant, which is where the parabola is.


      When $\theta$ is inside $\left( 0, \frac{\pi}{2})$, then our direction is into the first quadrant.   $\sin(\theta) > 0$ in the first quadrant, which means $r > 0$.  This means we are plotting points in the first quadrant, which is where the parabola is.
