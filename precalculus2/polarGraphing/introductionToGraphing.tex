...
Functions don't have variables. Functions are just packages of pairs. 

We represent functions using formulas and formulas use variables. 

Separate from these variables in the formulas is a geometric interpretation of the variable and function values.  We can view any function's formula as giving us rectangular information or as giving us circular information.  The formula has nothing to do with how it ``should'' be interpreted graphically.


Let's talk about how to plot functions in both rectangular and polar coordinate systems.  
...
\begin{example} Graphing in Both Coordinate Systems
...
It is sometimes desirable to refer to a graph via a polar equation $(r, \theta)$, and other times by a rectangular equation $(x, y)$.  Therefore it is necessary
to be able to convert between polar and rectangular coordinates.  Here
is the basic idea:

Given a function $y=f(x)$ thought of as written in rectangular coordinates, polar coordinates
...
Given a function $r(\theta)$ thought of as written in polar coordinates, rectangular
coordinates follow the relationships:
\[
...
\begin{warning} \textbf{\textcolor{red!70!black}{Negative $r$}}
...
\begin{warning} \textbf{\textcolor{red!70!black}{Dividing by $0$}}

      In the previous example we began with 

      \[  y = x^2 \] 

We replaced $x$ and $y$ to obtain

   \[  r \sin(\theta) = (r \cos(\theta))^2 = r^2 \, \cos^2(\theta) \] 


Then we divided both sides of the equation by $r$ and $\cos^2(\theta)$ to obtain
...
When $\theta = 0$, then $r = 0$. But then we were dividing by $0$.


Dividing both sides of an equation is dangerous, unless you know you are never ever dividing by $0$.

      $\theta = 0$ needs some thought.  When $\theta = 0$, then $\sin(\theta) = 0$ and $r = 0$.  That gives us the Cartesian point $(0,0)$, which is the bottom point on the parabola.  But, if $r = 0$ here, then how did we divide both sides of our equation above by $r$?
      ...
      In this case, we might just talk about $\theta = 0$ separately, by itself, and then let the algebra descibe the rest of the points.  

For some angles, we just stick with 

\[  r \sin(\theta) = r^2 \, \cos^2(\theta) \] 

FOr instance, when $\theta = 0$, we get 

\[  r \cdot 0 = r^2 \cdot 1 \] 

This tells us that $r = 0$.

Otherwise, the $r$ will never equal $0$ and our division is ok.
...
\begin{idea}  \textbf{\textcolor{blue!55!black}{Factor instead of Dividing}} 
