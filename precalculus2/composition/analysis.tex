...
\textbf{Analyze $H(x) = 5 \sqrt{x^2-3}$ with its natural domain. }
...
Algebraically, this produces the formula we want, $(f \circ g)(x) = f(g(x)) = 5 \sqrt{x^2-3}$.

To get us started, we are thinking that each component function comes with its natural domain.

\begin{itemize}
\item Let $f(t) = 5 \sqrt{t}$ with natural domain $[0, \infty)$ 
\item Let $g(y) = y^2 - 3$ with natural domain $(-\infty, \infty)$ 
\end{itemize}
...
Since, the natural domain of $g$ is $(-\infty, \infty)$, the domain of $H$ is also $(-\infty, -\sqrt{3} ] \cup [ \sqrt{3}, \infty)$. 
...
There are no other critical numbers.
...
The domain of the function $H$ is made of two separate intervals. In mathematics, we would say the domain is \textbf{disconnected}.  The two intervals, $( -\infty, -\sqrt{3} ]$ and $[ \sqrt{3} ,\infty)$ are maximal intervals in the domain of $H$.

Each maximal interval in the domain of $H$ has its own range for $H$.

We'll get these separate ranges and then their union will be the whole range of $H$.
