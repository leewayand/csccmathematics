...
\textbf{Analyze $K(x) = -4 \ln(5 - x^2)$ with its natural domain. }
...
\begin{itemize}
\item Let $f(t) = -4 \ln(t)$ with natural domain $(0, \infty)$ 
\item Let $g(y) = 5 - y^2$ with natural domain $(-\infty, \infty)$ 
\end{itemize}
...
Since, $f(t) = -4 \ln(t)$ is a logarithmic function with a negative leading coefficient and the inside linear function has a positive leading coefficient, we know that $f$ is a decreasing logarithmic function.
...
We already found the zeros of $g$.  They were $-\sqrt{5}$ and $\sqrt{5}$.  Now we have to think about which side of $-\sqrt{5}$ and $\sqrt{5}$ will make $g$ positive. 
...
$K$ is continuous (with no singularities), decreasing and then increasing, which means $K(0) = -4 \ln(5)$ is a local and global minimum. 
...
Since $K$ is continuous (with no singularities), the range is $[-4 \ln(5), \infty)$.
