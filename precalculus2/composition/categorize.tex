...
In Precalculus, we want to supply reasoning for our analysis choices.  Identifying the category for the function often is the reasoning, because we have a list of characteristics for each function type. 
...
\textbf{Note:} For us, in Precalculus, if the inside of the function is not linear, then it isn't an elementary function. 
...
If a function is one of our elementary functions, then that is how we would like to categorize it. We select the category and then we are done.

If a function is not one of our elementary functions, then our next preference is to see it is an operation.
...
\textbf{Note:} A constant multiple is a product in which one factor is a constant. This distinction is just helpful for Calculus.
...
This is our default category.  It signals us to use the Chain Rule in Calculus. 
...
\textbf{Note:} EVERY function can be viewed as a composition. That is why it is our third choice.  Otherwise, we woudl just say everything is a composition and gain no information.
