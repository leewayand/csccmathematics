...
\textbf{Analyze $F(x) = -3 \log_4(\sqrt{3 - 5x} - 1)$ with its natural domain. }
...
\begin{itemize}
\item Let $T(w) = -3 \log_4(w) + 2$ with natural domain $(0, \infty)$\\
\item Let $B(y) = \sqrt{3 - 5y} - 1$ with natural domain $\left(-\infty, \frac{3}{5} \right]$\\
\end{itemize}
...
This is because $B(y) = \sqrt{3 - 5y} - 1$ is a square root function with inside and outside leading coefficients of opposite signs. 
...
This is because $-3 \log_4(w) + 2$ is a logarithmic function with inside and outside leading coefficients of opposite signs, which makes it a decreasing logarithmic function.
...
From the end-behavior and singularity behavior, we know that 
...
Therefore, there is no global maximum or minimum. 
...
$F$ is continuous on $\left( -\infty, \frac{2}{5} \right)$ with 
