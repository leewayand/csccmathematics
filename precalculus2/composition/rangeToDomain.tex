\documentclass{ximera}

\input{../../preamble.tex}

\title{Chaining}

\begin{document}

\begin{abstract}
range to domain
\end{abstract}
\maketitle



Compositions are common place.

It is much easier to think of smaller, simplier component functions and how they fit together, than it is to contemplate the composition as a whole.

It is often necessary to trim the domain and range to fit the chain together.



\begin{idea} \textbf{\textcolor{blue!55!black}{Composition}}  


Let $f$ be a function with domain $D_f$.


Let $g$ be a function with domain $D_g$.




Then the \textbf{composition of f and g, $f \circ g$,} is defined as

\[
(f \circ g)(x) = f(g(x))
\] 


The value of $g$ becomes a domain number for $f$.


The composition is defined on a subset of the domain of $g$.  The composition is defined at those numbers in the domain of $g$ where the value of $g$ is in the domain of $f$.




\end{idea}

The values of $g$ (range) are becoming domain numbers for $f$. 



\begin{warning} \textbf{\textcolor{red!70!black}{Domain}}

The domain of $(f \circ g)(x) = f(g(x))$ is not the domain of $g$.

The domain of $(f \circ g)(x) = f(g(x))$ is a subset of the domain of $g$.

The domain of $(f \circ g)(x) = f(g(x))$ consists of the domain numbers of $g$, for which the value of $g$ is in the domain of $f$.


\end{warning}


We can only use numbers in the domain of the inner function when their function value is inside the domain of the outer function.




\subsection*{Domain}



Consider these component functions.

Let $X(a) = -\sqrt{3 - a}$ with its natural domain.


Let $Y(b) = \ln(1 + b)$ with its natural domain.


Let $Z(c) = \frac{3}{5 - c}$ with its natural domain.



\begin{question}

Is $2$ in the domain of $X$?

\begin{multipleChoice}
\choice [correct] {Yes}
\choice {No}
\end{multipleChoice}

\end{question}





\begin{question}

Is $2$ in the domain of $Y \circ X$?

\begin{multipleChoice}
\choice {Yes}
\choice [correct] {No}
\end{multipleChoice}

\end{question}



\begin{question}

Is $2$ in the domain of $Z \circ X$?

\begin{multipleChoice}
\choice [correct] {Yes}
\choice {No}
\end{multipleChoice}

\end{question}



We cannot always use the full domain of the inner component function as the domain of the composition.

Sometimes, we must remove items from the full domain of the inner component function so that the collection of function values for the inner function are inside the domain of the outer function.
























\begin{example}


Let $X(a) = -\sqrt{3 - a}$ with its natural domain.


Let $Y(b) = \ln(1 + b)$ with its natural domain.


Let $Z(c) = \frac{3}{5 - c}$ with its natural domain.



\begin{question}

Is $27$ in the domain of $Y$?

\begin{multipleChoice}
\choice [correct] {Yes}
\choice {No}
\end{multipleChoice}

\end{question}





\begin{question}

Is $27$ in the domain of $X \circ Y$?

\begin{multipleChoice}
\choice {Yes}
\choice [correct] {No}
\end{multipleChoice}

\end{question}



\begin{question}

Is $27$ in the domain of $Z \circ Y$?

\begin{multipleChoice}
\choice [correct] {Yes}
\choice {No}
\end{multipleChoice}

\end{question}




\end{example}
















\begin{example}


Let $X(a) = -\sqrt{3 - a}$ with its natural domain.


Let $Y(b) = \ln(1 + b)$ with its natural domain.


Let $Z(c) = \frac{3}{5 - c}$ with its natural domain.



\begin{question}

Is $5$ in the domain of $Z$?

\begin{multipleChoice}
\choice {Yes}
\choice [correct]{No}
\end{multipleChoice}

\end{question}





\begin{question}

Is $5$ in the domain of $X \circ Z$?

\begin{multipleChoice}
\choice {Yes}
\choice [correct] {No}
\end{multipleChoice}

\end{question}



\begin{question}

Is $5$ in the domain of $Y \circ Z$?

\begin{multipleChoice}
\choice {Yes}
\choice [correct] {No}
\end{multipleChoice}

\end{question}




\end{example}






\subsection*{Domain : Graphically}

Define $f$ and $g$ graphically as follows:



\begin{image}
\begin{tikzpicture}
    \begin{axis}[name = sinax, domain=-10:10, ymax=10, xmax=10, ymin=-10, xmin=-10,
            axis lines =center, xlabel=$x$, ylabel={$y=f(x)$}, grid = major, grid style={dashed},
            ytick={-10,-8,-6,-4,-2,2,4,6,8,10},
            xtick={-10,-8,-6,-4,-2,2,4,6,8,10},
            yticklabels={$-10$,$-8$,$-6$,$-4$,$-2$,$2$,$4$,$6$,$8$,$10$}, 
            xticklabels={$-10$,$-8$,$-6$,$-4$,$-2$,$2$,$4$,$6$,$8$,$10$},
            ticklabel style={font=\scriptsize},
            every axis y label/.style={at=(current axis.above origin),anchor=south},
            every axis x label/.style={at=(current axis.right of origin),anchor=west},
            axis on top
          ]
          
          \addplot [line width=2, penColor, smooth,samples=100,domain=(-10:-6)] ({x},{-0.5*x+1});
          \addplot [color=penColor,only marks,mark=*] coordinates{(-10,6)};
          \addplot [color=penColor,fill=background,only marks,mark=*] coordinates{(-6,4)}; 


          \addplot [line width=2, penColor, smooth,samples=100,domain=(0:5)] ({x},{0.5*x+1});
          \addplot [color=penColor,only marks,mark=*] coordinates{(0,1)};
          \addplot [color=penColor,only marks,mark=*] coordinates{(5,3.5)}; 

           

    \end{axis}
    \begin{axis}[at={(sinax.outer east)},anchor=outer west, domain=-10:10, ymax=10, xmax=10, ymin=-10, xmin=-10,
            axis lines =center, xlabel=$t$, ylabel={$y=g(t)$}, grid = major, grid style={dashed},
            ytick={-10,-8,-6,-4,-2,2,4,6,8,10},
            xtick={-10,-8,-6,-4,-2,2,4,6,8,10},
            yticklabels={$-10$,$-8$,$-6$,$-4$,$-2$,$2$,$4$,$6$,$8$,$10$}, 
            xticklabels={$-10$,$-8$,$-6$,$-4$,$-2$,$2$,$4$,$6$,$8$,$10$},
            ticklabel style={font=\scriptsize},
            every axis y label/.style={at=(current axis.above origin),anchor=south},
            every axis x label/.style={at=(current axis.right of origin),anchor=west},
            axis on top
          ]
          
          \addplot [line width=2, penColor2, smooth,samples=100,domain=(-6:8)] ({x},{x-3});
          \addplot [color=penColor2,only marks,mark=*] coordinates{(-6,-9)};
          \addplot [color=penColor2,fill=background,only marks,mark=*] coordinates{(8,5)}; 


  \end{axis}

\end{tikzpicture}
\end{image}






\begin{question}

Is $2$ in the domain of $f \circ g$?

\begin{multipleChoice}
\choice {Yes}
\choice [correct] {No}
\end{multipleChoice}


Is $2$ in the domain of $g \circ f$?

\begin{multipleChoice}
\choice [correct] {Yes}
\choice {No}
\end{multipleChoice}


\end{question}








\begin{question}

Is $1$ in the domain of $f \circ g$?

\begin{multipleChoice}
\choice {Yes}
\choice [correct] {No}
\end{multipleChoice}


Is $1$ in the domain of $g \circ f$?

\begin{multipleChoice}
\choice [correct] {Yes}
\choice {No}
\end{multipleChoice}


\end{question}



























Define $f$ and $g$ graphically as follows:



\begin{image}
\begin{tikzpicture}
    \begin{axis}[name = sinax, domain=-10:10, ymax=10, xmax=10, ymin=-10, xmin=-10,
            axis lines =center, xlabel=$x$, ylabel={$y=f(x)$}, grid = major, grid style={dashed},
            ytick={-10,-8,-6,-4,-2,2,4,6,8,10},
            xtick={-10,-8,-6,-4,-2,2,4,6,8,10},
            yticklabels={$-10$,$-8$,$-6$,$-4$,$-2$,$2$,$4$,$6$,$8$,$10$}, 
            xticklabels={$-10$,$-8$,$-6$,$-4$,$-2$,$2$,$4$,$6$,$8$,$10$},
            ticklabel style={font=\scriptsize},
            every axis y label/.style={at=(current axis.above origin),anchor=south},
            every axis x label/.style={at=(current axis.right of origin),anchor=west},
            axis on top
          ]
          
          \addplot [line width=2, penColor, smooth,samples=100,domain=(-10:-6)] ({x},{-0.5*x+1});
          \addplot [color=penColor,only marks,mark=*] coordinates{(-10,6)};
          \addplot [color=penColor,fill=background,only marks,mark=*] coordinates{(-6,4)}; 


          \addplot [line width=2, penColor, smooth,samples=100,domain=(0:5)] ({x},{0.5*x+1});
          \addplot [color=penColor,only marks,mark=*] coordinates{(0,1)};
          \addplot [color=penColor,only marks,mark=*] coordinates{(5,3.5)}; 

           

    \end{axis}
    \begin{axis}[at={(sinax.outer east)},anchor=outer west, domain=-10:10, ymax=10, xmax=10, ymin=-10, xmin=-10,
            axis lines =center, xlabel=$t$, ylabel={$y=g(t)$}, grid = major, grid style={dashed},
            ytick={-10,-8,-6,-4,-2,2,4,6,8,10},
            xtick={-10,-8,-6,-4,-2,2,4,6,8,10},
            yticklabels={$-10$,$-8$,$-6$,$-4$,$-2$,$2$,$4$,$6$,$8$,$10$}, 
            xticklabels={$-10$,$-8$,$-6$,$-4$,$-2$,$2$,$4$,$6$,$8$,$10$},
            ticklabel style={font=\scriptsize},
            every axis y label/.style={at=(current axis.above origin),anchor=south},
            every axis x label/.style={at=(current axis.right of origin),anchor=west},
            axis on top
          ]
          
          \addplot [line width=2, penColor2, smooth,samples=100,domain=(-4:0)] ({x},{-x-4});
          \addplot [color=penColor2,fill=background, only marks,mark=*] coordinates{(0,-4)};
          \addplot [color=penColor2,fill=background,only marks,mark=*] coordinates{(-4,0)}; 


          \addplot [line width=2, penColor2, smooth,samples=100,domain=(0:5)] ({x},{x-8});
          \addplot [color=penColor2,only marks,mark=*] coordinates{(0,-8)};
          \addplot [color=penColor2,fill=background,only marks,mark=*] coordinates{(5,-3)}; 

  \end{axis}

\end{tikzpicture}
\end{image}




















\begin{question}

Is $1$ in the domain of $f \circ g$?

\begin{multipleChoice}
\choice [correct] {Yes}
\choice {No}
\end{multipleChoice}


Is $1$ in the domain of $g \circ f$?

\begin{multipleChoice}
\choice [correct] {Yes}
\choice {No}
\end{multipleChoice}


\end{question}









\begin{question}

Is $-1$ in the domain of $f \circ g$?

\begin{multipleChoice}
\choice {Yes}
\choice [correct] {No}
\end{multipleChoice}


Is $-1$ in the domain of $g \circ f$?

\begin{multipleChoice}
\choice {Yes}
\choice [correct] {No}
\end{multipleChoice}


\end{question}






\begin{question}

Is $0$ in the domain of $g \circ g$?

\begin{multipleChoice}
\choice {Yes}
\choice [correct] {No}
\end{multipleChoice}


Is $1$ in the domain of $g \circ g$?

\begin{multipleChoice}
\choice {Yes}
\choice [correct] {No}
\end{multipleChoice}



Is $2$ in the domain of $g \circ g$?

\begin{multipleChoice}
\choice {Yes}
\choice [correct] {No}
\end{multipleChoice}



\end{question}






\begin{question}

Is $-2$ in the domain of $f \circ g$?

\begin{multipleChoice}
\choice {Yes}
\choice [correct] {No}
\end{multipleChoice}


Is $-2$ in the domain of $g \circ f$?

\begin{multipleChoice}
\choice {Yes}
\choice [correct] {No}
\end{multipleChoice}



Is $-2$ in the domain of $g \circ g$?

\begin{multipleChoice}
\choice [correct] {Yes}
\choice {No}
\end{multipleChoice}


Is $-2$ in the domain of $f \circ f$?

\begin{multipleChoice}
\choice {Yes}
\choice [correct] {No}
\end{multipleChoice}




\end{question}




\subsection*{Range Values}

The range of the whole composition is inside the range of the outer function.  However, if we are trimming the domain, the values of the composition may not make up the whole range of the outer function.






Let $H(x) = e^x + 20$ and $K(y) = \sqrt{y}$. 

Let $C(t) = (K \circ H)(t) = K(H(t))$. 




\textbf{For instance},

$4$ is in the range of $K$, since $16$ is in the natural domain of $K$ and $K(16) = \sqrt{16} = 4$. \\

However, $4$ is not in the range of $K \circ H$, because the value of $H$ is never $16$. 









\begin{question}



Is $1$ in the range of $K$?

\begin{multipleChoice}
\choice [correct]{Yes}
\choice {No}
\end{multipleChoice}





Is $1$ in the range of $K \circ H$?

\begin{multipleChoice}
\choice {Yes}
\choice [correct]{No}
\end{multipleChoice}


\end{question}


For $K(y) = \sqrt{y} = 1$, we need $y=1$.  However, the values of $K$ are never less than $20$.  The function $H$ can never supply the value needed for $K$ to equal $1$.






\begin{warning} \textbf{\textcolor{red!70!black}{Range}}


The range of $K \circ H$ is inside the range of $K$, but it might not be all of the range of $K$. 


It depends on how much of the domain of $K$ gets covered by the range of $H$.


\end{warning}
























\begin{center}
\textbf{\textcolor{green!50!black}{ooooo-=-=-=-ooOoo-=-=-=-ooooo}} \\

more examples can be found by following this link\\ \link[More Examples of Composition]{https://ximera.osu.edu/csccmathematics/precalculus2/precalculus2/composition/examples/exampleList}

\end{center}








\end{document}
