...
\begin{notation} \textbf{\textcolor{red!50!green}{Unit Component Vectors}}  

$\blacktriangleright$ $\langle 1, 0 \rangle$ is often written as $\hat{x}$. It represents a vector of length $1$ pointing in the positive $x$-direction.

$\blacktriangleright$ $\langle 0, 1 \rangle$ is often written as $\hat{y}$. It represents a vector of length $1$ pointing in the positive $y$-direction.
...
$\blacktriangleright$  As algebraic shorthand notation, the vector $ \langle 0, 1 \rangle $ is historically abbreviated as $i$.  It represents the square root of negative $1$, i.e. $\sqrt{-1}$.
...
Our 2-dimensional numbers have several representations.  Points and vectors are a geometric view, which will use extensively.
 ...
 $a + b \, i$, is our algebraic representation of a \textbf{Complex Number}. 
...
The Complex Numbers are our algebraic view of our new 2-dimensional numbers.  

In our story, the line between geometry and algebra is very blurry. It is almost like having double vision.  
...
\begin{center}
\textbf{\textcolor{red!70!black}{$i$ is the missing piece to the puzzle!}}
\end{center}
...
If we represented Complex Numbers with vectors, then each vector would have a \textbf{length}.  


Inside our algebraic viewpoint, $a + b \, i$ doesn't have a length. It has a \textbf{modulus}.  The symbol for the modulus looks exactly like the absolute value symbol.
...
The unit circle is the collection of all points a distance of $1$ from the origin.  We can identify these points via complex numbers.  The unit circle consists of the complex numbers whose modulus equals $1$
...
Nobody memorizes this rule for multiplication.  Just use the Distributive Property.
...
The calculations above demonstrate that $i = \sqrt{-1}$, which means that $i^2 = -1$.  It also means that $i^4 = 1$ and then we start all over.  The powers of $i$ are cyclic with a period of $4$.  Visually, on the unit circle, the powers of $i$ rotate around the unit circle in jumps of $90^{\circ}$ or $\frac{\pi}{2}$ radians.
...
In previous courses, we avoided Complex arithmetic. And, for good reason:
...
The symbol $\sqrt{ }$ was a fine symbol when we were only talking about positive numbers.

The symbol $\sqrt{ }$ is not a very good symbol when expending our arithmetic to Complex Numbers.

Unfortunately, the square root function is not a continuous function for Complex Numbers.  There is a discontinuity over at $-1$.

The Complex Numbers are full of surprises.



This is just the beginnign of the story.  It is not the complete story, but it will get us going.  More details coming up later.
