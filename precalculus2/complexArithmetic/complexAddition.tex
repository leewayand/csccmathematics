...
Complex Numbers are 2-dimensional numbers, which we represent as $a + b \, i$, where $i = \sqrt{-1}$ or rather $i^2 = -1$.  The square root symbol is a little tricky with Complex Numbers, but it gets the point across. 





On one hand, $\sqrt{-1}$ feels a little weird.  We are used to squaring real numbers and getting positive values. On the other hand, we have worked with expressions like $5 + 3 \sqrt{2}$ and $3 - 4 \sqrt{7}$ many times.  In this respect, $5 + 3 \sqrt{-1}$ is no different.  $i = \sqrt{-1}$ is simply the number you square to get $-1$.  That's all. If we don't push this too far, we'll be ok.  On the other hand, it is just fun to push it too far.



\begin{notation} \textbf{\textcolor{red!50!green}{Shorthand Notation}} 
...
This is just what we had for vector multiplication.  However, now there is no reason to memorize this template, because you can just use the DIstributive Property, like you have always used it.
...
\begin{itemize}
\item $r + t = (r + 0 \, i) + (t + 0 \, i) = (r + t) + 0 \, i = r + t$ \\ 
\item $r - t = (r + 0 \, i) - (t + 0 \, i) = (r - t) + 0 \, i = r - t$ \\ 
\item $r \cdot t = (r + 0 \, i) \cdot (t + 0 \, i) = (r \cdot t - 0 \cdot 0) + (r \cdot 0 + t \cdot 0) \, i = r \cdot t$ \\ 
\end{itemize}
