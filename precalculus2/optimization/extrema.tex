...
\end{tikzpicture}
\end{image}
The function $f$ has three local extremums: $5$, $6$, $-1$, and $4.5$  These occur at $x=-4$, $x=-2$, $x=0$ and $x=4$.
...
Consider the function $f(x) = 6 (x-2)^{\frac{1}{2}}$.
...
\begin{tikzpicture}
  \begin{axis}[
            ...
            \addplot [very thick, penColor, smooth, domain=(-6:-4)] {6 * (x-2)^0.66666};
...
The two pieces of the graph do, in fact, become parallel as you approach the cusp. And, ther eis a tangent line.  IT is a vertical tangent line.
...
\begin{tikzpicture}
  \begin{axis}[
            ...
            \addplot [very thick, penColor, smooth, domain=(-6:-4)] {6 * (x-2)^0.66666};
            \addplot [line width=1, penColor2, smooth,samples=100,domain=(-1:6)] ({x},{2});
