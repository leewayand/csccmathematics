...
The symbol for the core absolute value function are two vertical bars surrounding the domain value $|x|$.
...
\textbf{Note:}  In the template for absolute value functions, there is a leading coefficient for the whole function, which is also the leading coefficient of the outer linear function, if you are thinking of it as a composition. 


There is also a leading coefficient for inner the linear function, which is also the leading coefficient of the inner linear function, if you are thinking of it as a composition. 
...
\item Let $L_{out}(v) = -3 \, v + 5$, a decreasing linear function.
\end{itemize}
...
Since the end-behavior was unbounded negatively, $P$ has no global minimum value.

$P$ switches from increasing to decreasing at $-2$, which means that $P(-2) = 5$ is global maximum value.


\textbf{Local Maximums and Minimums:}  $P(-2) = 5$ is the only local maximum value. There are no local minimum values, since absolute value functions only have one critical number.
...
\textbf{Zeros:} Absolute value functions can have 2 zeros, 1 zero, or no zeros.
