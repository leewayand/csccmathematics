 ...
 Evaluating the integral, we find that the area is $A= \answer[given]{-2+9 \ln(3)}$.
  \begin{hint}
    \begin{align*}
      ...
      &=\bigg[\answer[given]{9\ln(x)-x^2+3x}\bigg]_1^3\\
    \end{align*}
  \end{hint}
\end{explanation}
\end{example}
...
y_{bot} = 0  & y_{bot} = 0 \\
A_I = \int_{x= \answer[given]{0}}^{x=\answer[given]{2}} \answer[given]{\sqrt{2x}} dx   & A_{II} = \int_{x= \answer[given]{2}}^{x=\answer[given]{6}}  \answer[given]{3-\frac{1}{2}x} dx
\end{array}
\]

Putting this together, we can write down a sum of integrals that gives the area.

\[A= \int_{x= \answer[given]{0}}^{x=\answer[given]{2}} \answer[given]{\sqrt{2x}} dx + \int_{x= \answer[given]{2}}^{x=\answer[given]{6}}\answer[given]{3-\frac{1}{2}x} dx\]
...
The length $h$ of the rectangle is thus $h=x_{right}-x_{left} = \answer[given]{(6-2y)-\left(\frac{1}{2}y^2\right)}$ and the area is given by the integral
  \[
 \int_{y=c}^{y=d} h dy =  \int_{y=0}^{y=2} 6-2y-\frac{1}{2}y^2 dy .
  \]
