...
An inverted conical tank has base with diameter $6\mathrm{m}$ and height
$9\mathrm{m}$. Suppose the tank is filled to a height of $6\mathrm{m}$
with acetone ($\rho=785 \mathrm{kg}/\mathrm{m}^3$). Find the work required
to pump the acetone out of the tank (use $g=9.8\mathrm{m}/\mathrm{s}^2$).
\begin{image}
\begin{tikzpicture}
\begin{axis}[
domain=-3:3,
...
\node[anchor=west] at (axis cs:2.2,3) {$6\mathrm{m}$};
\draw[decoration={brace,raise=.1cm},decorate,thin] (axis cs:3,9) -- (axis cs:3,0);
\node[anchor=west] at (axis cs:3.2,4.5) {$9\mathrm{m}$};
...
\[
W= \int_{y=\answer{0}}^{y=\answer{6}} \answer{7693\pi\left(\frac{y}{3}\right)^2 (9-y)} dy = \answer[tolerance=10]{276948\pi} \mathrm{J}
\]
