...
A local \textit{extremum} of a function $f$ corresponds to a point $(a,f(a))$ on the graph of $f$ where the
...
\begin{enumerate}
\item A function $f$ has a \textbf{local maximum} at $x=a$, if $f(a)\ge
  f(x)$ for every $x$ in some open interval I containing $a$.
\item A function $f$ has a \textbf{local minimum} at $x=a$, if $f(a)\le
  f(x)$ for every $x$  in some open interval I containing $a$.
\end{enumerate}
...
\begin{definition}\index{critical number}
...
If $f$ is continuous at $x=a$ and its graph's concavity changes either from up to down
or down to up at $x=a$, then the graph of $f$ has an \textbf{inflection point} at
$x=a$.
\end{definition}
...
\end{warning}
\begin{example}
Describe the concavity of the graph of $f(x)=x^4$. 
...
\begin{itemize}
\item If $f'(x)>0$ for all $x$ that are near and less than $a$ and $f'(x)<0$  for all $x$ that are near and greater than
  $a$, then $f(a)$ is a local maximum.
\item If $f'(x)<0$ for all $x$ that are near and  less than $a$ and $f'(x)>0$ for all $x$ that are near and  greater than
  $a$, then $f(a)$ is a local minimum.
\end{itemize}

Let $a$ be a critical number such that $f'(a)=0$.
...
the number in question. On the other hand, if $f'$ is increasing, then it changes from
...
number in question. We summarize this as the \textit{second derivative
  test}.
...
The second derivative test is often the easiest way to identify where in the domain local
maximum and minimum occur. Sometimes the test fails and sometimes
